...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRainVessenberg;
...
public class RainVessenberg implements IRainVessenberg {
	...
	public RainVessenberg(RainVessenberg rainVessenberg) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = rainVessenberg.getAnnotationID();
		for (int j = 0; j < rainVessenberg.getBirthPlaces().size(); j++) {
			if (rainVessenberg.getBirthPlaces().get(j) != null) {
				birthPlaces.add(rainVessenberg.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(rainVessenberg.getBirthPlaces().get(j).getClass())
						.newInstance(rainVessenberg.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (rainVessenberg.getBirthYear() != null)
			this.birthYear = rainVessenberg.getBirthYear().getClass()
					.getDeclaredConstructor(rainVessenberg.getBirthYear().getClass())
					.newInstance(rainVessenberg.getBirthYear());
		this.characterOffset = rainVessenberg.getCharacterOffset();
		this.characterOnset = rainVessenberg.getCharacterOnset();
		if (rainVessenberg.getPosition() != null)
			this.position = rainVessenberg.getPosition().getClass()
					.getDeclaredConstructor(rainVessenberg.getPosition().getClass())
					.newInstance(rainVessenberg.getPosition());
		for (int j = 0; j < rainVessenberg.getTeams().size(); j++) {
			if (rainVessenberg.getTeams().get(j) != null) {
				teams.add(rainVessenberg.getTeams().get(j).getClass()
						.getDeclaredConstructor(rainVessenberg.getTeams().get(j).getClass())
						.newInstance(rainVessenberg.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RainVessenberg [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
