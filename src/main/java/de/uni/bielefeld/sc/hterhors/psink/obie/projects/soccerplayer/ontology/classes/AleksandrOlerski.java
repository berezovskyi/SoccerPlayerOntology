...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAleksandrOlerski;
...
public class AleksandrOlerski implements IAleksandrOlerski {
	...
	public AleksandrOlerski(AleksandrOlerski aleksandrOlerski) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = aleksandrOlerski.getAnnotationID();
		for (int j = 0; j < aleksandrOlerski.getBirthPlaces().size(); j++) {
			if (aleksandrOlerski.getBirthPlaces().get(j) != null) {
				birthPlaces.add(aleksandrOlerski.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(aleksandrOlerski.getBirthPlaces().get(j).getClass())
						.newInstance(aleksandrOlerski.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (aleksandrOlerski.getBirthYear() != null)
			this.birthYear = aleksandrOlerski.getBirthYear().getClass()
					.getDeclaredConstructor(aleksandrOlerski.getBirthYear().getClass())
					.newInstance(aleksandrOlerski.getBirthYear());
		this.characterOffset = aleksandrOlerski.getCharacterOffset();
		this.characterOnset = aleksandrOlerski.getCharacterOnset();
		if (aleksandrOlerski.getPosition() != null)
			this.position = aleksandrOlerski.getPosition().getClass()
					.getDeclaredConstructor(aleksandrOlerski.getPosition().getClass())
					.newInstance(aleksandrOlerski.getPosition());
		for (int j = 0; j < aleksandrOlerski.getTeams().size(); j++) {
			if (aleksandrOlerski.getTeams().get(j) != null) {
				teams.add(aleksandrOlerski.getTeams().get(j).getClass()
						.getDeclaredConstructor(aleksandrOlerski.getTeams().get(j).getClass())
						.newInstance(aleksandrOlerski.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AleksandrOlerski [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
