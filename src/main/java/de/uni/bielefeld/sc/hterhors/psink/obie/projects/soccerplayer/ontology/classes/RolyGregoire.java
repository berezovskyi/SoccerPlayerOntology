...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRolyGregoire;
...
public class RolyGregoire implements IRolyGregoire {
	...
	public RolyGregoire(RolyGregoire rolyGregoire) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = rolyGregoire.getAnnotationID();
		for (int j = 0; j < rolyGregoire.getBirthPlaces().size(); j++) {
			if (rolyGregoire.getBirthPlaces().get(j) != null) {
				birthPlaces.add(rolyGregoire.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(rolyGregoire.getBirthPlaces().get(j).getClass())
						.newInstance(rolyGregoire.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (rolyGregoire.getBirthYear() != null)
			this.birthYear = rolyGregoire.getBirthYear().getClass()
					.getDeclaredConstructor(rolyGregoire.getBirthYear().getClass())
					.newInstance(rolyGregoire.getBirthYear());
		this.characterOffset = rolyGregoire.getCharacterOffset();
		this.characterOnset = rolyGregoire.getCharacterOnset();
		if (rolyGregoire.getPosition() != null)
			this.position = rolyGregoire.getPosition().getClass()
					.getDeclaredConstructor(rolyGregoire.getPosition().getClass())
					.newInstance(rolyGregoire.getPosition());
		for (int j = 0; j < rolyGregoire.getTeams().size(); j++) {
			if (rolyGregoire.getTeams().get(j) != null) {
				teams.add(rolyGregoire.getTeams().get(j).getClass()
						.getDeclaredConstructor(rolyGregoire.getTeams().get(j).getClass())
						.newInstance(rolyGregoire.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RolyGregoire [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
