...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAlfCarroll;
...
public class AlfCarroll implements IAlfCarroll {
	...
	public AlfCarroll(AlfCarroll alfCarroll) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = alfCarroll.getAnnotationID();
		for (int j = 0; j < alfCarroll.getBirthPlaces().size(); j++) {
			if (alfCarroll.getBirthPlaces().get(j) != null) {
				birthPlaces.add(alfCarroll.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(alfCarroll.getBirthPlaces().get(j).getClass())
						.newInstance(alfCarroll.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (alfCarroll.getBirthYear() != null)
			this.birthYear = alfCarroll.getBirthYear().getClass()
					.getDeclaredConstructor(alfCarroll.getBirthYear().getClass())
					.newInstance(alfCarroll.getBirthYear());
		this.characterOffset = alfCarroll.getCharacterOffset();
		this.characterOnset = alfCarroll.getCharacterOnset();
		if (alfCarroll.getPosition() != null)
			this.position = alfCarroll.getPosition().getClass()
					.getDeclaredConstructor(alfCarroll.getPosition().getClass()).newInstance(alfCarroll.getPosition());
		for (int j = 0; j < alfCarroll.getTeams().size(); j++) {
			if (alfCarroll.getTeams().get(j) != null) {
				teams.add(alfCarroll.getTeams().get(j).getClass()
						.getDeclaredConstructor(alfCarroll.getTeams().get(j).getClass())
						.newInstance(alfCarroll.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AlfCarroll [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
