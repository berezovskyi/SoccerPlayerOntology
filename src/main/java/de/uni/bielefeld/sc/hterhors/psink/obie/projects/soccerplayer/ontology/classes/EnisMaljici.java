...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IEnisMaljici;
...
public class EnisMaljici implements IEnisMaljici {
	...
	public EnisMaljici(EnisMaljici enisMaljici) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = enisMaljici.getAnnotationID();
		for (int j = 0; j < enisMaljici.getBirthPlaces().size(); j++) {
			if (enisMaljici.getBirthPlaces().get(j) != null) {
				birthPlaces.add(enisMaljici.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(enisMaljici.getBirthPlaces().get(j).getClass())
						.newInstance(enisMaljici.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (enisMaljici.getBirthYear() != null)
			this.birthYear = enisMaljici.getBirthYear().getClass()
					.getDeclaredConstructor(enisMaljici.getBirthYear().getClass())
					.newInstance(enisMaljici.getBirthYear());
		this.characterOffset = enisMaljici.getCharacterOffset();
		this.characterOnset = enisMaljici.getCharacterOnset();
		if (enisMaljici.getPosition() != null)
			this.position = enisMaljici.getPosition().getClass()
					.getDeclaredConstructor(enisMaljici.getPosition().getClass())
					.newInstance(enisMaljici.getPosition());
		for (int j = 0; j < enisMaljici.getTeams().size(); j++) {
			if (enisMaljici.getTeams().get(j) != null) {
				teams.add(enisMaljici.getTeams().get(j).getClass()
						.getDeclaredConstructor(enisMaljici.getTeams().get(j).getClass())
						.newInstance(enisMaljici.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "EnisMaljici [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
