...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDavidMacgregor;
...
public class DavidMacgregor implements IDavidMacgregor {
	...
	public DavidMacgregor(DavidMacgregor davidMacgregor) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = davidMacgregor.getAnnotationID();
		for (int j = 0; j < davidMacgregor.getBirthPlaces().size(); j++) {
			if (davidMacgregor.getBirthPlaces().get(j) != null) {
				birthPlaces.add(davidMacgregor.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(davidMacgregor.getBirthPlaces().get(j).getClass())
						.newInstance(davidMacgregor.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (davidMacgregor.getBirthYear() != null)
			this.birthYear = davidMacgregor.getBirthYear().getClass()
					.getDeclaredConstructor(davidMacgregor.getBirthYear().getClass())
					.newInstance(davidMacgregor.getBirthYear());
		this.characterOffset = davidMacgregor.getCharacterOffset();
		this.characterOnset = davidMacgregor.getCharacterOnset();
		if (davidMacgregor.getPosition() != null)
			this.position = davidMacgregor.getPosition().getClass()
					.getDeclaredConstructor(davidMacgregor.getPosition().getClass())
					.newInstance(davidMacgregor.getPosition());
		for (int j = 0; j < davidMacgregor.getTeams().size(); j++) {
			if (davidMacgregor.getTeams().get(j) != null) {
				teams.add(davidMacgregor.getTeams().get(j).getClass()
						.getDeclaredConstructor(davidMacgregor.getTeams().get(j).getClass())
						.newInstance(davidMacgregor.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DavidMacgregor [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
