...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IHorstNemec;
...
public class HorstNemec implements IHorstNemec {
	...
	public HorstNemec(HorstNemec horstNemec) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = horstNemec.getAnnotationID();
		for (int j = 0; j < horstNemec.getBirthPlaces().size(); j++) {
			if (horstNemec.getBirthPlaces().get(j) != null) {
				birthPlaces.add(horstNemec.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(horstNemec.getBirthPlaces().get(j).getClass())
						.newInstance(horstNemec.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (horstNemec.getBirthYear() != null)
			this.birthYear = horstNemec.getBirthYear().getClass()
					.getDeclaredConstructor(horstNemec.getBirthYear().getClass())
					.newInstance(horstNemec.getBirthYear());
		this.characterOffset = horstNemec.getCharacterOffset();
		this.characterOnset = horstNemec.getCharacterOnset();
		if (horstNemec.getPosition() != null)
			this.position = horstNemec.getPosition().getClass()
					.getDeclaredConstructor(horstNemec.getPosition().getClass()).newInstance(horstNemec.getPosition());
		for (int j = 0; j < horstNemec.getTeams().size(); j++) {
			if (horstNemec.getTeams().get(j) != null) {
				teams.add(horstNemec.getTeams().get(j).getClass()
						.getDeclaredConstructor(horstNemec.getTeams().get(j).getClass())
						.newInstance(horstNemec.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "HorstNemec [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
