...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.INormanHeath;
...
public class NormanHeath implements INormanHeath {
	...
	public NormanHeath(NormanHeath normanHeath) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = normanHeath.getAnnotationID();
		for (int j = 0; j < normanHeath.getBirthPlaces().size(); j++) {
			if (normanHeath.getBirthPlaces().get(j) != null) {
				birthPlaces.add(normanHeath.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(normanHeath.getBirthPlaces().get(j).getClass())
						.newInstance(normanHeath.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (normanHeath.getBirthYear() != null)
			this.birthYear = normanHeath.getBirthYear().getClass()
					.getDeclaredConstructor(normanHeath.getBirthYear().getClass())
					.newInstance(normanHeath.getBirthYear());
		this.characterOffset = normanHeath.getCharacterOffset();
		this.characterOnset = normanHeath.getCharacterOnset();
		if (normanHeath.getPosition() != null)
			this.position = normanHeath.getPosition().getClass()
					.getDeclaredConstructor(normanHeath.getPosition().getClass())
					.newInstance(normanHeath.getPosition());
		for (int j = 0; j < normanHeath.getTeams().size(); j++) {
			if (normanHeath.getTeams().get(j) != null) {
				teams.add(normanHeath.getTeams().get(j).getClass()
						.getDeclaredConstructor(normanHeath.getTeams().get(j).getClass())
						.newInstance(normanHeath.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "NormanHeath [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
