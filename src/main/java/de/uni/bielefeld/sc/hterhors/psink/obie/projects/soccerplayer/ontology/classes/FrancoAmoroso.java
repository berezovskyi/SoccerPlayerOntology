...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IFrancoAmoroso;
...
public class FrancoAmoroso implements IFrancoAmoroso {
	...
	public FrancoAmoroso(FrancoAmoroso francoAmoroso) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = francoAmoroso.getAnnotationID();
		for (int j = 0; j < francoAmoroso.getBirthPlaces().size(); j++) {
			if (francoAmoroso.getBirthPlaces().get(j) != null) {
				birthPlaces.add(francoAmoroso.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(francoAmoroso.getBirthPlaces().get(j).getClass())
						.newInstance(francoAmoroso.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (francoAmoroso.getBirthYear() != null)
			this.birthYear = francoAmoroso.getBirthYear().getClass()
					.getDeclaredConstructor(francoAmoroso.getBirthYear().getClass())
					.newInstance(francoAmoroso.getBirthYear());
		this.characterOffset = francoAmoroso.getCharacterOffset();
		this.characterOnset = francoAmoroso.getCharacterOnset();
		if (francoAmoroso.getPosition() != null)
			this.position = francoAmoroso.getPosition().getClass()
					.getDeclaredConstructor(francoAmoroso.getPosition().getClass())
					.newInstance(francoAmoroso.getPosition());
		for (int j = 0; j < francoAmoroso.getTeams().size(); j++) {
			if (francoAmoroso.getTeams().get(j) != null) {
				teams.add(francoAmoroso.getTeams().get(j).getClass()
						.getDeclaredConstructor(francoAmoroso.getTeams().get(j).getClass())
						.newInstance(francoAmoroso.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "FrancoAmoroso [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
