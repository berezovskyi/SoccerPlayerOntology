...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILesterLanglais;
...
public class LesterLanglais implements ILesterLanglais {
	...
	public LesterLanglais(LesterLanglais lesterLanglais) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = lesterLanglais.getAnnotationID();
		for (int j = 0; j < lesterLanglais.getBirthPlaces().size(); j++) {
			if (lesterLanglais.getBirthPlaces().get(j) != null) {
				birthPlaces.add(lesterLanglais.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(lesterLanglais.getBirthPlaces().get(j).getClass())
						.newInstance(lesterLanglais.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (lesterLanglais.getBirthYear() != null)
			this.birthYear = lesterLanglais.getBirthYear().getClass()
					.getDeclaredConstructor(lesterLanglais.getBirthYear().getClass())
					.newInstance(lesterLanglais.getBirthYear());
		this.characterOffset = lesterLanglais.getCharacterOffset();
		this.characterOnset = lesterLanglais.getCharacterOnset();
		if (lesterLanglais.getPosition() != null)
			this.position = lesterLanglais.getPosition().getClass()
					.getDeclaredConstructor(lesterLanglais.getPosition().getClass())
					.newInstance(lesterLanglais.getPosition());
		for (int j = 0; j < lesterLanglais.getTeams().size(); j++) {
			if (lesterLanglais.getTeams().get(j) != null) {
				teams.add(lesterLanglais.getTeams().get(j).getClass()
						.getDeclaredConstructor(lesterLanglais.getTeams().get(j).getClass())
						.newInstance(lesterLanglais.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LesterLanglais [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
