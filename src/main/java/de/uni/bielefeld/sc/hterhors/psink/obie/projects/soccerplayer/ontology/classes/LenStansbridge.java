...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILenStansbridge;
...
public class LenStansbridge implements ILenStansbridge {
	...
	public LenStansbridge(LenStansbridge lenStansbridge) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = lenStansbridge.getAnnotationID();
		for (int j = 0; j < lenStansbridge.getBirthPlaces().size(); j++) {
			if (lenStansbridge.getBirthPlaces().get(j) != null) {
				birthPlaces.add(lenStansbridge.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(lenStansbridge.getBirthPlaces().get(j).getClass())
						.newInstance(lenStansbridge.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (lenStansbridge.getBirthYear() != null)
			this.birthYear = lenStansbridge.getBirthYear().getClass()
					.getDeclaredConstructor(lenStansbridge.getBirthYear().getClass())
					.newInstance(lenStansbridge.getBirthYear());
		this.characterOffset = lenStansbridge.getCharacterOffset();
		this.characterOnset = lenStansbridge.getCharacterOnset();
		if (lenStansbridge.getPosition() != null)
			this.position = lenStansbridge.getPosition().getClass()
					.getDeclaredConstructor(lenStansbridge.getPosition().getClass())
					.newInstance(lenStansbridge.getPosition());
		for (int j = 0; j < lenStansbridge.getTeams().size(); j++) {
			if (lenStansbridge.getTeams().get(j) != null) {
				teams.add(lenStansbridge.getTeams().get(j).getClass()
						.getDeclaredConstructor(lenStansbridge.getTeams().get(j).getClass())
						.newInstance(lenStansbridge.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LenStansbridge [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
