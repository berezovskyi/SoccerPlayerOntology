...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.INieTao;
...
public class NieTao implements INieTao {
	...
	public NieTao(NieTao nieTao) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = nieTao.getAnnotationID();
		for (int j = 0; j < nieTao.getBirthPlaces().size(); j++) {
			if (nieTao.getBirthPlaces().get(j) != null) {
				birthPlaces.add(nieTao.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(nieTao.getBirthPlaces().get(j).getClass())
						.newInstance(nieTao.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (nieTao.getBirthYear() != null)
			this.birthYear = nieTao.getBirthYear().getClass().getDeclaredConstructor(nieTao.getBirthYear().getClass())
					.newInstance(nieTao.getBirthYear());
		this.characterOffset = nieTao.getCharacterOffset();
		this.characterOnset = nieTao.getCharacterOnset();
		if (nieTao.getPosition() != null)
			this.position = nieTao.getPosition().getClass().getDeclaredConstructor(nieTao.getPosition().getClass())
					.newInstance(nieTao.getPosition());
		for (int j = 0; j < nieTao.getTeams().size(); j++) {
			if (nieTao.getTeams().get(j) != null) {
				teams.add(
						nieTao.getTeams().get(j).getClass().getDeclaredConstructor(nieTao.getTeams().get(j).getClass())
								.newInstance(nieTao.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "NieTao [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
