...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBernardMiddleditch;
...
public class BernardMiddleditch implements IBernardMiddleditch {
	...
	public BernardMiddleditch(BernardMiddleditch bernardMiddleditch)
			throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
			NoSuchMethodException, SecurityException {
		this.annotationID = bernardMiddleditch.getAnnotationID();
		for (int j = 0; j < bernardMiddleditch.getBirthPlaces().size(); j++) {
			if (bernardMiddleditch.getBirthPlaces().get(j) != null) {
				birthPlaces.add(bernardMiddleditch.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(bernardMiddleditch.getBirthPlaces().get(j).getClass())
						.newInstance(bernardMiddleditch.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (bernardMiddleditch.getBirthYear() != null)
			this.birthYear = bernardMiddleditch.getBirthYear().getClass()
					.getDeclaredConstructor(bernardMiddleditch.getBirthYear().getClass())
					.newInstance(bernardMiddleditch.getBirthYear());
		this.characterOffset = bernardMiddleditch.getCharacterOffset();
		this.characterOnset = bernardMiddleditch.getCharacterOnset();
		if (bernardMiddleditch.getPosition() != null)
			this.position = bernardMiddleditch.getPosition().getClass()
					.getDeclaredConstructor(bernardMiddleditch.getPosition().getClass())
					.newInstance(bernardMiddleditch.getPosition());
		for (int j = 0; j < bernardMiddleditch.getTeams().size(); j++) {
			if (bernardMiddleditch.getTeams().get(j) != null) {
				teams.add(bernardMiddleditch.getTeams().get(j).getClass()
						.getDeclaredConstructor(bernardMiddleditch.getTeams().get(j).getClass())
						.newInstance(bernardMiddleditch.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BernardMiddleditch [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
