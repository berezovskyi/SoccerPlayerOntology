...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDelroyScott;
...
public class DelroyScott implements IDelroyScott {
	...
	public DelroyScott(DelroyScott delroyScott) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = delroyScott.getAnnotationID();
		for (int j = 0; j < delroyScott.getBirthPlaces().size(); j++) {
			if (delroyScott.getBirthPlaces().get(j) != null) {
				birthPlaces.add(delroyScott.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(delroyScott.getBirthPlaces().get(j).getClass())
						.newInstance(delroyScott.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (delroyScott.getBirthYear() != null)
			this.birthYear = delroyScott.getBirthYear().getClass()
					.getDeclaredConstructor(delroyScott.getBirthYear().getClass())
					.newInstance(delroyScott.getBirthYear());
		this.characterOffset = delroyScott.getCharacterOffset();
		this.characterOnset = delroyScott.getCharacterOnset();
		if (delroyScott.getPosition() != null)
			this.position = delroyScott.getPosition().getClass()
					.getDeclaredConstructor(delroyScott.getPosition().getClass())
					.newInstance(delroyScott.getPosition());
		for (int j = 0; j < delroyScott.getTeams().size(); j++) {
			if (delroyScott.getTeams().get(j) != null) {
				teams.add(delroyScott.getTeams().get(j).getClass()
						.getDeclaredConstructor(delroyScott.getTeams().get(j).getClass())
						.newInstance(delroyScott.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DelroyScott [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
