...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IThomasDanks;
...
public class ThomasDanks implements IThomasDanks {
	...
	public ThomasDanks(ThomasDanks thomasDanks) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = thomasDanks.getAnnotationID();
		for (int j = 0; j < thomasDanks.getBirthPlaces().size(); j++) {
			if (thomasDanks.getBirthPlaces().get(j) != null) {
				birthPlaces.add(thomasDanks.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(thomasDanks.getBirthPlaces().get(j).getClass())
						.newInstance(thomasDanks.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (thomasDanks.getBirthYear() != null)
			this.birthYear = thomasDanks.getBirthYear().getClass()
					.getDeclaredConstructor(thomasDanks.getBirthYear().getClass())
					.newInstance(thomasDanks.getBirthYear());
		this.characterOffset = thomasDanks.getCharacterOffset();
		this.characterOnset = thomasDanks.getCharacterOnset();
		if (thomasDanks.getPosition() != null)
			this.position = thomasDanks.getPosition().getClass()
					.getDeclaredConstructor(thomasDanks.getPosition().getClass())
					.newInstance(thomasDanks.getPosition());
		for (int j = 0; j < thomasDanks.getTeams().size(); j++) {
			if (thomasDanks.getTeams().get(j) != null) {
				teams.add(thomasDanks.getTeams().get(j).getClass()
						.getDeclaredConstructor(thomasDanks.getTeams().get(j).getClass())
						.newInstance(thomasDanks.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ThomasDanks [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
