...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IHarryHaddon;
...
public class HarryHaddon implements IHarryHaddon {
	...
	public HarryHaddon(HarryHaddon harryHaddon) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = harryHaddon.getAnnotationID();
		for (int j = 0; j < harryHaddon.getBirthPlaces().size(); j++) {
			if (harryHaddon.getBirthPlaces().get(j) != null) {
				birthPlaces.add(harryHaddon.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(harryHaddon.getBirthPlaces().get(j).getClass())
						.newInstance(harryHaddon.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (harryHaddon.getBirthYear() != null)
			this.birthYear = harryHaddon.getBirthYear().getClass()
					.getDeclaredConstructor(harryHaddon.getBirthYear().getClass())
					.newInstance(harryHaddon.getBirthYear());
		this.characterOffset = harryHaddon.getCharacterOffset();
		this.characterOnset = harryHaddon.getCharacterOnset();
		if (harryHaddon.getPosition() != null)
			this.position = harryHaddon.getPosition().getClass()
					.getDeclaredConstructor(harryHaddon.getPosition().getClass())
					.newInstance(harryHaddon.getPosition());
		for (int j = 0; j < harryHaddon.getTeams().size(); j++) {
			if (harryHaddon.getTeams().get(j) != null) {
				teams.add(harryHaddon.getTeams().get(j).getClass()
						.getDeclaredConstructor(harryHaddon.getTeams().get(j).getClass())
						.newInstance(harryHaddon.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "HarryHaddon [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
