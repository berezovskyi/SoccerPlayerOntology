...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAcurcioCarrelo;
...
public class AcurcioCarrelo implements IAcurcioCarrelo {
	...
	public AcurcioCarrelo(AcurcioCarrelo acurcioCarrelo) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = acurcioCarrelo.getAnnotationID();
		for (int j = 0; j < acurcioCarrelo.getBirthPlaces().size(); j++) {
			if (acurcioCarrelo.getBirthPlaces().get(j) != null) {
				birthPlaces.add(acurcioCarrelo.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(acurcioCarrelo.getBirthPlaces().get(j).getClass())
						.newInstance(acurcioCarrelo.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (acurcioCarrelo.getBirthYear() != null)
			this.birthYear = acurcioCarrelo.getBirthYear().getClass()
					.getDeclaredConstructor(acurcioCarrelo.getBirthYear().getClass())
					.newInstance(acurcioCarrelo.getBirthYear());
		this.characterOffset = acurcioCarrelo.getCharacterOffset();
		this.characterOnset = acurcioCarrelo.getCharacterOnset();
		if (acurcioCarrelo.getPosition() != null)
			this.position = acurcioCarrelo.getPosition().getClass()
					.getDeclaredConstructor(acurcioCarrelo.getPosition().getClass())
					.newInstance(acurcioCarrelo.getPosition());
		for (int j = 0; j < acurcioCarrelo.getTeams().size(); j++) {
			if (acurcioCarrelo.getTeams().get(j) != null) {
				teams.add(acurcioCarrelo.getTeams().get(j).getClass()
						.getDeclaredConstructor(acurcioCarrelo.getTeams().get(j).getClass())
						.newInstance(acurcioCarrelo.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AcurcioCarrelo [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
