...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMikaelMandron;
...
public class MikaelMandron implements IMikaelMandron {
	...
	public MikaelMandron(MikaelMandron mikaelMandron) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = mikaelMandron.getAnnotationID();
		for (int j = 0; j < mikaelMandron.getBirthPlaces().size(); j++) {
			if (mikaelMandron.getBirthPlaces().get(j) != null) {
				birthPlaces.add(mikaelMandron.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(mikaelMandron.getBirthPlaces().get(j).getClass())
						.newInstance(mikaelMandron.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (mikaelMandron.getBirthYear() != null)
			this.birthYear = mikaelMandron.getBirthYear().getClass()
					.getDeclaredConstructor(mikaelMandron.getBirthYear().getClass())
					.newInstance(mikaelMandron.getBirthYear());
		this.characterOffset = mikaelMandron.getCharacterOffset();
		this.characterOnset = mikaelMandron.getCharacterOnset();
		if (mikaelMandron.getPosition() != null)
			this.position = mikaelMandron.getPosition().getClass()
					.getDeclaredConstructor(mikaelMandron.getPosition().getClass())
					.newInstance(mikaelMandron.getPosition());
		for (int j = 0; j < mikaelMandron.getTeams().size(); j++) {
			if (mikaelMandron.getTeams().get(j) != null) {
				teams.add(mikaelMandron.getTeams().get(j).getClass()
						.getDeclaredConstructor(mikaelMandron.getTeams().get(j).getClass())
						.newInstance(mikaelMandron.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MikaelMandron [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
