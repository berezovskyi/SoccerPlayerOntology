...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IIvanToney;
...
public class IvanToney implements IIvanToney {
	...
	public IvanToney(IvanToney ivanToney) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = ivanToney.getAnnotationID();
		for (int j = 0; j < ivanToney.getBirthPlaces().size(); j++) {
			if (ivanToney.getBirthPlaces().get(j) != null) {
				birthPlaces.add(ivanToney.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(ivanToney.getBirthPlaces().get(j).getClass())
						.newInstance(ivanToney.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (ivanToney.getBirthYear() != null)
			this.birthYear = ivanToney.getBirthYear().getClass()
					.getDeclaredConstructor(ivanToney.getBirthYear().getClass()).newInstance(ivanToney.getBirthYear());
		this.characterOffset = ivanToney.getCharacterOffset();
		this.characterOnset = ivanToney.getCharacterOnset();
		if (ivanToney.getPosition() != null)
			this.position = ivanToney.getPosition().getClass()
					.getDeclaredConstructor(ivanToney.getPosition().getClass()).newInstance(ivanToney.getPosition());
		for (int j = 0; j < ivanToney.getTeams().size(); j++) {
			if (ivanToney.getTeams().get(j) != null) {
				teams.add(ivanToney.getTeams().get(j).getClass()
						.getDeclaredConstructor(ivanToney.getTeams().get(j).getClass())
						.newInstance(ivanToney.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "IvanToney [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
