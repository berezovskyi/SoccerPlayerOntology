...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IPeterGideon;
...
public class PeterGideon implements IPeterGideon {
	...
	public PeterGideon(PeterGideon peterGideon) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = peterGideon.getAnnotationID();
		for (int j = 0; j < peterGideon.getBirthPlaces().size(); j++) {
			if (peterGideon.getBirthPlaces().get(j) != null) {
				birthPlaces.add(peterGideon.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(peterGideon.getBirthPlaces().get(j).getClass())
						.newInstance(peterGideon.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (peterGideon.getBirthYear() != null)
			this.birthYear = peterGideon.getBirthYear().getClass()
					.getDeclaredConstructor(peterGideon.getBirthYear().getClass())
					.newInstance(peterGideon.getBirthYear());
		this.characterOffset = peterGideon.getCharacterOffset();
		this.characterOnset = peterGideon.getCharacterOnset();
		if (peterGideon.getPosition() != null)
			this.position = peterGideon.getPosition().getClass()
					.getDeclaredConstructor(peterGideon.getPosition().getClass())
					.newInstance(peterGideon.getPosition());
		for (int j = 0; j < peterGideon.getTeams().size(); j++) {
			if (peterGideon.getTeams().get(j) != null) {
				teams.add(peterGideon.getTeams().get(j).getClass()
						.getDeclaredConstructor(peterGideon.getTeams().get(j).getClass())
						.newInstance(peterGideon.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "PeterGideon [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
