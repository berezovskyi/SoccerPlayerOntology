...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IHenryHealless;
...
public class HenryHealless implements IHenryHealless {
	...
	public HenryHealless(HenryHealless henryHealless) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = henryHealless.getAnnotationID();
		for (int j = 0; j < henryHealless.getBirthPlaces().size(); j++) {
			if (henryHealless.getBirthPlaces().get(j) != null) {
				birthPlaces.add(henryHealless.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(henryHealless.getBirthPlaces().get(j).getClass())
						.newInstance(henryHealless.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (henryHealless.getBirthYear() != null)
			this.birthYear = henryHealless.getBirthYear().getClass()
					.getDeclaredConstructor(henryHealless.getBirthYear().getClass())
					.newInstance(henryHealless.getBirthYear());
		this.characterOffset = henryHealless.getCharacterOffset();
		this.characterOnset = henryHealless.getCharacterOnset();
		if (henryHealless.getPosition() != null)
			this.position = henryHealless.getPosition().getClass()
					.getDeclaredConstructor(henryHealless.getPosition().getClass())
					.newInstance(henryHealless.getPosition());
		for (int j = 0; j < henryHealless.getTeams().size(); j++) {
			if (henryHealless.getTeams().get(j) != null) {
				teams.add(henryHealless.getTeams().get(j).getClass()
						.getDeclaredConstructor(henryHealless.getTeams().get(j).getClass())
						.newInstance(henryHealless.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "HenryHealless [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
