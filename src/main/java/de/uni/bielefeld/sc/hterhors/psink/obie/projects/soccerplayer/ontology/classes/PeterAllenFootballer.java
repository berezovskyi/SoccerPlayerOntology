...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IPeterAllenFootballer;
...
public class PeterAllenFootballer implements IPeterAllenFootballer {
	...
	public PeterAllenFootballer(PeterAllenFootballer peterAllenFootballer)
			throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
			NoSuchMethodException, SecurityException {
		this.annotationID = peterAllenFootballer.getAnnotationID();
		for (int j = 0; j < peterAllenFootballer.getBirthPlaces().size(); j++) {
			if (peterAllenFootballer.getBirthPlaces().get(j) != null) {
				birthPlaces.add(peterAllenFootballer.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(peterAllenFootballer.getBirthPlaces().get(j).getClass())
						.newInstance(peterAllenFootballer.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (peterAllenFootballer.getBirthYear() != null)
			this.birthYear = peterAllenFootballer.getBirthYear().getClass()
					.getDeclaredConstructor(peterAllenFootballer.getBirthYear().getClass())
					.newInstance(peterAllenFootballer.getBirthYear());
		this.characterOffset = peterAllenFootballer.getCharacterOffset();
		this.characterOnset = peterAllenFootballer.getCharacterOnset();
		if (peterAllenFootballer.getPosition() != null)
			this.position = peterAllenFootballer.getPosition().getClass()
					.getDeclaredConstructor(peterAllenFootballer.getPosition().getClass())
					.newInstance(peterAllenFootballer.getPosition());
		for (int j = 0; j < peterAllenFootballer.getTeams().size(); j++) {
			if (peterAllenFootballer.getTeams().get(j) != null) {
				teams.add(peterAllenFootballer.getTeams().get(j).getClass()
						.getDeclaredConstructor(peterAllenFootballer.getTeams().get(j).getClass())
						.newInstance(peterAllenFootballer.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "PeterAllenFootballer [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
