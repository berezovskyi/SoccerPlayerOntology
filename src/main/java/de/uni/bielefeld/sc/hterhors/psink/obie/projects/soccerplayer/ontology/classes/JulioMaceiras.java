...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJulioMaceiras;
...
public class JulioMaceiras implements IJulioMaceiras {
	...
	public JulioMaceiras(JulioMaceiras julioMaceiras) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = julioMaceiras.getAnnotationID();
		for (int j = 0; j < julioMaceiras.getBirthPlaces().size(); j++) {
			if (julioMaceiras.getBirthPlaces().get(j) != null) {
				birthPlaces.add(julioMaceiras.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(julioMaceiras.getBirthPlaces().get(j).getClass())
						.newInstance(julioMaceiras.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (julioMaceiras.getBirthYear() != null)
			this.birthYear = julioMaceiras.getBirthYear().getClass()
					.getDeclaredConstructor(julioMaceiras.getBirthYear().getClass())
					.newInstance(julioMaceiras.getBirthYear());
		this.characterOffset = julioMaceiras.getCharacterOffset();
		this.characterOnset = julioMaceiras.getCharacterOnset();
		if (julioMaceiras.getPosition() != null)
			this.position = julioMaceiras.getPosition().getClass()
					.getDeclaredConstructor(julioMaceiras.getPosition().getClass())
					.newInstance(julioMaceiras.getPosition());
		for (int j = 0; j < julioMaceiras.getTeams().size(); j++) {
			if (julioMaceiras.getTeams().get(j) != null) {
				teams.add(julioMaceiras.getTeams().get(j).getClass()
						.getDeclaredConstructor(julioMaceiras.getTeams().get(j).getClass())
						.newInstance(julioMaceiras.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JulioMaceiras [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
