...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMarcelTisserand;
...
public class MarcelTisserand implements IMarcelTisserand {
	...
	public MarcelTisserand(MarcelTisserand marcelTisserand) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = marcelTisserand.getAnnotationID();
		for (int j = 0; j < marcelTisserand.getBirthPlaces().size(); j++) {
			if (marcelTisserand.getBirthPlaces().get(j) != null) {
				birthPlaces.add(marcelTisserand.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(marcelTisserand.getBirthPlaces().get(j).getClass())
						.newInstance(marcelTisserand.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (marcelTisserand.getBirthYear() != null)
			this.birthYear = marcelTisserand.getBirthYear().getClass()
					.getDeclaredConstructor(marcelTisserand.getBirthYear().getClass())
					.newInstance(marcelTisserand.getBirthYear());
		this.characterOffset = marcelTisserand.getCharacterOffset();
		this.characterOnset = marcelTisserand.getCharacterOnset();
		if (marcelTisserand.getPosition() != null)
			this.position = marcelTisserand.getPosition().getClass()
					.getDeclaredConstructor(marcelTisserand.getPosition().getClass())
					.newInstance(marcelTisserand.getPosition());
		for (int j = 0; j < marcelTisserand.getTeams().size(); j++) {
			if (marcelTisserand.getTeams().get(j) != null) {
				teams.add(marcelTisserand.getTeams().get(j).getClass()
						.getDeclaredConstructor(marcelTisserand.getTeams().get(j).getClass())
						.newInstance(marcelTisserand.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MarcelTisserand [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
