...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJaredSims;
...
public class JaredSims implements IJaredSims {
	...
	public JaredSims(JaredSims jaredSims) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = jaredSims.getAnnotationID();
		for (int j = 0; j < jaredSims.getBirthPlaces().size(); j++) {
			if (jaredSims.getBirthPlaces().get(j) != null) {
				birthPlaces.add(jaredSims.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(jaredSims.getBirthPlaces().get(j).getClass())
						.newInstance(jaredSims.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (jaredSims.getBirthYear() != null)
			this.birthYear = jaredSims.getBirthYear().getClass()
					.getDeclaredConstructor(jaredSims.getBirthYear().getClass()).newInstance(jaredSims.getBirthYear());
		this.characterOffset = jaredSims.getCharacterOffset();
		this.characterOnset = jaredSims.getCharacterOnset();
		if (jaredSims.getPosition() != null)
			this.position = jaredSims.getPosition().getClass()
					.getDeclaredConstructor(jaredSims.getPosition().getClass()).newInstance(jaredSims.getPosition());
		for (int j = 0; j < jaredSims.getTeams().size(); j++) {
			if (jaredSims.getTeams().get(j) != null) {
				teams.add(jaredSims.getTeams().get(j).getClass()
						.getDeclaredConstructor(jaredSims.getTeams().get(j).getClass())
						.newInstance(jaredSims.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JaredSims [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
