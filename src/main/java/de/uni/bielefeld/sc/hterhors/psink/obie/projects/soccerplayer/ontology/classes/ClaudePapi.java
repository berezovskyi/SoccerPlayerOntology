...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IClaudePapi;
...
public class ClaudePapi implements IClaudePapi {
	...
	public ClaudePapi(ClaudePapi claudePapi) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = claudePapi.getAnnotationID();
		for (int j = 0; j < claudePapi.getBirthPlaces().size(); j++) {
			if (claudePapi.getBirthPlaces().get(j) != null) {
				birthPlaces.add(claudePapi.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(claudePapi.getBirthPlaces().get(j).getClass())
						.newInstance(claudePapi.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (claudePapi.getBirthYear() != null)
			this.birthYear = claudePapi.getBirthYear().getClass()
					.getDeclaredConstructor(claudePapi.getBirthYear().getClass())
					.newInstance(claudePapi.getBirthYear());
		this.characterOffset = claudePapi.getCharacterOffset();
		this.characterOnset = claudePapi.getCharacterOnset();
		if (claudePapi.getPosition() != null)
			this.position = claudePapi.getPosition().getClass()
					.getDeclaredConstructor(claudePapi.getPosition().getClass()).newInstance(claudePapi.getPosition());
		for (int j = 0; j < claudePapi.getTeams().size(); j++) {
			if (claudePapi.getTeams().get(j) != null) {
				teams.add(claudePapi.getTeams().get(j).getClass()
						.getDeclaredConstructor(claudePapi.getTeams().get(j).getClass())
						.newInstance(claudePapi.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ClaudePapi [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
