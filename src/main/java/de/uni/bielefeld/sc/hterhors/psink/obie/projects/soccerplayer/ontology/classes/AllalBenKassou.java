...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAllalBenKassou;
...
public class AllalBenKassou implements IAllalBenKassou {
	...
	public AllalBenKassou(AllalBenKassou allalBenKassou) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = allalBenKassou.getAnnotationID();
		for (int j = 0; j < allalBenKassou.getBirthPlaces().size(); j++) {
			if (allalBenKassou.getBirthPlaces().get(j) != null) {
				birthPlaces.add(allalBenKassou.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(allalBenKassou.getBirthPlaces().get(j).getClass())
						.newInstance(allalBenKassou.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (allalBenKassou.getBirthYear() != null)
			this.birthYear = allalBenKassou.getBirthYear().getClass()
					.getDeclaredConstructor(allalBenKassou.getBirthYear().getClass())
					.newInstance(allalBenKassou.getBirthYear());
		this.characterOffset = allalBenKassou.getCharacterOffset();
		this.characterOnset = allalBenKassou.getCharacterOnset();
		if (allalBenKassou.getPosition() != null)
			this.position = allalBenKassou.getPosition().getClass()
					.getDeclaredConstructor(allalBenKassou.getPosition().getClass())
					.newInstance(allalBenKassou.getPosition());
		for (int j = 0; j < allalBenKassou.getTeams().size(); j++) {
			if (allalBenKassou.getTeams().get(j) != null) {
				teams.add(allalBenKassou.getTeams().get(j).getClass()
						.getDeclaredConstructor(allalBenKassou.getTeams().get(j).getClass())
						.newInstance(allalBenKassou.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AllalBenKassou [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
