...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAdamMarjan;
...
public class AdamMarjan implements IAdamMarjan {
	...
	public AdamMarjan(AdamMarjan adamMarjan) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = adamMarjan.getAnnotationID();
		for (int j = 0; j < adamMarjan.getBirthPlaces().size(); j++) {
			if (adamMarjan.getBirthPlaces().get(j) != null) {
				birthPlaces.add(adamMarjan.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(adamMarjan.getBirthPlaces().get(j).getClass())
						.newInstance(adamMarjan.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (adamMarjan.getBirthYear() != null)
			this.birthYear = adamMarjan.getBirthYear().getClass()
					.getDeclaredConstructor(adamMarjan.getBirthYear().getClass())
					.newInstance(adamMarjan.getBirthYear());
		this.characterOffset = adamMarjan.getCharacterOffset();
		this.characterOnset = adamMarjan.getCharacterOnset();
		if (adamMarjan.getPosition() != null)
			this.position = adamMarjan.getPosition().getClass()
					.getDeclaredConstructor(adamMarjan.getPosition().getClass()).newInstance(adamMarjan.getPosition());
		for (int j = 0; j < adamMarjan.getTeams().size(); j++) {
			if (adamMarjan.getTeams().get(j) != null) {
				teams.add(adamMarjan.getTeams().get(j).getClass()
						.getDeclaredConstructor(adamMarjan.getTeams().get(j).getClass())
						.newInstance(adamMarjan.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AdamMarjan [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
