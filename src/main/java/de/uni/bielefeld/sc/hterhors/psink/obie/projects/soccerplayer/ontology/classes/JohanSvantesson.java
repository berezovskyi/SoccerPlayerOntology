...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJohanSvantesson;
...
public class JohanSvantesson implements IJohanSvantesson {
	...
	public JohanSvantesson(JohanSvantesson johanSvantesson) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = johanSvantesson.getAnnotationID();
		for (int j = 0; j < johanSvantesson.getBirthPlaces().size(); j++) {
			if (johanSvantesson.getBirthPlaces().get(j) != null) {
				birthPlaces.add(johanSvantesson.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(johanSvantesson.getBirthPlaces().get(j).getClass())
						.newInstance(johanSvantesson.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (johanSvantesson.getBirthYear() != null)
			this.birthYear = johanSvantesson.getBirthYear().getClass()
					.getDeclaredConstructor(johanSvantesson.getBirthYear().getClass())
					.newInstance(johanSvantesson.getBirthYear());
		this.characterOffset = johanSvantesson.getCharacterOffset();
		this.characterOnset = johanSvantesson.getCharacterOnset();
		if (johanSvantesson.getPosition() != null)
			this.position = johanSvantesson.getPosition().getClass()
					.getDeclaredConstructor(johanSvantesson.getPosition().getClass())
					.newInstance(johanSvantesson.getPosition());
		for (int j = 0; j < johanSvantesson.getTeams().size(); j++) {
			if (johanSvantesson.getTeams().get(j) != null) {
				teams.add(johanSvantesson.getTeams().get(j).getClass()
						.getDeclaredConstructor(johanSvantesson.getTeams().get(j).getClass())
						.newInstance(johanSvantesson.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JohanSvantesson [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
