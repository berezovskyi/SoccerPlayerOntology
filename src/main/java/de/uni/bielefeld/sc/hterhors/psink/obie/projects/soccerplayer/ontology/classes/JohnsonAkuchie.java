...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJohnsonAkuchie;
...
public class JohnsonAkuchie implements IJohnsonAkuchie {
	...
	public JohnsonAkuchie(JohnsonAkuchie johnsonAkuchie) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = johnsonAkuchie.getAnnotationID();
		for (int j = 0; j < johnsonAkuchie.getBirthPlaces().size(); j++) {
			if (johnsonAkuchie.getBirthPlaces().get(j) != null) {
				birthPlaces.add(johnsonAkuchie.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(johnsonAkuchie.getBirthPlaces().get(j).getClass())
						.newInstance(johnsonAkuchie.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (johnsonAkuchie.getBirthYear() != null)
			this.birthYear = johnsonAkuchie.getBirthYear().getClass()
					.getDeclaredConstructor(johnsonAkuchie.getBirthYear().getClass())
					.newInstance(johnsonAkuchie.getBirthYear());
		this.characterOffset = johnsonAkuchie.getCharacterOffset();
		this.characterOnset = johnsonAkuchie.getCharacterOnset();
		if (johnsonAkuchie.getPosition() != null)
			this.position = johnsonAkuchie.getPosition().getClass()
					.getDeclaredConstructor(johnsonAkuchie.getPosition().getClass())
					.newInstance(johnsonAkuchie.getPosition());
		for (int j = 0; j < johnsonAkuchie.getTeams().size(); j++) {
			if (johnsonAkuchie.getTeams().get(j) != null) {
				teams.add(johnsonAkuchie.getTeams().get(j).getClass()
						.getDeclaredConstructor(johnsonAkuchie.getTeams().get(j).getClass())
						.newInstance(johnsonAkuchie.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JohnsonAkuchie [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
