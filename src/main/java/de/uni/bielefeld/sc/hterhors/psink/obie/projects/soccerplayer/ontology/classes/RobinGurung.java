...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRobinGurung;
...
public class RobinGurung implements IRobinGurung {
	...
	public RobinGurung(RobinGurung robinGurung) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = robinGurung.getAnnotationID();
		for (int j = 0; j < robinGurung.getBirthPlaces().size(); j++) {
			if (robinGurung.getBirthPlaces().get(j) != null) {
				birthPlaces.add(robinGurung.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(robinGurung.getBirthPlaces().get(j).getClass())
						.newInstance(robinGurung.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (robinGurung.getBirthYear() != null)
			this.birthYear = robinGurung.getBirthYear().getClass()
					.getDeclaredConstructor(robinGurung.getBirthYear().getClass())
					.newInstance(robinGurung.getBirthYear());
		this.characterOffset = robinGurung.getCharacterOffset();
		this.characterOnset = robinGurung.getCharacterOnset();
		if (robinGurung.getPosition() != null)
			this.position = robinGurung.getPosition().getClass()
					.getDeclaredConstructor(robinGurung.getPosition().getClass())
					.newInstance(robinGurung.getPosition());
		for (int j = 0; j < robinGurung.getTeams().size(); j++) {
			if (robinGurung.getTeams().get(j) != null) {
				teams.add(robinGurung.getTeams().get(j).getClass()
						.getDeclaredConstructor(robinGurung.getTeams().get(j).getClass())
						.newInstance(robinGurung.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RobinGurung [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
