...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.INassirAlGhanim;
...
public class NassirAlGhanim implements INassirAlGhanim {
	...
	public NassirAlGhanim(NassirAlGhanim nassirAlGhanim) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = nassirAlGhanim.getAnnotationID();
		for (int j = 0; j < nassirAlGhanim.getBirthPlaces().size(); j++) {
			if (nassirAlGhanim.getBirthPlaces().get(j) != null) {
				birthPlaces.add(nassirAlGhanim.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(nassirAlGhanim.getBirthPlaces().get(j).getClass())
						.newInstance(nassirAlGhanim.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (nassirAlGhanim.getBirthYear() != null)
			this.birthYear = nassirAlGhanim.getBirthYear().getClass()
					.getDeclaredConstructor(nassirAlGhanim.getBirthYear().getClass())
					.newInstance(nassirAlGhanim.getBirthYear());
		this.characterOffset = nassirAlGhanim.getCharacterOffset();
		this.characterOnset = nassirAlGhanim.getCharacterOnset();
		if (nassirAlGhanim.getPosition() != null)
			this.position = nassirAlGhanim.getPosition().getClass()
					.getDeclaredConstructor(nassirAlGhanim.getPosition().getClass())
					.newInstance(nassirAlGhanim.getPosition());
		for (int j = 0; j < nassirAlGhanim.getTeams().size(); j++) {
			if (nassirAlGhanim.getTeams().get(j) != null) {
				teams.add(nassirAlGhanim.getTeams().get(j).getClass()
						.getDeclaredConstructor(nassirAlGhanim.getTeams().get(j).getClass())
						.newInstance(nassirAlGhanim.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "NassirAlGhanim [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
