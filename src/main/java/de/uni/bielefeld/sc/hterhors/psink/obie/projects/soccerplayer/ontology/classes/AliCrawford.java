...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAliCrawford;
...
public class AliCrawford implements IAliCrawford {
	...
	public AliCrawford(AliCrawford aliCrawford) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = aliCrawford.getAnnotationID();
		for (int j = 0; j < aliCrawford.getBirthPlaces().size(); j++) {
			if (aliCrawford.getBirthPlaces().get(j) != null) {
				birthPlaces.add(aliCrawford.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(aliCrawford.getBirthPlaces().get(j).getClass())
						.newInstance(aliCrawford.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (aliCrawford.getBirthYear() != null)
			this.birthYear = aliCrawford.getBirthYear().getClass()
					.getDeclaredConstructor(aliCrawford.getBirthYear().getClass())
					.newInstance(aliCrawford.getBirthYear());
		this.characterOffset = aliCrawford.getCharacterOffset();
		this.characterOnset = aliCrawford.getCharacterOnset();
		if (aliCrawford.getPosition() != null)
			this.position = aliCrawford.getPosition().getClass()
					.getDeclaredConstructor(aliCrawford.getPosition().getClass())
					.newInstance(aliCrawford.getPosition());
		for (int j = 0; j < aliCrawford.getTeams().size(); j++) {
			if (aliCrawford.getTeams().get(j) != null) {
				teams.add(aliCrawford.getTeams().get(j).getClass()
						.getDeclaredConstructor(aliCrawford.getTeams().get(j).getClass())
						.newInstance(aliCrawford.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AliCrawford [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
