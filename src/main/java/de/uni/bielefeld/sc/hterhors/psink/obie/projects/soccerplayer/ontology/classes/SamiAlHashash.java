...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISamiAlHashash;
...
public class SamiAlHashash implements ISamiAlHashash {
	...
	public SamiAlHashash(SamiAlHashash samiAlHash) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = samiAlHash.getAnnotationID();
		for (int j = 0; j < samiAlHash.getBirthPlaces().size(); j++) {
			if (samiAlHash.getBirthPlaces().get(j) != null) {
				birthPlaces.add(samiAlHash.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(samiAlHash.getBirthPlaces().get(j).getClass())
						.newInstance(samiAlHash.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (samiAlHash.getBirthYear() != null)
			this.birthYear = samiAlHash.getBirthYear().getClass()
					.getDeclaredConstructor(samiAlHash.getBirthYear().getClass())
					.newInstance(samiAlHash.getBirthYear());
		this.characterOffset = samiAlHash.getCharacterOffset();
		this.characterOnset = samiAlHash.getCharacterOnset();
		if (samiAlHash.getPosition() != null)
			this.position = samiAlHash.getPosition().getClass()
					.getDeclaredConstructor(samiAlHash.getPosition().getClass()).newInstance(samiAlHash.getPosition());
		for (int j = 0; j < samiAlHash.getTeams().size(); j++) {
			if (samiAlHash.getTeams().get(j) != null) {
				teams.add(samiAlHash.getTeams().get(j).getClass()
						.getDeclaredConstructor(samiAlHash.getTeams().get(j).getClass())
						.newInstance(samiAlHash.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SamiAlHashash [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
