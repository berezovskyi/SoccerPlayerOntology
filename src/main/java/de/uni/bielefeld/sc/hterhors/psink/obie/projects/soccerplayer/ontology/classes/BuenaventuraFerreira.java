...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBuenaventuraFerreira;
...
public class BuenaventuraFerreira implements IBuenaventuraFerreira {
	...
	public BuenaventuraFerreira(BuenaventuraFerreira buenaventuraFerreira)
			throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
			NoSuchMethodException, SecurityException {
		this.annotationID = buenaventuraFerreira.getAnnotationID();
		for (int j = 0; j < buenaventuraFerreira.getBirthPlaces().size(); j++) {
			if (buenaventuraFerreira.getBirthPlaces().get(j) != null) {
				birthPlaces.add(buenaventuraFerreira.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(buenaventuraFerreira.getBirthPlaces().get(j).getClass())
						.newInstance(buenaventuraFerreira.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (buenaventuraFerreira.getBirthYear() != null)
			this.birthYear = buenaventuraFerreira.getBirthYear().getClass()
					.getDeclaredConstructor(buenaventuraFerreira.getBirthYear().getClass())
					.newInstance(buenaventuraFerreira.getBirthYear());
		this.characterOffset = buenaventuraFerreira.getCharacterOffset();
		this.characterOnset = buenaventuraFerreira.getCharacterOnset();
		if (buenaventuraFerreira.getPosition() != null)
			this.position = buenaventuraFerreira.getPosition().getClass()
					.getDeclaredConstructor(buenaventuraFerreira.getPosition().getClass())
					.newInstance(buenaventuraFerreira.getPosition());
		for (int j = 0; j < buenaventuraFerreira.getTeams().size(); j++) {
			if (buenaventuraFerreira.getTeams().get(j) != null) {
				teams.add(buenaventuraFerreira.getTeams().get(j).getClass()
						.getDeclaredConstructor(buenaventuraFerreira.getTeams().get(j).getClass())
						.newInstance(buenaventuraFerreira.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BuenaventuraFerreira [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
