...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IHoracioSequeira;
...
public class HoracioSequeira implements IHoracioSequeira {
	...
	public HoracioSequeira(HoracioSequeira horacioSequeira) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = horacioSequeira.getAnnotationID();
		for (int j = 0; j < horacioSequeira.getBirthPlaces().size(); j++) {
			if (horacioSequeira.getBirthPlaces().get(j) != null) {
				birthPlaces.add(horacioSequeira.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(horacioSequeira.getBirthPlaces().get(j).getClass())
						.newInstance(horacioSequeira.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (horacioSequeira.getBirthYear() != null)
			this.birthYear = horacioSequeira.getBirthYear().getClass()
					.getDeclaredConstructor(horacioSequeira.getBirthYear().getClass())
					.newInstance(horacioSequeira.getBirthYear());
		this.characterOffset = horacioSequeira.getCharacterOffset();
		this.characterOnset = horacioSequeira.getCharacterOnset();
		if (horacioSequeira.getPosition() != null)
			this.position = horacioSequeira.getPosition().getClass()
					.getDeclaredConstructor(horacioSequeira.getPosition().getClass())
					.newInstance(horacioSequeira.getPosition());
		for (int j = 0; j < horacioSequeira.getTeams().size(); j++) {
			if (horacioSequeira.getTeams().get(j) != null) {
				teams.add(horacioSequeira.getTeams().get(j).getClass()
						.getDeclaredConstructor(horacioSequeira.getTeams().get(j).getClass())
						.newInstance(horacioSequeira.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "HoracioSequeira [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
