...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBozhidarMitrev;
...
public class BozhidarMitrev implements IBozhidarMitrev {
	...
	public BozhidarMitrev(BozhidarMitrev bozhidarMitrev) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = bozhidarMitrev.getAnnotationID();
		for (int j = 0; j < bozhidarMitrev.getBirthPlaces().size(); j++) {
			if (bozhidarMitrev.getBirthPlaces().get(j) != null) {
				birthPlaces.add(bozhidarMitrev.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(bozhidarMitrev.getBirthPlaces().get(j).getClass())
						.newInstance(bozhidarMitrev.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (bozhidarMitrev.getBirthYear() != null)
			this.birthYear = bozhidarMitrev.getBirthYear().getClass()
					.getDeclaredConstructor(bozhidarMitrev.getBirthYear().getClass())
					.newInstance(bozhidarMitrev.getBirthYear());
		this.characterOffset = bozhidarMitrev.getCharacterOffset();
		this.characterOnset = bozhidarMitrev.getCharacterOnset();
		if (bozhidarMitrev.getPosition() != null)
			this.position = bozhidarMitrev.getPosition().getClass()
					.getDeclaredConstructor(bozhidarMitrev.getPosition().getClass())
					.newInstance(bozhidarMitrev.getPosition());
		for (int j = 0; j < bozhidarMitrev.getTeams().size(); j++) {
			if (bozhidarMitrev.getTeams().get(j) != null) {
				teams.add(bozhidarMitrev.getTeams().get(j).getClass()
						.getDeclaredConstructor(bozhidarMitrev.getTeams().get(j).getClass())
						.newInstance(bozhidarMitrev.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BozhidarMitrev [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
