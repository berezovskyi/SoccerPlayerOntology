...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAlariLell;
...
public class AlariLell implements IAlariLell {
	...
	public AlariLell(AlariLell alariLell) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = alariLell.getAnnotationID();
		for (int j = 0; j < alariLell.getBirthPlaces().size(); j++) {
			if (alariLell.getBirthPlaces().get(j) != null) {
				birthPlaces.add(alariLell.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(alariLell.getBirthPlaces().get(j).getClass())
						.newInstance(alariLell.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (alariLell.getBirthYear() != null)
			this.birthYear = alariLell.getBirthYear().getClass()
					.getDeclaredConstructor(alariLell.getBirthYear().getClass()).newInstance(alariLell.getBirthYear());
		this.characterOffset = alariLell.getCharacterOffset();
		this.characterOnset = alariLell.getCharacterOnset();
		if (alariLell.getPosition() != null)
			this.position = alariLell.getPosition().getClass()
					.getDeclaredConstructor(alariLell.getPosition().getClass()).newInstance(alariLell.getPosition());
		for (int j = 0; j < alariLell.getTeams().size(); j++) {
			if (alariLell.getTeams().get(j) != null) {
				teams.add(alariLell.getTeams().get(j).getClass()
						.getDeclaredConstructor(alariLell.getTeams().get(j).getClass())
						.newInstance(alariLell.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AlariLell [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
