...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJoeLodge;
...
public class JoeLodge implements IJoeLodge {
	...
	public JoeLodge(JoeLodge joeLodge) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = joeLodge.getAnnotationID();
		for (int j = 0; j < joeLodge.getBirthPlaces().size(); j++) {
			if (joeLodge.getBirthPlaces().get(j) != null) {
				birthPlaces.add(joeLodge.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(joeLodge.getBirthPlaces().get(j).getClass())
						.newInstance(joeLodge.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (joeLodge.getBirthYear() != null)
			this.birthYear = joeLodge.getBirthYear().getClass()
					.getDeclaredConstructor(joeLodge.getBirthYear().getClass()).newInstance(joeLodge.getBirthYear());
		this.characterOffset = joeLodge.getCharacterOffset();
		this.characterOnset = joeLodge.getCharacterOnset();
		if (joeLodge.getPosition() != null)
			this.position = joeLodge.getPosition().getClass().getDeclaredConstructor(joeLodge.getPosition().getClass())
					.newInstance(joeLodge.getPosition());
		for (int j = 0; j < joeLodge.getTeams().size(); j++) {
			if (joeLodge.getTeams().get(j) != null) {
				teams.add(joeLodge.getTeams().get(j).getClass()
						.getDeclaredConstructor(joeLodge.getTeams().get(j).getClass())
						.newInstance(joeLodge.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JoeLodge [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
