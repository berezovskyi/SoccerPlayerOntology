...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IVictorKros;
...
public class VictorKros implements IVictorKros {
	...
	public VictorKros(VictorKros victorKros) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = victorKros.getAnnotationID();
		for (int j = 0; j < victorKros.getBirthPlaces().size(); j++) {
			if (victorKros.getBirthPlaces().get(j) != null) {
				birthPlaces.add(victorKros.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(victorKros.getBirthPlaces().get(j).getClass())
						.newInstance(victorKros.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (victorKros.getBirthYear() != null)
			this.birthYear = victorKros.getBirthYear().getClass()
					.getDeclaredConstructor(victorKros.getBirthYear().getClass())
					.newInstance(victorKros.getBirthYear());
		this.characterOffset = victorKros.getCharacterOffset();
		this.characterOnset = victorKros.getCharacterOnset();
		if (victorKros.getPosition() != null)
			this.position = victorKros.getPosition().getClass()
					.getDeclaredConstructor(victorKros.getPosition().getClass()).newInstance(victorKros.getPosition());
		for (int j = 0; j < victorKros.getTeams().size(); j++) {
			if (victorKros.getTeams().get(j) != null) {
				teams.add(victorKros.getTeams().get(j).getClass()
						.getDeclaredConstructor(victorKros.getTeams().get(j).getClass())
						.newInstance(victorKros.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "VictorKros [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
