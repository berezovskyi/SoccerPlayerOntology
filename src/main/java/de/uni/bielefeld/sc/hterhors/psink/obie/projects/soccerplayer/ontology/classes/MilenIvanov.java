...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMilenIvanov;
...
public class MilenIvanov implements IMilenIvanov {
	...
	public MilenIvanov(MilenIvanov milenIvanov) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = milenIvanov.getAnnotationID();
		for (int j = 0; j < milenIvanov.getBirthPlaces().size(); j++) {
			if (milenIvanov.getBirthPlaces().get(j) != null) {
				birthPlaces.add(milenIvanov.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(milenIvanov.getBirthPlaces().get(j).getClass())
						.newInstance(milenIvanov.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (milenIvanov.getBirthYear() != null)
			this.birthYear = milenIvanov.getBirthYear().getClass()
					.getDeclaredConstructor(milenIvanov.getBirthYear().getClass())
					.newInstance(milenIvanov.getBirthYear());
		this.characterOffset = milenIvanov.getCharacterOffset();
		this.characterOnset = milenIvanov.getCharacterOnset();
		if (milenIvanov.getPosition() != null)
			this.position = milenIvanov.getPosition().getClass()
					.getDeclaredConstructor(milenIvanov.getPosition().getClass())
					.newInstance(milenIvanov.getPosition());
		for (int j = 0; j < milenIvanov.getTeams().size(); j++) {
			if (milenIvanov.getTeams().get(j) != null) {
				teams.add(milenIvanov.getTeams().get(j).getClass()
						.getDeclaredConstructor(milenIvanov.getTeams().get(j).getClass())
						.newInstance(milenIvanov.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MilenIvanov [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
