...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRobbieHaw;
...
public class RobbieHaw implements IRobbieHaw {
	...
	public RobbieHaw(RobbieHaw robbieHaw) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = robbieHaw.getAnnotationID();
		for (int j = 0; j < robbieHaw.getBirthPlaces().size(); j++) {
			if (robbieHaw.getBirthPlaces().get(j) != null) {
				birthPlaces.add(robbieHaw.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(robbieHaw.getBirthPlaces().get(j).getClass())
						.newInstance(robbieHaw.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (robbieHaw.getBirthYear() != null)
			this.birthYear = robbieHaw.getBirthYear().getClass()
					.getDeclaredConstructor(robbieHaw.getBirthYear().getClass()).newInstance(robbieHaw.getBirthYear());
		this.characterOffset = robbieHaw.getCharacterOffset();
		this.characterOnset = robbieHaw.getCharacterOnset();
		if (robbieHaw.getPosition() != null)
			this.position = robbieHaw.getPosition().getClass()
					.getDeclaredConstructor(robbieHaw.getPosition().getClass()).newInstance(robbieHaw.getPosition());
		for (int j = 0; j < robbieHaw.getTeams().size(); j++) {
			if (robbieHaw.getTeams().get(j) != null) {
				teams.add(robbieHaw.getTeams().get(j).getClass()
						.getDeclaredConstructor(robbieHaw.getTeams().get(j).getClass())
						.newInstance(robbieHaw.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RobbieHaw [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
