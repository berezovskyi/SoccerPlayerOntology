...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDimitriosManos;
...
public class DimitriosManos implements IDimitriosManos {
	...
	public DimitriosManos(DimitriosManos dimitriosManos) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = dimitriosManos.getAnnotationID();
		for (int j = 0; j < dimitriosManos.getBirthPlaces().size(); j++) {
			if (dimitriosManos.getBirthPlaces().get(j) != null) {
				birthPlaces.add(dimitriosManos.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(dimitriosManos.getBirthPlaces().get(j).getClass())
						.newInstance(dimitriosManos.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (dimitriosManos.getBirthYear() != null)
			this.birthYear = dimitriosManos.getBirthYear().getClass()
					.getDeclaredConstructor(dimitriosManos.getBirthYear().getClass())
					.newInstance(dimitriosManos.getBirthYear());
		this.characterOffset = dimitriosManos.getCharacterOffset();
		this.characterOnset = dimitriosManos.getCharacterOnset();
		if (dimitriosManos.getPosition() != null)
			this.position = dimitriosManos.getPosition().getClass()
					.getDeclaredConstructor(dimitriosManos.getPosition().getClass())
					.newInstance(dimitriosManos.getPosition());
		for (int j = 0; j < dimitriosManos.getTeams().size(); j++) {
			if (dimitriosManos.getTeams().get(j) != null) {
				teams.add(dimitriosManos.getTeams().get(j).getClass()
						.getDeclaredConstructor(dimitriosManos.getTeams().get(j).getClass())
						.newInstance(dimitriosManos.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DimitriosManos [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
