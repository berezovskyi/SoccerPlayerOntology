...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IArturDyson;
...
public class ArturDyson implements IArturDyson {
	...
	public ArturDyson(ArturDyson arturDyson) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = arturDyson.getAnnotationID();
		for (int j = 0; j < arturDyson.getBirthPlaces().size(); j++) {
			if (arturDyson.getBirthPlaces().get(j) != null) {
				birthPlaces.add(arturDyson.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(arturDyson.getBirthPlaces().get(j).getClass())
						.newInstance(arturDyson.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (arturDyson.getBirthYear() != null)
			this.birthYear = arturDyson.getBirthYear().getClass()
					.getDeclaredConstructor(arturDyson.getBirthYear().getClass())
					.newInstance(arturDyson.getBirthYear());
		this.characterOffset = arturDyson.getCharacterOffset();
		this.characterOnset = arturDyson.getCharacterOnset();
		if (arturDyson.getPosition() != null)
			this.position = arturDyson.getPosition().getClass()
					.getDeclaredConstructor(arturDyson.getPosition().getClass()).newInstance(arturDyson.getPosition());
		for (int j = 0; j < arturDyson.getTeams().size(); j++) {
			if (arturDyson.getTeams().get(j) != null) {
				teams.add(arturDyson.getTeams().get(j).getClass()
						.getDeclaredConstructor(arturDyson.getTeams().get(j).getClass())
						.newInstance(arturDyson.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ArturDyson [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
