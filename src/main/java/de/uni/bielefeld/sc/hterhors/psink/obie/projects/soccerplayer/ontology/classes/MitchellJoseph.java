...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMitchellJoseph;
...
public class MitchellJoseph implements IMitchellJoseph {
	...
	public MitchellJoseph(MitchellJoseph mitchellJoseph) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = mitchellJoseph.getAnnotationID();
		for (int j = 0; j < mitchellJoseph.getBirthPlaces().size(); j++) {
			if (mitchellJoseph.getBirthPlaces().get(j) != null) {
				birthPlaces.add(mitchellJoseph.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(mitchellJoseph.getBirthPlaces().get(j).getClass())
						.newInstance(mitchellJoseph.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (mitchellJoseph.getBirthYear() != null)
			this.birthYear = mitchellJoseph.getBirthYear().getClass()
					.getDeclaredConstructor(mitchellJoseph.getBirthYear().getClass())
					.newInstance(mitchellJoseph.getBirthYear());
		this.characterOffset = mitchellJoseph.getCharacterOffset();
		this.characterOnset = mitchellJoseph.getCharacterOnset();
		if (mitchellJoseph.getPosition() != null)
			this.position = mitchellJoseph.getPosition().getClass()
					.getDeclaredConstructor(mitchellJoseph.getPosition().getClass())
					.newInstance(mitchellJoseph.getPosition());
		for (int j = 0; j < mitchellJoseph.getTeams().size(); j++) {
			if (mitchellJoseph.getTeams().get(j) != null) {
				teams.add(mitchellJoseph.getTeams().get(j).getClass()
						.getDeclaredConstructor(mitchellJoseph.getTeams().get(j).getClass())
						.newInstance(mitchellJoseph.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MitchellJoseph [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
