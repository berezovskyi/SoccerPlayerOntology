...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IPaulGladon;
...
public class PaulGladon implements IPaulGladon {
	...
	public PaulGladon(PaulGladon paulGladon) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = paulGladon.getAnnotationID();
		for (int j = 0; j < paulGladon.getBirthPlaces().size(); j++) {
			if (paulGladon.getBirthPlaces().get(j) != null) {
				birthPlaces.add(paulGladon.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(paulGladon.getBirthPlaces().get(j).getClass())
						.newInstance(paulGladon.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (paulGladon.getBirthYear() != null)
			this.birthYear = paulGladon.getBirthYear().getClass()
					.getDeclaredConstructor(paulGladon.getBirthYear().getClass())
					.newInstance(paulGladon.getBirthYear());
		this.characterOffset = paulGladon.getCharacterOffset();
		this.characterOnset = paulGladon.getCharacterOnset();
		if (paulGladon.getPosition() != null)
			this.position = paulGladon.getPosition().getClass()
					.getDeclaredConstructor(paulGladon.getPosition().getClass()).newInstance(paulGladon.getPosition());
		for (int j = 0; j < paulGladon.getTeams().size(); j++) {
			if (paulGladon.getTeams().get(j) != null) {
				teams.add(paulGladon.getTeams().get(j).getClass()
						.getDeclaredConstructor(paulGladon.getTeams().get(j).getClass())
						.newInstance(paulGladon.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "PaulGladon [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
