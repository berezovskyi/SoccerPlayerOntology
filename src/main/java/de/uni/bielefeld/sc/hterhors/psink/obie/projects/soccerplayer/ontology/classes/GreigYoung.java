...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IGreigYoung;
...
public class GreigYoung implements IGreigYoung {
	...
	public GreigYoung(GreigYoung greigYoung) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = greigYoung.getAnnotationID();
		for (int j = 0; j < greigYoung.getBirthPlaces().size(); j++) {
			if (greigYoung.getBirthPlaces().get(j) != null) {
				birthPlaces.add(greigYoung.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(greigYoung.getBirthPlaces().get(j).getClass())
						.newInstance(greigYoung.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (greigYoung.getBirthYear() != null)
			this.birthYear = greigYoung.getBirthYear().getClass()
					.getDeclaredConstructor(greigYoung.getBirthYear().getClass())
					.newInstance(greigYoung.getBirthYear());
		this.characterOffset = greigYoung.getCharacterOffset();
		this.characterOnset = greigYoung.getCharacterOnset();
		if (greigYoung.getPosition() != null)
			this.position = greigYoung.getPosition().getClass()
					.getDeclaredConstructor(greigYoung.getPosition().getClass()).newInstance(greigYoung.getPosition());
		for (int j = 0; j < greigYoung.getTeams().size(); j++) {
			if (greigYoung.getTeams().get(j) != null) {
				teams.add(greigYoung.getTeams().get(j).getClass()
						.getDeclaredConstructor(greigYoung.getTeams().get(j).getClass())
						.newInstance(greigYoung.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "GreigYoung [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
