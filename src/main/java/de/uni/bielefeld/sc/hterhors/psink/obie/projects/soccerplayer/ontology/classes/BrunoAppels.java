...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBrunoAppels;
...
public class BrunoAppels implements IBrunoAppels {
	...
	public BrunoAppels(BrunoAppels brunoAppels) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = brunoAppels.getAnnotationID();
		for (int j = 0; j < brunoAppels.getBirthPlaces().size(); j++) {
			if (brunoAppels.getBirthPlaces().get(j) != null) {
				birthPlaces.add(brunoAppels.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(brunoAppels.getBirthPlaces().get(j).getClass())
						.newInstance(brunoAppels.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (brunoAppels.getBirthYear() != null)
			this.birthYear = brunoAppels.getBirthYear().getClass()
					.getDeclaredConstructor(brunoAppels.getBirthYear().getClass())
					.newInstance(brunoAppels.getBirthYear());
		this.characterOffset = brunoAppels.getCharacterOffset();
		this.characterOnset = brunoAppels.getCharacterOnset();
		if (brunoAppels.getPosition() != null)
			this.position = brunoAppels.getPosition().getClass()
					.getDeclaredConstructor(brunoAppels.getPosition().getClass())
					.newInstance(brunoAppels.getPosition());
		for (int j = 0; j < brunoAppels.getTeams().size(); j++) {
			if (brunoAppels.getTeams().get(j) != null) {
				teams.add(brunoAppels.getTeams().get(j).getClass()
						.getDeclaredConstructor(brunoAppels.getTeams().get(j).getClass())
						.newInstance(brunoAppels.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BrunoAppels [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
