...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IChrisPhilipps;
...
public class ChrisPhilipps implements IChrisPhilipps {
	...
	public ChrisPhilipps(ChrisPhilipps chrisPhilipps) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = chrisPhilipps.getAnnotationID();
		for (int j = 0; j < chrisPhilipps.getBirthPlaces().size(); j++) {
			if (chrisPhilipps.getBirthPlaces().get(j) != null) {
				birthPlaces.add(chrisPhilipps.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(chrisPhilipps.getBirthPlaces().get(j).getClass())
						.newInstance(chrisPhilipps.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (chrisPhilipps.getBirthYear() != null)
			this.birthYear = chrisPhilipps.getBirthYear().getClass()
					.getDeclaredConstructor(chrisPhilipps.getBirthYear().getClass())
					.newInstance(chrisPhilipps.getBirthYear());
		this.characterOffset = chrisPhilipps.getCharacterOffset();
		this.characterOnset = chrisPhilipps.getCharacterOnset();
		if (chrisPhilipps.getPosition() != null)
			this.position = chrisPhilipps.getPosition().getClass()
					.getDeclaredConstructor(chrisPhilipps.getPosition().getClass())
					.newInstance(chrisPhilipps.getPosition());
		for (int j = 0; j < chrisPhilipps.getTeams().size(); j++) {
			if (chrisPhilipps.getTeams().get(j) != null) {
				teams.add(chrisPhilipps.getTeams().get(j).getClass()
						.getDeclaredConstructor(chrisPhilipps.getTeams().get(j).getClass())
						.newInstance(chrisPhilipps.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ChrisPhilipps [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
