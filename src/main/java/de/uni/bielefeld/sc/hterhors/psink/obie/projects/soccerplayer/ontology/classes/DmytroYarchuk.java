...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDmytroYarchuk;
...
public class DmytroYarchuk implements IDmytroYarchuk {
	...
	public DmytroYarchuk(DmytroYarchuk dmytroYarchuk) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = dmytroYarchuk.getAnnotationID();
		for (int j = 0; j < dmytroYarchuk.getBirthPlaces().size(); j++) {
			if (dmytroYarchuk.getBirthPlaces().get(j) != null) {
				birthPlaces.add(dmytroYarchuk.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(dmytroYarchuk.getBirthPlaces().get(j).getClass())
						.newInstance(dmytroYarchuk.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (dmytroYarchuk.getBirthYear() != null)
			this.birthYear = dmytroYarchuk.getBirthYear().getClass()
					.getDeclaredConstructor(dmytroYarchuk.getBirthYear().getClass())
					.newInstance(dmytroYarchuk.getBirthYear());
		this.characterOffset = dmytroYarchuk.getCharacterOffset();
		this.characterOnset = dmytroYarchuk.getCharacterOnset();
		if (dmytroYarchuk.getPosition() != null)
			this.position = dmytroYarchuk.getPosition().getClass()
					.getDeclaredConstructor(dmytroYarchuk.getPosition().getClass())
					.newInstance(dmytroYarchuk.getPosition());
		for (int j = 0; j < dmytroYarchuk.getTeams().size(); j++) {
			if (dmytroYarchuk.getTeams().get(j) != null) {
				teams.add(dmytroYarchuk.getTeams().get(j).getClass()
						.getDeclaredConstructor(dmytroYarchuk.getTeams().get(j).getClass())
						.newInstance(dmytroYarchuk.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DmytroYarchuk [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
