...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILukeHyam;
...
public class LukeHyam implements ILukeHyam {
	...
	public LukeHyam(LukeHyam lukeHyam) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = lukeHyam.getAnnotationID();
		for (int j = 0; j < lukeHyam.getBirthPlaces().size(); j++) {
			if (lukeHyam.getBirthPlaces().get(j) != null) {
				birthPlaces.add(lukeHyam.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(lukeHyam.getBirthPlaces().get(j).getClass())
						.newInstance(lukeHyam.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (lukeHyam.getBirthYear() != null)
			this.birthYear = lukeHyam.getBirthYear().getClass()
					.getDeclaredConstructor(lukeHyam.getBirthYear().getClass()).newInstance(lukeHyam.getBirthYear());
		this.characterOffset = lukeHyam.getCharacterOffset();
		this.characterOnset = lukeHyam.getCharacterOnset();
		if (lukeHyam.getPosition() != null)
			this.position = lukeHyam.getPosition().getClass().getDeclaredConstructor(lukeHyam.getPosition().getClass())
					.newInstance(lukeHyam.getPosition());
		for (int j = 0; j < lukeHyam.getTeams().size(); j++) {
			if (lukeHyam.getTeams().get(j) != null) {
				teams.add(lukeHyam.getTeams().get(j).getClass()
						.getDeclaredConstructor(lukeHyam.getTeams().get(j).getClass())
						.newInstance(lukeHyam.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LukeHyam [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
