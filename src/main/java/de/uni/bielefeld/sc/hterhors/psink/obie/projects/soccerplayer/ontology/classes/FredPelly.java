...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IFredPelly;
...
public class FredPelly implements IFredPelly {
	...
	public FredPelly(FredPelly fredPelly) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = fredPelly.getAnnotationID();
		for (int j = 0; j < fredPelly.getBirthPlaces().size(); j++) {
			if (fredPelly.getBirthPlaces().get(j) != null) {
				birthPlaces.add(fredPelly.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(fredPelly.getBirthPlaces().get(j).getClass())
						.newInstance(fredPelly.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (fredPelly.getBirthYear() != null)
			this.birthYear = fredPelly.getBirthYear().getClass()
					.getDeclaredConstructor(fredPelly.getBirthYear().getClass()).newInstance(fredPelly.getBirthYear());
		this.characterOffset = fredPelly.getCharacterOffset();
		this.characterOnset = fredPelly.getCharacterOnset();
		if (fredPelly.getPosition() != null)
			this.position = fredPelly.getPosition().getClass()
					.getDeclaredConstructor(fredPelly.getPosition().getClass()).newInstance(fredPelly.getPosition());
		for (int j = 0; j < fredPelly.getTeams().size(); j++) {
			if (fredPelly.getTeams().get(j) != null) {
				teams.add(fredPelly.getTeams().get(j).getClass()
						.getDeclaredConstructor(fredPelly.getTeams().get(j).getClass())
						.newInstance(fredPelly.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "FredPelly [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
