...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IIliyaDimitrov;
...
public class IliyaDimitrov implements IIliyaDimitrov {
	...
	public IliyaDimitrov(IliyaDimitrov iliyaDimitrov) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = iliyaDimitrov.getAnnotationID();
		for (int j = 0; j < iliyaDimitrov.getBirthPlaces().size(); j++) {
			if (iliyaDimitrov.getBirthPlaces().get(j) != null) {
				birthPlaces.add(iliyaDimitrov.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(iliyaDimitrov.getBirthPlaces().get(j).getClass())
						.newInstance(iliyaDimitrov.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (iliyaDimitrov.getBirthYear() != null)
			this.birthYear = iliyaDimitrov.getBirthYear().getClass()
					.getDeclaredConstructor(iliyaDimitrov.getBirthYear().getClass())
					.newInstance(iliyaDimitrov.getBirthYear());
		this.characterOffset = iliyaDimitrov.getCharacterOffset();
		this.characterOnset = iliyaDimitrov.getCharacterOnset();
		if (iliyaDimitrov.getPosition() != null)
			this.position = iliyaDimitrov.getPosition().getClass()
					.getDeclaredConstructor(iliyaDimitrov.getPosition().getClass())
					.newInstance(iliyaDimitrov.getPosition());
		for (int j = 0; j < iliyaDimitrov.getTeams().size(); j++) {
			if (iliyaDimitrov.getTeams().get(j) != null) {
				teams.add(iliyaDimitrov.getTeams().get(j).getClass()
						.getDeclaredConstructor(iliyaDimitrov.getTeams().get(j).getClass())
						.newInstance(iliyaDimitrov.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "IliyaDimitrov [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
