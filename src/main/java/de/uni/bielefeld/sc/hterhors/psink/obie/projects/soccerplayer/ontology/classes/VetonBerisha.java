...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IVetonBerisha;
...
public class VetonBerisha implements IVetonBerisha {
	...
	public VetonBerisha(VetonBerisha vetonBerisha) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = vetonBerisha.getAnnotationID();
		for (int j = 0; j < vetonBerisha.getBirthPlaces().size(); j++) {
			if (vetonBerisha.getBirthPlaces().get(j) != null) {
				birthPlaces.add(vetonBerisha.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(vetonBerisha.getBirthPlaces().get(j).getClass())
						.newInstance(vetonBerisha.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (vetonBerisha.getBirthYear() != null)
			this.birthYear = vetonBerisha.getBirthYear().getClass()
					.getDeclaredConstructor(vetonBerisha.getBirthYear().getClass())
					.newInstance(vetonBerisha.getBirthYear());
		this.characterOffset = vetonBerisha.getCharacterOffset();
		this.characterOnset = vetonBerisha.getCharacterOnset();
		if (vetonBerisha.getPosition() != null)
			this.position = vetonBerisha.getPosition().getClass()
					.getDeclaredConstructor(vetonBerisha.getPosition().getClass())
					.newInstance(vetonBerisha.getPosition());
		for (int j = 0; j < vetonBerisha.getTeams().size(); j++) {
			if (vetonBerisha.getTeams().get(j) != null) {
				teams.add(vetonBerisha.getTeams().get(j).getClass()
						.getDeclaredConstructor(vetonBerisha.getTeams().get(j).getClass())
						.newInstance(vetonBerisha.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "VetonBerisha [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
