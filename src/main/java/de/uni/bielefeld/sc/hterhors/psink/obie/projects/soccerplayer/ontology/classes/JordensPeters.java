...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJordensPeters;
...
public class JordensPeters implements IJordensPeters {
	...
	public JordensPeters(JordensPeters jordensPeters) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = jordensPeters.getAnnotationID();
		for (int j = 0; j < jordensPeters.getBirthPlaces().size(); j++) {
			if (jordensPeters.getBirthPlaces().get(j) != null) {
				birthPlaces.add(jordensPeters.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(jordensPeters.getBirthPlaces().get(j).getClass())
						.newInstance(jordensPeters.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (jordensPeters.getBirthYear() != null)
			this.birthYear = jordensPeters.getBirthYear().getClass()
					.getDeclaredConstructor(jordensPeters.getBirthYear().getClass())
					.newInstance(jordensPeters.getBirthYear());
		this.characterOffset = jordensPeters.getCharacterOffset();
		this.characterOnset = jordensPeters.getCharacterOnset();
		if (jordensPeters.getPosition() != null)
			this.position = jordensPeters.getPosition().getClass()
					.getDeclaredConstructor(jordensPeters.getPosition().getClass())
					.newInstance(jordensPeters.getPosition());
		for (int j = 0; j < jordensPeters.getTeams().size(); j++) {
			if (jordensPeters.getTeams().get(j) != null) {
				teams.add(jordensPeters.getTeams().get(j).getClass()
						.getDeclaredConstructor(jordensPeters.getTeams().get(j).getClass())
						.newInstance(jordensPeters.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JordensPeters [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
