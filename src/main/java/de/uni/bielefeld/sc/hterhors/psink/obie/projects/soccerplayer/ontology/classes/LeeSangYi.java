...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILeeSangYi;
...
public class LeeSangYi implements ILeeSangYi {
	...
	public LeeSangYi(LeeSangYi leeSangYi) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = leeSangYi.getAnnotationID();
		for (int j = 0; j < leeSangYi.getBirthPlaces().size(); j++) {
			if (leeSangYi.getBirthPlaces().get(j) != null) {
				birthPlaces.add(leeSangYi.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(leeSangYi.getBirthPlaces().get(j).getClass())
						.newInstance(leeSangYi.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (leeSangYi.getBirthYear() != null)
			this.birthYear = leeSangYi.getBirthYear().getClass()
					.getDeclaredConstructor(leeSangYi.getBirthYear().getClass()).newInstance(leeSangYi.getBirthYear());
		this.characterOffset = leeSangYi.getCharacterOffset();
		this.characterOnset = leeSangYi.getCharacterOnset();
		if (leeSangYi.getPosition() != null)
			this.position = leeSangYi.getPosition().getClass()
					.getDeclaredConstructor(leeSangYi.getPosition().getClass()).newInstance(leeSangYi.getPosition());
		for (int j = 0; j < leeSangYi.getTeams().size(); j++) {
			if (leeSangYi.getTeams().get(j) != null) {
				teams.add(leeSangYi.getTeams().get(j).getClass()
						.getDeclaredConstructor(leeSangYi.getTeams().get(j).getClass())
						.newInstance(leeSangYi.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LeeSangYi [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
