...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILeeMargerison;
...
public class LeeMargerison implements ILeeMargerison {
	...
	public LeeMargerison(LeeMargerison leeMargerison) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = leeMargerison.getAnnotationID();
		for (int j = 0; j < leeMargerison.getBirthPlaces().size(); j++) {
			if (leeMargerison.getBirthPlaces().get(j) != null) {
				birthPlaces.add(leeMargerison.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(leeMargerison.getBirthPlaces().get(j).getClass())
						.newInstance(leeMargerison.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (leeMargerison.getBirthYear() != null)
			this.birthYear = leeMargerison.getBirthYear().getClass()
					.getDeclaredConstructor(leeMargerison.getBirthYear().getClass())
					.newInstance(leeMargerison.getBirthYear());
		this.characterOffset = leeMargerison.getCharacterOffset();
		this.characterOnset = leeMargerison.getCharacterOnset();
		if (leeMargerison.getPosition() != null)
			this.position = leeMargerison.getPosition().getClass()
					.getDeclaredConstructor(leeMargerison.getPosition().getClass())
					.newInstance(leeMargerison.getPosition());
		for (int j = 0; j < leeMargerison.getTeams().size(); j++) {
			if (leeMargerison.getTeams().get(j) != null) {
				teams.add(leeMargerison.getTeams().get(j).getClass()
						.getDeclaredConstructor(leeMargerison.getTeams().get(j).getClass())
						.newInstance(leeMargerison.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LeeMargerison [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
