...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ITonyWingate;
...
public class TonyWingate implements ITonyWingate {
	...
	public TonyWingate(TonyWingate tonyWingate) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = tonyWingate.getAnnotationID();
		for (int j = 0; j < tonyWingate.getBirthPlaces().size(); j++) {
			if (tonyWingate.getBirthPlaces().get(j) != null) {
				birthPlaces.add(tonyWingate.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(tonyWingate.getBirthPlaces().get(j).getClass())
						.newInstance(tonyWingate.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (tonyWingate.getBirthYear() != null)
			this.birthYear = tonyWingate.getBirthYear().getClass()
					.getDeclaredConstructor(tonyWingate.getBirthYear().getClass())
					.newInstance(tonyWingate.getBirthYear());
		this.characterOffset = tonyWingate.getCharacterOffset();
		this.characterOnset = tonyWingate.getCharacterOnset();
		if (tonyWingate.getPosition() != null)
			this.position = tonyWingate.getPosition().getClass()
					.getDeclaredConstructor(tonyWingate.getPosition().getClass())
					.newInstance(tonyWingate.getPosition());
		for (int j = 0; j < tonyWingate.getTeams().size(); j++) {
			if (tonyWingate.getTeams().get(j) != null) {
				teams.add(tonyWingate.getTeams().get(j).getClass()
						.getDeclaredConstructor(tonyWingate.getTeams().get(j).getClass())
						.newInstance(tonyWingate.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "TonyWingate [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
