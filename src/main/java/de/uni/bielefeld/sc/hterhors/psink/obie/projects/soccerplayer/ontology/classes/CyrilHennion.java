...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ICyrilHennion;
...
public class CyrilHennion implements ICyrilHennion {
	...
	public CyrilHennion(CyrilHennion cyrilHennion) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = cyrilHennion.getAnnotationID();
		for (int j = 0; j < cyrilHennion.getBirthPlaces().size(); j++) {
			if (cyrilHennion.getBirthPlaces().get(j) != null) {
				birthPlaces.add(cyrilHennion.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(cyrilHennion.getBirthPlaces().get(j).getClass())
						.newInstance(cyrilHennion.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (cyrilHennion.getBirthYear() != null)
			this.birthYear = cyrilHennion.getBirthYear().getClass()
					.getDeclaredConstructor(cyrilHennion.getBirthYear().getClass())
					.newInstance(cyrilHennion.getBirthYear());
		this.characterOffset = cyrilHennion.getCharacterOffset();
		this.characterOnset = cyrilHennion.getCharacterOnset();
		if (cyrilHennion.getPosition() != null)
			this.position = cyrilHennion.getPosition().getClass()
					.getDeclaredConstructor(cyrilHennion.getPosition().getClass())
					.newInstance(cyrilHennion.getPosition());
		for (int j = 0; j < cyrilHennion.getTeams().size(); j++) {
			if (cyrilHennion.getTeams().get(j) != null) {
				teams.add(cyrilHennion.getTeams().get(j).getClass()
						.getDeclaredConstructor(cyrilHennion.getTeams().get(j).getClass())
						.newInstance(cyrilHennion.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "CyrilHennion [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
