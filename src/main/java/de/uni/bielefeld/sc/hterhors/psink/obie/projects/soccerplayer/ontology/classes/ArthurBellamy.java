...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IArthurBellamy;
...
public class ArthurBellamy implements IArthurBellamy {
	...
	public ArthurBellamy(ArthurBellamy arthurBellamy) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = arthurBellamy.getAnnotationID();
		for (int j = 0; j < arthurBellamy.getBirthPlaces().size(); j++) {
			if (arthurBellamy.getBirthPlaces().get(j) != null) {
				birthPlaces.add(arthurBellamy.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(arthurBellamy.getBirthPlaces().get(j).getClass())
						.newInstance(arthurBellamy.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (arthurBellamy.getBirthYear() != null)
			this.birthYear = arthurBellamy.getBirthYear().getClass()
					.getDeclaredConstructor(arthurBellamy.getBirthYear().getClass())
					.newInstance(arthurBellamy.getBirthYear());
		this.characterOffset = arthurBellamy.getCharacterOffset();
		this.characterOnset = arthurBellamy.getCharacterOnset();
		if (arthurBellamy.getPosition() != null)
			this.position = arthurBellamy.getPosition().getClass()
					.getDeclaredConstructor(arthurBellamy.getPosition().getClass())
					.newInstance(arthurBellamy.getPosition());
		for (int j = 0; j < arthurBellamy.getTeams().size(); j++) {
			if (arthurBellamy.getTeams().get(j) != null) {
				teams.add(arthurBellamy.getTeams().get(j).getClass()
						.getDeclaredConstructor(arthurBellamy.getTeams().get(j).getClass())
						.newInstance(arthurBellamy.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ArthurBellamy [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
