...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.INikolaParshanov;
...
public class NikolaParshanov implements INikolaParshanov {
	...
	public NikolaParshanov(NikolaParshanov nikolaParshanov) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = nikolaParshanov.getAnnotationID();
		for (int j = 0; j < nikolaParshanov.getBirthPlaces().size(); j++) {
			if (nikolaParshanov.getBirthPlaces().get(j) != null) {
				birthPlaces.add(nikolaParshanov.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(nikolaParshanov.getBirthPlaces().get(j).getClass())
						.newInstance(nikolaParshanov.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (nikolaParshanov.getBirthYear() != null)
			this.birthYear = nikolaParshanov.getBirthYear().getClass()
					.getDeclaredConstructor(nikolaParshanov.getBirthYear().getClass())
					.newInstance(nikolaParshanov.getBirthYear());
		this.characterOffset = nikolaParshanov.getCharacterOffset();
		this.characterOnset = nikolaParshanov.getCharacterOnset();
		if (nikolaParshanov.getPosition() != null)
			this.position = nikolaParshanov.getPosition().getClass()
					.getDeclaredConstructor(nikolaParshanov.getPosition().getClass())
					.newInstance(nikolaParshanov.getPosition());
		for (int j = 0; j < nikolaParshanov.getTeams().size(); j++) {
			if (nikolaParshanov.getTeams().get(j) != null) {
				teams.add(nikolaParshanov.getTeams().get(j).getClass()
						.getDeclaredConstructor(nikolaParshanov.getTeams().get(j).getClass())
						.newInstance(nikolaParshanov.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "NikolaParshanov [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
