...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ITonyMillerFootballer;
...
public class TonyMillerFootballer implements ITonyMillerFootballer {
	...
	public TonyMillerFootballer(TonyMillerFootballer tonyMillerFootballer)
			throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
			NoSuchMethodException, SecurityException {
		this.annotationID = tonyMillerFootballer.getAnnotationID();
		for (int j = 0; j < tonyMillerFootballer.getBirthPlaces().size(); j++) {
			if (tonyMillerFootballer.getBirthPlaces().get(j) != null) {
				birthPlaces.add(tonyMillerFootballer.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(tonyMillerFootballer.getBirthPlaces().get(j).getClass())
						.newInstance(tonyMillerFootballer.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (tonyMillerFootballer.getBirthYear() != null)
			this.birthYear = tonyMillerFootballer.getBirthYear().getClass()
					.getDeclaredConstructor(tonyMillerFootballer.getBirthYear().getClass())
					.newInstance(tonyMillerFootballer.getBirthYear());
		this.characterOffset = tonyMillerFootballer.getCharacterOffset();
		this.characterOnset = tonyMillerFootballer.getCharacterOnset();
		if (tonyMillerFootballer.getPosition() != null)
			this.position = tonyMillerFootballer.getPosition().getClass()
					.getDeclaredConstructor(tonyMillerFootballer.getPosition().getClass())
					.newInstance(tonyMillerFootballer.getPosition());
		for (int j = 0; j < tonyMillerFootballer.getTeams().size(); j++) {
			if (tonyMillerFootballer.getTeams().get(j) != null) {
				teams.add(tonyMillerFootballer.getTeams().get(j).getClass()
						.getDeclaredConstructor(tonyMillerFootballer.getTeams().get(j).getClass())
						.newInstance(tonyMillerFootballer.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "TonyMillerFootballer [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
