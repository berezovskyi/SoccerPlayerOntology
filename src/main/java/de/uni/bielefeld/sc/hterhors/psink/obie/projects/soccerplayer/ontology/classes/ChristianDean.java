...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IChristianDean;
...
public class ChristianDean implements IChristianDean {
	...
	public ChristianDean(ChristianDean christianDean) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = christianDean.getAnnotationID();
		for (int j = 0; j < christianDean.getBirthPlaces().size(); j++) {
			if (christianDean.getBirthPlaces().get(j) != null) {
				birthPlaces.add(christianDean.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(christianDean.getBirthPlaces().get(j).getClass())
						.newInstance(christianDean.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (christianDean.getBirthYear() != null)
			this.birthYear = christianDean.getBirthYear().getClass()
					.getDeclaredConstructor(christianDean.getBirthYear().getClass())
					.newInstance(christianDean.getBirthYear());
		this.characterOffset = christianDean.getCharacterOffset();
		this.characterOnset = christianDean.getCharacterOnset();
		if (christianDean.getPosition() != null)
			this.position = christianDean.getPosition().getClass()
					.getDeclaredConstructor(christianDean.getPosition().getClass())
					.newInstance(christianDean.getPosition());
		for (int j = 0; j < christianDean.getTeams().size(); j++) {
			if (christianDean.getTeams().get(j) != null) {
				teams.add(christianDean.getTeams().get(j).getClass()
						.getDeclaredConstructor(christianDean.getTeams().get(j).getClass())
						.newInstance(christianDean.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ChristianDean [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
