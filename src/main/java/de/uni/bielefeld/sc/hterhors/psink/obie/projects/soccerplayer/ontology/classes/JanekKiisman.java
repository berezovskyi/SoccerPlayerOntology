...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJanekKiisman;
...
public class JanekKiisman implements IJanekKiisman {
	...
	public JanekKiisman(JanekKiisman janekKiisman) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = janekKiisman.getAnnotationID();
		for (int j = 0; j < janekKiisman.getBirthPlaces().size(); j++) {
			if (janekKiisman.getBirthPlaces().get(j) != null) {
				birthPlaces.add(janekKiisman.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(janekKiisman.getBirthPlaces().get(j).getClass())
						.newInstance(janekKiisman.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (janekKiisman.getBirthYear() != null)
			this.birthYear = janekKiisman.getBirthYear().getClass()
					.getDeclaredConstructor(janekKiisman.getBirthYear().getClass())
					.newInstance(janekKiisman.getBirthYear());
		this.characterOffset = janekKiisman.getCharacterOffset();
		this.characterOnset = janekKiisman.getCharacterOnset();
		if (janekKiisman.getPosition() != null)
			this.position = janekKiisman.getPosition().getClass()
					.getDeclaredConstructor(janekKiisman.getPosition().getClass())
					.newInstance(janekKiisman.getPosition());
		for (int j = 0; j < janekKiisman.getTeams().size(); j++) {
			if (janekKiisman.getTeams().get(j) != null) {
				teams.add(janekKiisman.getTeams().get(j).getClass()
						.getDeclaredConstructor(janekKiisman.getTeams().get(j).getClass())
						.newInstance(janekKiisman.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JanekKiisman [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
