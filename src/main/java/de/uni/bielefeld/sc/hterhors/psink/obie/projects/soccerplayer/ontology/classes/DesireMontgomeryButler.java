...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDesireMontgomeryButler;
...
public class DesireMontgomeryButler implements IDesireMontgomeryButler {
	...
	public DesireMontgomeryButler(DesireMontgomeryButler desireMontgomeryButler)
			throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
			NoSuchMethodException, SecurityException {
		this.annotationID = desireMontgomeryButler.getAnnotationID();
		for (int j = 0; j < desireMontgomeryButler.getBirthPlaces().size(); j++) {
			if (desireMontgomeryButler.getBirthPlaces().get(j) != null) {
				birthPlaces.add(desireMontgomeryButler.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(desireMontgomeryButler.getBirthPlaces().get(j).getClass())
						.newInstance(desireMontgomeryButler.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (desireMontgomeryButler.getBirthYear() != null)
			this.birthYear = desireMontgomeryButler.getBirthYear().getClass()
					.getDeclaredConstructor(desireMontgomeryButler.getBirthYear().getClass())
					.newInstance(desireMontgomeryButler.getBirthYear());
		...
		if (desireMontgomeryButler.getPosition() != null)
			this.position = desireMontgomeryButler.getPosition().getClass()
					.getDeclaredConstructor(desireMontgomeryButler.getPosition().getClass())
					.newInstance(desireMontgomeryButler.getPosition());
		for (int j = 0; j < desireMontgomeryButler.getTeams().size(); j++) {
			if (desireMontgomeryButler.getTeams().get(j) != null) {
				teams.add(desireMontgomeryButler.getTeams().get(j).getClass()
						.getDeclaredConstructor(desireMontgomeryButler.getTeams().get(j).getClass())
						.newInstance(desireMontgomeryButler.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DesireMontgomeryButler [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
