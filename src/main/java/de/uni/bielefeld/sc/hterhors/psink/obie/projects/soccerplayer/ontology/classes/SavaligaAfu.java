...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISavaligaAfu;
...
public class SavaligaAfu implements ISavaligaAfu {
	...
	public SavaligaAfu(SavaligaAfu savaligaAfu) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = savaligaAfu.getAnnotationID();
		for (int j = 0; j < savaligaAfu.getBirthPlaces().size(); j++) {
			if (savaligaAfu.getBirthPlaces().get(j) != null) {
				birthPlaces.add(savaligaAfu.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(savaligaAfu.getBirthPlaces().get(j).getClass())
						.newInstance(savaligaAfu.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (savaligaAfu.getBirthYear() != null)
			this.birthYear = savaligaAfu.getBirthYear().getClass()
					.getDeclaredConstructor(savaligaAfu.getBirthYear().getClass())
					.newInstance(savaligaAfu.getBirthYear());
		this.characterOffset = savaligaAfu.getCharacterOffset();
		this.characterOnset = savaligaAfu.getCharacterOnset();
		if (savaligaAfu.getPosition() != null)
			this.position = savaligaAfu.getPosition().getClass()
					.getDeclaredConstructor(savaligaAfu.getPosition().getClass())
					.newInstance(savaligaAfu.getPosition());
		for (int j = 0; j < savaligaAfu.getTeams().size(); j++) {
			if (savaligaAfu.getTeams().get(j) != null) {
				teams.add(savaligaAfu.getTeams().get(j).getClass()
						.getDeclaredConstructor(savaligaAfu.getTeams().get(j).getClass())
						.newInstance(savaligaAfu.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SavaligaAfu [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
