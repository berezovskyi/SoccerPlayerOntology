...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJoshRowbotham;
...
public class JoshRowbotham implements IJoshRowbotham {
	...
	public JoshRowbotham(JoshRowbotham joshRowbotham) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = joshRowbotham.getAnnotationID();
		for (int j = 0; j < joshRowbotham.getBirthPlaces().size(); j++) {
			if (joshRowbotham.getBirthPlaces().get(j) != null) {
				birthPlaces.add(joshRowbotham.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(joshRowbotham.getBirthPlaces().get(j).getClass())
						.newInstance(joshRowbotham.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (joshRowbotham.getBirthYear() != null)
			this.birthYear = joshRowbotham.getBirthYear().getClass()
					.getDeclaredConstructor(joshRowbotham.getBirthYear().getClass())
					.newInstance(joshRowbotham.getBirthYear());
		this.characterOffset = joshRowbotham.getCharacterOffset();
		this.characterOnset = joshRowbotham.getCharacterOnset();
		if (joshRowbotham.getPosition() != null)
			this.position = joshRowbotham.getPosition().getClass()
					.getDeclaredConstructor(joshRowbotham.getPosition().getClass())
					.newInstance(joshRowbotham.getPosition());
		for (int j = 0; j < joshRowbotham.getTeams().size(); j++) {
			if (joshRowbotham.getTeams().get(j) != null) {
				teams.add(joshRowbotham.getTeams().get(j).getClass()
						.getDeclaredConstructor(joshRowbotham.getTeams().get(j).getClass())
						.newInstance(joshRowbotham.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JoshRowbotham [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
