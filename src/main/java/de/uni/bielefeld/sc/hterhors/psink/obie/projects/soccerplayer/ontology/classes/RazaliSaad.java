...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRazaliSaad;
...
public class RazaliSaad implements IRazaliSaad {
	...
	public RazaliSaad(RazaliSaad razaliSaad) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = razaliSaad.getAnnotationID();
		for (int j = 0; j < razaliSaad.getBirthPlaces().size(); j++) {
			if (razaliSaad.getBirthPlaces().get(j) != null) {
				birthPlaces.add(razaliSaad.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(razaliSaad.getBirthPlaces().get(j).getClass())
						.newInstance(razaliSaad.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (razaliSaad.getBirthYear() != null)
			this.birthYear = razaliSaad.getBirthYear().getClass()
					.getDeclaredConstructor(razaliSaad.getBirthYear().getClass())
					.newInstance(razaliSaad.getBirthYear());
		this.characterOffset = razaliSaad.getCharacterOffset();
		this.characterOnset = razaliSaad.getCharacterOnset();
		if (razaliSaad.getPosition() != null)
			this.position = razaliSaad.getPosition().getClass()
					.getDeclaredConstructor(razaliSaad.getPosition().getClass()).newInstance(razaliSaad.getPosition());
		for (int j = 0; j < razaliSaad.getTeams().size(); j++) {
			if (razaliSaad.getTeams().get(j) != null) {
				teams.add(razaliSaad.getTeams().get(j).getClass()
						.getDeclaredConstructor(razaliSaad.getTeams().get(j).getClass())
						.newInstance(razaliSaad.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RazaliSaad [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
