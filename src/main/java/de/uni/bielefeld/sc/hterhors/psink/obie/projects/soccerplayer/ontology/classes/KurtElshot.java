...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IKurtElshot;
...
public class KurtElshot implements IKurtElshot {
	...
	public KurtElshot(KurtElshot kurtElshot) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = kurtElshot.getAnnotationID();
		for (int j = 0; j < kurtElshot.getBirthPlaces().size(); j++) {
			if (kurtElshot.getBirthPlaces().get(j) != null) {
				birthPlaces.add(kurtElshot.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(kurtElshot.getBirthPlaces().get(j).getClass())
						.newInstance(kurtElshot.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (kurtElshot.getBirthYear() != null)
			this.birthYear = kurtElshot.getBirthYear().getClass()
					.getDeclaredConstructor(kurtElshot.getBirthYear().getClass())
					.newInstance(kurtElshot.getBirthYear());
		this.characterOffset = kurtElshot.getCharacterOffset();
		this.characterOnset = kurtElshot.getCharacterOnset();
		if (kurtElshot.getPosition() != null)
			this.position = kurtElshot.getPosition().getClass()
					.getDeclaredConstructor(kurtElshot.getPosition().getClass()).newInstance(kurtElshot.getPosition());
		for (int j = 0; j < kurtElshot.getTeams().size(); j++) {
			if (kurtElshot.getTeams().get(j) != null) {
				teams.add(kurtElshot.getTeams().get(j).getClass()
						.getDeclaredConstructor(kurtElshot.getTeams().get(j).getClass())
						.newInstance(kurtElshot.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "KurtElshot [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
