...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.INatWalton;
...
public class NatWalton implements INatWalton {
	...
	public NatWalton(NatWalton natWalton) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = natWalton.getAnnotationID();
		for (int j = 0; j < natWalton.getBirthPlaces().size(); j++) {
			if (natWalton.getBirthPlaces().get(j) != null) {
				birthPlaces.add(natWalton.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(natWalton.getBirthPlaces().get(j).getClass())
						.newInstance(natWalton.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (natWalton.getBirthYear() != null)
			this.birthYear = natWalton.getBirthYear().getClass()
					.getDeclaredConstructor(natWalton.getBirthYear().getClass()).newInstance(natWalton.getBirthYear());
		this.characterOffset = natWalton.getCharacterOffset();
		this.characterOnset = natWalton.getCharacterOnset();
		if (natWalton.getPosition() != null)
			this.position = natWalton.getPosition().getClass()
					.getDeclaredConstructor(natWalton.getPosition().getClass()).newInstance(natWalton.getPosition());
		for (int j = 0; j < natWalton.getTeams().size(); j++) {
			if (natWalton.getTeams().get(j) != null) {
				teams.add(natWalton.getTeams().get(j).getClass()
						.getDeclaredConstructor(natWalton.getTeams().get(j).getClass())
						.newInstance(natWalton.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "NatWalton [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
