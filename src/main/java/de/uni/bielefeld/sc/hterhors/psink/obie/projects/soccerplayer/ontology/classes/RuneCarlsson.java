...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRuneCarlsson;
...
public class RuneCarlsson implements IRuneCarlsson {
	...
	public RuneCarlsson(RuneCarlsson runeCarlsson) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = runeCarlsson.getAnnotationID();
		for (int j = 0; j < runeCarlsson.getBirthPlaces().size(); j++) {
			if (runeCarlsson.getBirthPlaces().get(j) != null) {
				birthPlaces.add(runeCarlsson.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(runeCarlsson.getBirthPlaces().get(j).getClass())
						.newInstance(runeCarlsson.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (runeCarlsson.getBirthYear() != null)
			this.birthYear = runeCarlsson.getBirthYear().getClass()
					.getDeclaredConstructor(runeCarlsson.getBirthYear().getClass())
					.newInstance(runeCarlsson.getBirthYear());
		this.characterOffset = runeCarlsson.getCharacterOffset();
		this.characterOnset = runeCarlsson.getCharacterOnset();
		if (runeCarlsson.getPosition() != null)
			this.position = runeCarlsson.getPosition().getClass()
					.getDeclaredConstructor(runeCarlsson.getPosition().getClass())
					.newInstance(runeCarlsson.getPosition());
		for (int j = 0; j < runeCarlsson.getTeams().size(); j++) {
			if (runeCarlsson.getTeams().get(j) != null) {
				teams.add(runeCarlsson.getTeams().get(j).getClass()
						.getDeclaredConstructor(runeCarlsson.getTeams().get(j).getClass())
						.newInstance(runeCarlsson.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RuneCarlsson [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
