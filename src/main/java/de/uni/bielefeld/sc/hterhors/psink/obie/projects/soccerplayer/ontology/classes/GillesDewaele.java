...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IGillesDewaele;
...
public class GillesDewaele implements IGillesDewaele {
	...
	public GillesDewaele(GillesDewaele gillesDewaele) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = gillesDewaele.getAnnotationID();
		for (int j = 0; j < gillesDewaele.getBirthPlaces().size(); j++) {
			if (gillesDewaele.getBirthPlaces().get(j) != null) {
				birthPlaces.add(gillesDewaele.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(gillesDewaele.getBirthPlaces().get(j).getClass())
						.newInstance(gillesDewaele.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (gillesDewaele.getBirthYear() != null)
			this.birthYear = gillesDewaele.getBirthYear().getClass()
					.getDeclaredConstructor(gillesDewaele.getBirthYear().getClass())
					.newInstance(gillesDewaele.getBirthYear());
		this.characterOffset = gillesDewaele.getCharacterOffset();
		this.characterOnset = gillesDewaele.getCharacterOnset();
		if (gillesDewaele.getPosition() != null)
			this.position = gillesDewaele.getPosition().getClass()
					.getDeclaredConstructor(gillesDewaele.getPosition().getClass())
					.newInstance(gillesDewaele.getPosition());
		for (int j = 0; j < gillesDewaele.getTeams().size(); j++) {
			if (gillesDewaele.getTeams().get(j) != null) {
				teams.add(gillesDewaele.getTeams().get(j).getClass()
						.getDeclaredConstructor(gillesDewaele.getTeams().get(j).getClass())
						.newInstance(gillesDewaele.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "GillesDewaele [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
