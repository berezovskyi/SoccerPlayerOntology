...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IStewartBright;
...
public class StewartBright implements IStewartBright {
	...
	public StewartBright(StewartBright stewartBright) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = stewartBright.getAnnotationID();
		for (int j = 0; j < stewartBright.getBirthPlaces().size(); j++) {
			if (stewartBright.getBirthPlaces().get(j) != null) {
				birthPlaces.add(stewartBright.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(stewartBright.getBirthPlaces().get(j).getClass())
						.newInstance(stewartBright.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (stewartBright.getBirthYear() != null)
			this.birthYear = stewartBright.getBirthYear().getClass()
					.getDeclaredConstructor(stewartBright.getBirthYear().getClass())
					.newInstance(stewartBright.getBirthYear());
		this.characterOffset = stewartBright.getCharacterOffset();
		this.characterOnset = stewartBright.getCharacterOnset();
		if (stewartBright.getPosition() != null)
			this.position = stewartBright.getPosition().getClass()
					.getDeclaredConstructor(stewartBright.getPosition().getClass())
					.newInstance(stewartBright.getPosition());
		for (int j = 0; j < stewartBright.getTeams().size(); j++) {
			if (stewartBright.getTeams().get(j) != null) {
				teams.add(stewartBright.getTeams().get(j).getClass()
						.getDeclaredConstructor(stewartBright.getTeams().get(j).getClass())
						.newInstance(stewartBright.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "StewartBright [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
