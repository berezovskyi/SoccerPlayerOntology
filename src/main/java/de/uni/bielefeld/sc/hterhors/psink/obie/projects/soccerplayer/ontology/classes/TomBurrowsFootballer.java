...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ITomBurrowsFootballer;
...
public class TomBurrowsFootballer implements ITomBurrowsFootballer {
	...
	public TomBurrowsFootballer(TomBurrowsFootballer tomBurrowsFootballer)
			throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
			NoSuchMethodException, SecurityException {
		this.annotationID = tomBurrowsFootballer.getAnnotationID();
		for (int j = 0; j < tomBurrowsFootballer.getBirthPlaces().size(); j++) {
			if (tomBurrowsFootballer.getBirthPlaces().get(j) != null) {
				birthPlaces.add(tomBurrowsFootballer.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(tomBurrowsFootballer.getBirthPlaces().get(j).getClass())
						.newInstance(tomBurrowsFootballer.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (tomBurrowsFootballer.getBirthYear() != null)
			this.birthYear = tomBurrowsFootballer.getBirthYear().getClass()
					.getDeclaredConstructor(tomBurrowsFootballer.getBirthYear().getClass())
					.newInstance(tomBurrowsFootballer.getBirthYear());
		this.characterOffset = tomBurrowsFootballer.getCharacterOffset();
		this.characterOnset = tomBurrowsFootballer.getCharacterOnset();
		if (tomBurrowsFootballer.getPosition() != null)
			this.position = tomBurrowsFootballer.getPosition().getClass()
					.getDeclaredConstructor(tomBurrowsFootballer.getPosition().getClass())
					.newInstance(tomBurrowsFootballer.getPosition());
		for (int j = 0; j < tomBurrowsFootballer.getTeams().size(); j++) {
			if (tomBurrowsFootballer.getTeams().get(j) != null) {
				teams.add(tomBurrowsFootballer.getTeams().get(j).getClass()
						.getDeclaredConstructor(tomBurrowsFootballer.getTeams().get(j).getClass())
						.newInstance(tomBurrowsFootballer.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "TomBurrowsFootballer [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
