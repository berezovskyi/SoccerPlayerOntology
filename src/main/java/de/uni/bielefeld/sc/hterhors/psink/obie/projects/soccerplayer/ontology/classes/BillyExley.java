...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBillyExley;
...
public class BillyExley implements IBillyExley {
	...
	public BillyExley(BillyExley billyExley) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = billyExley.getAnnotationID();
		for (int j = 0; j < billyExley.getBirthPlaces().size(); j++) {
			if (billyExley.getBirthPlaces().get(j) != null) {
				birthPlaces.add(billyExley.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(billyExley.getBirthPlaces().get(j).getClass())
						.newInstance(billyExley.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (billyExley.getBirthYear() != null)
			this.birthYear = billyExley.getBirthYear().getClass()
					.getDeclaredConstructor(billyExley.getBirthYear().getClass())
					.newInstance(billyExley.getBirthYear());
		this.characterOffset = billyExley.getCharacterOffset();
		this.characterOnset = billyExley.getCharacterOnset();
		if (billyExley.getPosition() != null)
			this.position = billyExley.getPosition().getClass()
					.getDeclaredConstructor(billyExley.getPosition().getClass()).newInstance(billyExley.getPosition());
		for (int j = 0; j < billyExley.getTeams().size(); j++) {
			if (billyExley.getTeams().get(j) != null) {
				teams.add(billyExley.getTeams().get(j).getClass()
						.getDeclaredConstructor(billyExley.getTeams().get(j).getClass())
						.newInstance(billyExley.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BillyExley [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
