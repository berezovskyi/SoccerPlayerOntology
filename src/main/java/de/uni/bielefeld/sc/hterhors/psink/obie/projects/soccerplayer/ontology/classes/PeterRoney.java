...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IPeterRoney;
...
public class PeterRoney implements IPeterRoney {
	...
	public PeterRoney(PeterRoney peterRoney) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = peterRoney.getAnnotationID();
		for (int j = 0; j < peterRoney.getBirthPlaces().size(); j++) {
			if (peterRoney.getBirthPlaces().get(j) != null) {
				birthPlaces.add(peterRoney.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(peterRoney.getBirthPlaces().get(j).getClass())
						.newInstance(peterRoney.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (peterRoney.getBirthYear() != null)
			this.birthYear = peterRoney.getBirthYear().getClass()
					.getDeclaredConstructor(peterRoney.getBirthYear().getClass())
					.newInstance(peterRoney.getBirthYear());
		this.characterOffset = peterRoney.getCharacterOffset();
		this.characterOnset = peterRoney.getCharacterOnset();
		if (peterRoney.getPosition() != null)
			this.position = peterRoney.getPosition().getClass()
					.getDeclaredConstructor(peterRoney.getPosition().getClass()).newInstance(peterRoney.getPosition());
		for (int j = 0; j < peterRoney.getTeams().size(); j++) {
			if (peterRoney.getTeams().get(j) != null) {
				teams.add(peterRoney.getTeams().get(j).getClass()
						.getDeclaredConstructor(peterRoney.getTeams().get(j).getClass())
						.newInstance(peterRoney.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "PeterRoney [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
