...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IHeinzVersteeg;
...
public class HeinzVersteeg implements IHeinzVersteeg {
	...
	public HeinzVersteeg(HeinzVersteeg heinzVersteeg) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = heinzVersteeg.getAnnotationID();
		for (int j = 0; j < heinzVersteeg.getBirthPlaces().size(); j++) {
			if (heinzVersteeg.getBirthPlaces().get(j) != null) {
				birthPlaces.add(heinzVersteeg.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(heinzVersteeg.getBirthPlaces().get(j).getClass())
						.newInstance(heinzVersteeg.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (heinzVersteeg.getBirthYear() != null)
			this.birthYear = heinzVersteeg.getBirthYear().getClass()
					.getDeclaredConstructor(heinzVersteeg.getBirthYear().getClass())
					.newInstance(heinzVersteeg.getBirthYear());
		this.characterOffset = heinzVersteeg.getCharacterOffset();
		this.characterOnset = heinzVersteeg.getCharacterOnset();
		if (heinzVersteeg.getPosition() != null)
			this.position = heinzVersteeg.getPosition().getClass()
					.getDeclaredConstructor(heinzVersteeg.getPosition().getClass())
					.newInstance(heinzVersteeg.getPosition());
		for (int j = 0; j < heinzVersteeg.getTeams().size(); j++) {
			if (heinzVersteeg.getTeams().get(j) != null) {
				teams.add(heinzVersteeg.getTeams().get(j).getClass()
						.getDeclaredConstructor(heinzVersteeg.getTeams().get(j).getClass())
						.newInstance(heinzVersteeg.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "HeinzVersteeg [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
