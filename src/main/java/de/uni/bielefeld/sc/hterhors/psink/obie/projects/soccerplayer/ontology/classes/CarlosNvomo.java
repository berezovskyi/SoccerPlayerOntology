...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ICarlosNvomo;
...
public class CarlosNvomo implements ICarlosNvomo {
	...
	public CarlosNvomo(CarlosNvomo carlosNvomo) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = carlosNvomo.getAnnotationID();
		for (int j = 0; j < carlosNvomo.getBirthPlaces().size(); j++) {
			if (carlosNvomo.getBirthPlaces().get(j) != null) {
				birthPlaces.add(carlosNvomo.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(carlosNvomo.getBirthPlaces().get(j).getClass())
						.newInstance(carlosNvomo.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (carlosNvomo.getBirthYear() != null)
			this.birthYear = carlosNvomo.getBirthYear().getClass()
					.getDeclaredConstructor(carlosNvomo.getBirthYear().getClass())
					.newInstance(carlosNvomo.getBirthYear());
		this.characterOffset = carlosNvomo.getCharacterOffset();
		this.characterOnset = carlosNvomo.getCharacterOnset();
		if (carlosNvomo.getPosition() != null)
			this.position = carlosNvomo.getPosition().getClass()
					.getDeclaredConstructor(carlosNvomo.getPosition().getClass())
					.newInstance(carlosNvomo.getPosition());
		for (int j = 0; j < carlosNvomo.getTeams().size(); j++) {
			if (carlosNvomo.getTeams().get(j) != null) {
				teams.add(carlosNvomo.getTeams().get(j).getClass()
						.getDeclaredConstructor(carlosNvomo.getTeams().get(j).getClass())
						.newInstance(carlosNvomo.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "CarlosNvomo [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
