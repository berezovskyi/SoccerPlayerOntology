...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IArthurTopham;
...
public class ArthurTopham implements IArthurTopham {
	...
	public ArthurTopham(ArthurTopham arthurTopham) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = arthurTopham.getAnnotationID();
		for (int j = 0; j < arthurTopham.getBirthPlaces().size(); j++) {
			if (arthurTopham.getBirthPlaces().get(j) != null) {
				birthPlaces.add(arthurTopham.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(arthurTopham.getBirthPlaces().get(j).getClass())
						.newInstance(arthurTopham.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (arthurTopham.getBirthYear() != null)
			this.birthYear = arthurTopham.getBirthYear().getClass()
					.getDeclaredConstructor(arthurTopham.getBirthYear().getClass())
					.newInstance(arthurTopham.getBirthYear());
		this.characterOffset = arthurTopham.getCharacterOffset();
		this.characterOnset = arthurTopham.getCharacterOnset();
		if (arthurTopham.getPosition() != null)
			this.position = arthurTopham.getPosition().getClass()
					.getDeclaredConstructor(arthurTopham.getPosition().getClass())
					.newInstance(arthurTopham.getPosition());
		for (int j = 0; j < arthurTopham.getTeams().size(); j++) {
			if (arthurTopham.getTeams().get(j) != null) {
				teams.add(arthurTopham.getTeams().get(j).getClass()
						.getDeclaredConstructor(arthurTopham.getTeams().get(j).getClass())
						.newInstance(arthurTopham.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ArthurTopham [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
