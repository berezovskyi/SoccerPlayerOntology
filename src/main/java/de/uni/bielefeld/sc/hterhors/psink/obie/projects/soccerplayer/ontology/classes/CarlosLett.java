...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ICarlosLett;
...
public class CarlosLett implements ICarlosLett {
	...
	public CarlosLett(CarlosLett carlosLett) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = carlosLett.getAnnotationID();
		for (int j = 0; j < carlosLett.getBirthPlaces().size(); j++) {
			if (carlosLett.getBirthPlaces().get(j) != null) {
				birthPlaces.add(carlosLett.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(carlosLett.getBirthPlaces().get(j).getClass())
						.newInstance(carlosLett.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (carlosLett.getBirthYear() != null)
			this.birthYear = carlosLett.getBirthYear().getClass()
					.getDeclaredConstructor(carlosLett.getBirthYear().getClass())
					.newInstance(carlosLett.getBirthYear());
		this.characterOffset = carlosLett.getCharacterOffset();
		this.characterOnset = carlosLett.getCharacterOnset();
		if (carlosLett.getPosition() != null)
			this.position = carlosLett.getPosition().getClass()
					.getDeclaredConstructor(carlosLett.getPosition().getClass()).newInstance(carlosLett.getPosition());
		for (int j = 0; j < carlosLett.getTeams().size(); j++) {
			if (carlosLett.getTeams().get(j) != null) {
				teams.add(carlosLett.getTeams().get(j).getClass()
						.getDeclaredConstructor(carlosLett.getTeams().get(j).getClass())
						.newInstance(carlosLett.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "CarlosLett [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
