...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IFadelJilal;
...
public class FadelJilal implements IFadelJilal {
	...
	public FadelJilal(FadelJilal fadelJilal) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = fadelJilal.getAnnotationID();
		for (int j = 0; j < fadelJilal.getBirthPlaces().size(); j++) {
			if (fadelJilal.getBirthPlaces().get(j) != null) {
				birthPlaces.add(fadelJilal.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(fadelJilal.getBirthPlaces().get(j).getClass())
						.newInstance(fadelJilal.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (fadelJilal.getBirthYear() != null)
			this.birthYear = fadelJilal.getBirthYear().getClass()
					.getDeclaredConstructor(fadelJilal.getBirthYear().getClass())
					.newInstance(fadelJilal.getBirthYear());
		this.characterOffset = fadelJilal.getCharacterOffset();
		this.characterOnset = fadelJilal.getCharacterOnset();
		if (fadelJilal.getPosition() != null)
			this.position = fadelJilal.getPosition().getClass()
					.getDeclaredConstructor(fadelJilal.getPosition().getClass()).newInstance(fadelJilal.getPosition());
		for (int j = 0; j < fadelJilal.getTeams().size(); j++) {
			if (fadelJilal.getTeams().get(j) != null) {
				teams.add(fadelJilal.getTeams().get(j).getClass()
						.getDeclaredConstructor(fadelJilal.getTeams().get(j).getClass())
						.newInstance(fadelJilal.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "FadelJilal [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
