...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.INoelGeorge;
...
public class NoelGeorge implements INoelGeorge {
	...
	public NoelGeorge(NoelGeorge noelGeorge) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = noelGeorge.getAnnotationID();
		for (int j = 0; j < noelGeorge.getBirthPlaces().size(); j++) {
			if (noelGeorge.getBirthPlaces().get(j) != null) {
				birthPlaces.add(noelGeorge.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(noelGeorge.getBirthPlaces().get(j).getClass())
						.newInstance(noelGeorge.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (noelGeorge.getBirthYear() != null)
			this.birthYear = noelGeorge.getBirthYear().getClass()
					.getDeclaredConstructor(noelGeorge.getBirthYear().getClass())
					.newInstance(noelGeorge.getBirthYear());
		this.characterOffset = noelGeorge.getCharacterOffset();
		this.characterOnset = noelGeorge.getCharacterOnset();
		if (noelGeorge.getPosition() != null)
			this.position = noelGeorge.getPosition().getClass()
					.getDeclaredConstructor(noelGeorge.getPosition().getClass()).newInstance(noelGeorge.getPosition());
		for (int j = 0; j < noelGeorge.getTeams().size(); j++) {
			if (noelGeorge.getTeams().get(j) != null) {
				teams.add(noelGeorge.getTeams().get(j).getClass()
						.getDeclaredConstructor(noelGeorge.getTeams().get(j).getClass())
						.newInstance(noelGeorge.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "NoelGeorge [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
