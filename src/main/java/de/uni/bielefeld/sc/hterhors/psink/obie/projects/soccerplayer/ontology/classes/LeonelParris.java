...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILeonelParris;
...
public class LeonelParris implements ILeonelParris {
	...
	public LeonelParris(LeonelParris leonelParris) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = leonelParris.getAnnotationID();
		for (int j = 0; j < leonelParris.getBirthPlaces().size(); j++) {
			if (leonelParris.getBirthPlaces().get(j) != null) {
				birthPlaces.add(leonelParris.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(leonelParris.getBirthPlaces().get(j).getClass())
						.newInstance(leonelParris.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (leonelParris.getBirthYear() != null)
			this.birthYear = leonelParris.getBirthYear().getClass()
					.getDeclaredConstructor(leonelParris.getBirthYear().getClass())
					.newInstance(leonelParris.getBirthYear());
		this.characterOffset = leonelParris.getCharacterOffset();
		this.characterOnset = leonelParris.getCharacterOnset();
		if (leonelParris.getPosition() != null)
			this.position = leonelParris.getPosition().getClass()
					.getDeclaredConstructor(leonelParris.getPosition().getClass())
					.newInstance(leonelParris.getPosition());
		for (int j = 0; j < leonelParris.getTeams().size(); j++) {
			if (leonelParris.getTeams().get(j) != null) {
				teams.add(leonelParris.getTeams().get(j).getClass()
						.getDeclaredConstructor(leonelParris.getTeams().get(j).getClass())
						.newInstance(leonelParris.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LeonelParris [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
