...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILewisDunk;
...
public class LewisDunk implements ILewisDunk {
	...
	public LewisDunk(LewisDunk lewisDunk) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = lewisDunk.getAnnotationID();
		for (int j = 0; j < lewisDunk.getBirthPlaces().size(); j++) {
			if (lewisDunk.getBirthPlaces().get(j) != null) {
				birthPlaces.add(lewisDunk.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(lewisDunk.getBirthPlaces().get(j).getClass())
						.newInstance(lewisDunk.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (lewisDunk.getBirthYear() != null)
			this.birthYear = lewisDunk.getBirthYear().getClass()
					.getDeclaredConstructor(lewisDunk.getBirthYear().getClass()).newInstance(lewisDunk.getBirthYear());
		this.characterOffset = lewisDunk.getCharacterOffset();
		this.characterOnset = lewisDunk.getCharacterOnset();
		if (lewisDunk.getPosition() != null)
			this.position = lewisDunk.getPosition().getClass()
					.getDeclaredConstructor(lewisDunk.getPosition().getClass()).newInstance(lewisDunk.getPosition());
		for (int j = 0; j < lewisDunk.getTeams().size(); j++) {
			if (lewisDunk.getTeams().get(j) != null) {
				teams.add(lewisDunk.getTeams().get(j).getClass()
						.getDeclaredConstructor(lewisDunk.getTeams().get(j).getClass())
						.newInstance(lewisDunk.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LewisDunk [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
