...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ITeddyBrayshaw;
...
public class TeddyBrayshaw implements ITeddyBrayshaw {
	...
	public TeddyBrayshaw(TeddyBrayshaw teddyBrayshaw) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = teddyBrayshaw.getAnnotationID();
		for (int j = 0; j < teddyBrayshaw.getBirthPlaces().size(); j++) {
			if (teddyBrayshaw.getBirthPlaces().get(j) != null) {
				birthPlaces.add(teddyBrayshaw.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(teddyBrayshaw.getBirthPlaces().get(j).getClass())
						.newInstance(teddyBrayshaw.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (teddyBrayshaw.getBirthYear() != null)
			this.birthYear = teddyBrayshaw.getBirthYear().getClass()
					.getDeclaredConstructor(teddyBrayshaw.getBirthYear().getClass())
					.newInstance(teddyBrayshaw.getBirthYear());
		this.characterOffset = teddyBrayshaw.getCharacterOffset();
		this.characterOnset = teddyBrayshaw.getCharacterOnset();
		if (teddyBrayshaw.getPosition() != null)
			this.position = teddyBrayshaw.getPosition().getClass()
					.getDeclaredConstructor(teddyBrayshaw.getPosition().getClass())
					.newInstance(teddyBrayshaw.getPosition());
		for (int j = 0; j < teddyBrayshaw.getTeams().size(); j++) {
			if (teddyBrayshaw.getTeams().get(j) != null) {
				teams.add(teddyBrayshaw.getTeams().get(j).getClass()
						.getDeclaredConstructor(teddyBrayshaw.getTeams().get(j).getClass())
						.newInstance(teddyBrayshaw.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "TeddyBrayshaw [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
