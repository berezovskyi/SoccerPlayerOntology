...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IArthurStanton;
...
public class ArthurStanton implements IArthurStanton {
	...
	public ArthurStanton(ArthurStanton arthurStanton) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = arthurStanton.getAnnotationID();
		for (int j = 0; j < arthurStanton.getBirthPlaces().size(); j++) {
			if (arthurStanton.getBirthPlaces().get(j) != null) {
				birthPlaces.add(arthurStanton.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(arthurStanton.getBirthPlaces().get(j).getClass())
						.newInstance(arthurStanton.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (arthurStanton.getBirthYear() != null)
			this.birthYear = arthurStanton.getBirthYear().getClass()
					.getDeclaredConstructor(arthurStanton.getBirthYear().getClass())
					.newInstance(arthurStanton.getBirthYear());
		this.characterOffset = arthurStanton.getCharacterOffset();
		this.characterOnset = arthurStanton.getCharacterOnset();
		if (arthurStanton.getPosition() != null)
			this.position = arthurStanton.getPosition().getClass()
					.getDeclaredConstructor(arthurStanton.getPosition().getClass())
					.newInstance(arthurStanton.getPosition());
		for (int j = 0; j < arthurStanton.getTeams().size(); j++) {
			if (arthurStanton.getTeams().get(j) != null) {
				teams.add(arthurStanton.getTeams().get(j).getClass()
						.getDeclaredConstructor(arthurStanton.getTeams().get(j).getClass())
						.newInstance(arthurStanton.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ArthurStanton [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
