...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISurjayPariyar;
...
public class SurjayPariyar implements ISurjayPariyar {
	...
	public SurjayPariyar(SurjayPariyar surjayPariyar) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = surjayPariyar.getAnnotationID();
		for (int j = 0; j < surjayPariyar.getBirthPlaces().size(); j++) {
			if (surjayPariyar.getBirthPlaces().get(j) != null) {
				birthPlaces.add(surjayPariyar.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(surjayPariyar.getBirthPlaces().get(j).getClass())
						.newInstance(surjayPariyar.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (surjayPariyar.getBirthYear() != null)
			this.birthYear = surjayPariyar.getBirthYear().getClass()
					.getDeclaredConstructor(surjayPariyar.getBirthYear().getClass())
					.newInstance(surjayPariyar.getBirthYear());
		this.characterOffset = surjayPariyar.getCharacterOffset();
		this.characterOnset = surjayPariyar.getCharacterOnset();
		if (surjayPariyar.getPosition() != null)
			this.position = surjayPariyar.getPosition().getClass()
					.getDeclaredConstructor(surjayPariyar.getPosition().getClass())
					.newInstance(surjayPariyar.getPosition());
		for (int j = 0; j < surjayPariyar.getTeams().size(); j++) {
			if (surjayPariyar.getTeams().get(j) != null) {
				teams.add(surjayPariyar.getTeams().get(j).getClass()
						.getDeclaredConstructor(surjayPariyar.getTeams().get(j).getClass())
						.newInstance(surjayPariyar.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SurjayPariyar [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
