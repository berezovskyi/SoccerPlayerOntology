...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IPelayoEribo;
...
public class PelayoEribo implements IPelayoEribo {
	...
	public PelayoEribo(PelayoEribo pelayoEribo) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = pelayoEribo.getAnnotationID();
		for (int j = 0; j < pelayoEribo.getBirthPlaces().size(); j++) {
			if (pelayoEribo.getBirthPlaces().get(j) != null) {
				birthPlaces.add(pelayoEribo.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(pelayoEribo.getBirthPlaces().get(j).getClass())
						.newInstance(pelayoEribo.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (pelayoEribo.getBirthYear() != null)
			this.birthYear = pelayoEribo.getBirthYear().getClass()
					.getDeclaredConstructor(pelayoEribo.getBirthYear().getClass())
					.newInstance(pelayoEribo.getBirthYear());
		this.characterOffset = pelayoEribo.getCharacterOffset();
		this.characterOnset = pelayoEribo.getCharacterOnset();
		if (pelayoEribo.getPosition() != null)
			this.position = pelayoEribo.getPosition().getClass()
					.getDeclaredConstructor(pelayoEribo.getPosition().getClass())
					.newInstance(pelayoEribo.getPosition());
		for (int j = 0; j < pelayoEribo.getTeams().size(); j++) {
			if (pelayoEribo.getTeams().get(j) != null) {
				teams.add(pelayoEribo.getTeams().get(j).getClass()
						.getDeclaredConstructor(pelayoEribo.getTeams().get(j).getClass())
						.newInstance(pelayoEribo.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "PelayoEribo [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
