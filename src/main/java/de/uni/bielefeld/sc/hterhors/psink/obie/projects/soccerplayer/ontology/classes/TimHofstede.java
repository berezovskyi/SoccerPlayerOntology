...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ITimHofstede;
...
public class TimHofstede implements ITimHofstede {
	...
	public TimHofstede(TimHofstede timHofstede) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = timHofstede.getAnnotationID();
		for (int j = 0; j < timHofstede.getBirthPlaces().size(); j++) {
			if (timHofstede.getBirthPlaces().get(j) != null) {
				birthPlaces.add(timHofstede.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(timHofstede.getBirthPlaces().get(j).getClass())
						.newInstance(timHofstede.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (timHofstede.getBirthYear() != null)
			this.birthYear = timHofstede.getBirthYear().getClass()
					.getDeclaredConstructor(timHofstede.getBirthYear().getClass())
					.newInstance(timHofstede.getBirthYear());
		this.characterOffset = timHofstede.getCharacterOffset();
		this.characterOnset = timHofstede.getCharacterOnset();
		if (timHofstede.getPosition() != null)
			this.position = timHofstede.getPosition().getClass()
					.getDeclaredConstructor(timHofstede.getPosition().getClass())
					.newInstance(timHofstede.getPosition());
		for (int j = 0; j < timHofstede.getTeams().size(); j++) {
			if (timHofstede.getTeams().get(j) != null) {
				teams.add(timHofstede.getTeams().get(j).getClass()
						.getDeclaredConstructor(timHofstede.getTeams().get(j).getClass())
						.newInstance(timHofstede.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "TimHofstede [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
