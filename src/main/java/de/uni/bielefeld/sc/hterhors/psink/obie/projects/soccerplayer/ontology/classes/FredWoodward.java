...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IFredWoodward;
...
public class FredWoodward implements IFredWoodward {
	...
	public FredWoodward(FredWoodward fredWoodward) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = fredWoodward.getAnnotationID();
		for (int j = 0; j < fredWoodward.getBirthPlaces().size(); j++) {
			if (fredWoodward.getBirthPlaces().get(j) != null) {
				birthPlaces.add(fredWoodward.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(fredWoodward.getBirthPlaces().get(j).getClass())
						.newInstance(fredWoodward.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (fredWoodward.getBirthYear() != null)
			this.birthYear = fredWoodward.getBirthYear().getClass()
					.getDeclaredConstructor(fredWoodward.getBirthYear().getClass())
					.newInstance(fredWoodward.getBirthYear());
		this.characterOffset = fredWoodward.getCharacterOffset();
		this.characterOnset = fredWoodward.getCharacterOnset();
		if (fredWoodward.getPosition() != null)
			this.position = fredWoodward.getPosition().getClass()
					.getDeclaredConstructor(fredWoodward.getPosition().getClass())
					.newInstance(fredWoodward.getPosition());
		for (int j = 0; j < fredWoodward.getTeams().size(); j++) {
			if (fredWoodward.getTeams().get(j) != null) {
				teams.add(fredWoodward.getTeams().get(j).getClass()
						.getDeclaredConstructor(fredWoodward.getTeams().get(j).getClass())
						.newInstance(fredWoodward.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "FredWoodward [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
