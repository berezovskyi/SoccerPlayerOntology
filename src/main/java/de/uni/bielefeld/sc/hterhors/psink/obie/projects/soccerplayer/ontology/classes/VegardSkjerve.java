...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IVegardSkjerve;
...
public class VegardSkjerve implements IVegardSkjerve {
	...
	public VegardSkjerve(VegardSkjerve vegardSkjerve) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = vegardSkjerve.getAnnotationID();
		for (int j = 0; j < vegardSkjerve.getBirthPlaces().size(); j++) {
			if (vegardSkjerve.getBirthPlaces().get(j) != null) {
				birthPlaces.add(vegardSkjerve.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(vegardSkjerve.getBirthPlaces().get(j).getClass())
						.newInstance(vegardSkjerve.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (vegardSkjerve.getBirthYear() != null)
			this.birthYear = vegardSkjerve.getBirthYear().getClass()
					.getDeclaredConstructor(vegardSkjerve.getBirthYear().getClass())
					.newInstance(vegardSkjerve.getBirthYear());
		this.characterOffset = vegardSkjerve.getCharacterOffset();
		this.characterOnset = vegardSkjerve.getCharacterOnset();
		if (vegardSkjerve.getPosition() != null)
			this.position = vegardSkjerve.getPosition().getClass()
					.getDeclaredConstructor(vegardSkjerve.getPosition().getClass())
					.newInstance(vegardSkjerve.getPosition());
		for (int j = 0; j < vegardSkjerve.getTeams().size(); j++) {
			if (vegardSkjerve.getTeams().get(j) != null) {
				teams.add(vegardSkjerve.getTeams().get(j).getClass()
						.getDeclaredConstructor(vegardSkjerve.getTeams().get(j).getClass())
						.newInstance(vegardSkjerve.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "VegardSkjerve [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
