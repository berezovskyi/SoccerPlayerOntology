...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRowenMuscat;
...
public class RowenMuscat implements IRowenMuscat {
	...
	public RowenMuscat(RowenMuscat rowenMuscat) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = rowenMuscat.getAnnotationID();
		for (int j = 0; j < rowenMuscat.getBirthPlaces().size(); j++) {
			if (rowenMuscat.getBirthPlaces().get(j) != null) {
				birthPlaces.add(rowenMuscat.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(rowenMuscat.getBirthPlaces().get(j).getClass())
						.newInstance(rowenMuscat.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (rowenMuscat.getBirthYear() != null)
			this.birthYear = rowenMuscat.getBirthYear().getClass()
					.getDeclaredConstructor(rowenMuscat.getBirthYear().getClass())
					.newInstance(rowenMuscat.getBirthYear());
		this.characterOffset = rowenMuscat.getCharacterOffset();
		this.characterOnset = rowenMuscat.getCharacterOnset();
		if (rowenMuscat.getPosition() != null)
			this.position = rowenMuscat.getPosition().getClass()
					.getDeclaredConstructor(rowenMuscat.getPosition().getClass())
					.newInstance(rowenMuscat.getPosition());
		for (int j = 0; j < rowenMuscat.getTeams().size(); j++) {
			if (rowenMuscat.getTeams().get(j) != null) {
				teams.add(rowenMuscat.getTeams().get(j).getClass()
						.getDeclaredConstructor(rowenMuscat.getTeams().get(j).getClass())
						.newInstance(rowenMuscat.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RowenMuscat [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
