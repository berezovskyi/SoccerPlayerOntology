...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IHamoudAlShemmari;
...
public class HamoudAlShemmari implements IHamoudAlShemmari {
	...
	public HamoudAlShemmari(HamoudAlShemmari hamoudAlShemmari) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = hamoudAlShemmari.getAnnotationID();
		for (int j = 0; j < hamoudAlShemmari.getBirthPlaces().size(); j++) {
			if (hamoudAlShemmari.getBirthPlaces().get(j) != null) {
				birthPlaces.add(hamoudAlShemmari.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(hamoudAlShemmari.getBirthPlaces().get(j).getClass())
						.newInstance(hamoudAlShemmari.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (hamoudAlShemmari.getBirthYear() != null)
			this.birthYear = hamoudAlShemmari.getBirthYear().getClass()
					.getDeclaredConstructor(hamoudAlShemmari.getBirthYear().getClass())
					.newInstance(hamoudAlShemmari.getBirthYear());
		this.characterOffset = hamoudAlShemmari.getCharacterOffset();
		this.characterOnset = hamoudAlShemmari.getCharacterOnset();
		if (hamoudAlShemmari.getPosition() != null)
			this.position = hamoudAlShemmari.getPosition().getClass()
					.getDeclaredConstructor(hamoudAlShemmari.getPosition().getClass())
					.newInstance(hamoudAlShemmari.getPosition());
		for (int j = 0; j < hamoudAlShemmari.getTeams().size(); j++) {
			if (hamoudAlShemmari.getTeams().get(j) != null) {
				teams.add(hamoudAlShemmari.getTeams().get(j).getClass()
						.getDeclaredConstructor(hamoudAlShemmari.getTeams().get(j).getClass())
						.newInstance(hamoudAlShemmari.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "HamoudAlShemmari [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
