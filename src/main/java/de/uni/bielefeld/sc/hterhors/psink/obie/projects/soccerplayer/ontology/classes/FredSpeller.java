...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IFredSpeller;
...
public class FredSpeller implements IFredSpeller {
	...
	public FredSpeller(FredSpeller fredSpeller) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = fredSpeller.getAnnotationID();
		for (int j = 0; j < fredSpeller.getBirthPlaces().size(); j++) {
			if (fredSpeller.getBirthPlaces().get(j) != null) {
				birthPlaces.add(fredSpeller.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(fredSpeller.getBirthPlaces().get(j).getClass())
						.newInstance(fredSpeller.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (fredSpeller.getBirthYear() != null)
			this.birthYear = fredSpeller.getBirthYear().getClass()
					.getDeclaredConstructor(fredSpeller.getBirthYear().getClass())
					.newInstance(fredSpeller.getBirthYear());
		this.characterOffset = fredSpeller.getCharacterOffset();
		this.characterOnset = fredSpeller.getCharacterOnset();
		if (fredSpeller.getPosition() != null)
			this.position = fredSpeller.getPosition().getClass()
					.getDeclaredConstructor(fredSpeller.getPosition().getClass())
					.newInstance(fredSpeller.getPosition());
		for (int j = 0; j < fredSpeller.getTeams().size(); j++) {
			if (fredSpeller.getTeams().get(j) != null) {
				teams.add(fredSpeller.getTeams().get(j).getClass()
						.getDeclaredConstructor(fredSpeller.getTeams().get(j).getClass())
						.newInstance(fredSpeller.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "FredSpeller [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
