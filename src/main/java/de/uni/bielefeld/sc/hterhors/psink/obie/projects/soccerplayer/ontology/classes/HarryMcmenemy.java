...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IHarryMcmenemy;
...
public class HarryMcmenemy implements IHarryMcmenemy {
	...
	public HarryMcmenemy(HarryMcmenemy harryMcmenemy) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = harryMcmenemy.getAnnotationID();
		for (int j = 0; j < harryMcmenemy.getBirthPlaces().size(); j++) {
			if (harryMcmenemy.getBirthPlaces().get(j) != null) {
				birthPlaces.add(harryMcmenemy.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(harryMcmenemy.getBirthPlaces().get(j).getClass())
						.newInstance(harryMcmenemy.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (harryMcmenemy.getBirthYear() != null)
			this.birthYear = harryMcmenemy.getBirthYear().getClass()
					.getDeclaredConstructor(harryMcmenemy.getBirthYear().getClass())
					.newInstance(harryMcmenemy.getBirthYear());
		this.characterOffset = harryMcmenemy.getCharacterOffset();
		this.characterOnset = harryMcmenemy.getCharacterOnset();
		if (harryMcmenemy.getPosition() != null)
			this.position = harryMcmenemy.getPosition().getClass()
					.getDeclaredConstructor(harryMcmenemy.getPosition().getClass())
					.newInstance(harryMcmenemy.getPosition());
		for (int j = 0; j < harryMcmenemy.getTeams().size(); j++) {
			if (harryMcmenemy.getTeams().get(j) != null) {
				teams.add(harryMcmenemy.getTeams().get(j).getClass()
						.getDeclaredConstructor(harryMcmenemy.getTeams().get(j).getClass())
						.newInstance(harryMcmenemy.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "HarryMcmenemy [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
