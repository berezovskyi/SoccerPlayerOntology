...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMiguelVanDamme;
...
public class MiguelVanDamme implements IMiguelVanDamme {
	...
	public MiguelVanDamme(MiguelVanDamme miguelVanDamme) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = miguelVanDamme.getAnnotationID();
		for (int j = 0; j < miguelVanDamme.getBirthPlaces().size(); j++) {
			if (miguelVanDamme.getBirthPlaces().get(j) != null) {
				birthPlaces.add(miguelVanDamme.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(miguelVanDamme.getBirthPlaces().get(j).getClass())
						.newInstance(miguelVanDamme.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (miguelVanDamme.getBirthYear() != null)
			this.birthYear = miguelVanDamme.getBirthYear().getClass()
					.getDeclaredConstructor(miguelVanDamme.getBirthYear().getClass())
					.newInstance(miguelVanDamme.getBirthYear());
		this.characterOffset = miguelVanDamme.getCharacterOffset();
		this.characterOnset = miguelVanDamme.getCharacterOnset();
		if (miguelVanDamme.getPosition() != null)
			this.position = miguelVanDamme.getPosition().getClass()
					.getDeclaredConstructor(miguelVanDamme.getPosition().getClass())
					.newInstance(miguelVanDamme.getPosition());
		for (int j = 0; j < miguelVanDamme.getTeams().size(); j++) {
			if (miguelVanDamme.getTeams().get(j) != null) {
				teams.add(miguelVanDamme.getTeams().get(j).getClass()
						.getDeclaredConstructor(miguelVanDamme.getTeams().get(j).getClass())
						.newInstance(miguelVanDamme.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MiguelVanDamme [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
