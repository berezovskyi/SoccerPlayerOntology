...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJohannWindisch;
...
public class JohannWindisch implements IJohannWindisch {
	...
	public JohannWindisch(JohannWindisch johannWindisch) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = johannWindisch.getAnnotationID();
		for (int j = 0; j < johannWindisch.getBirthPlaces().size(); j++) {
			if (johannWindisch.getBirthPlaces().get(j) != null) {
				birthPlaces.add(johannWindisch.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(johannWindisch.getBirthPlaces().get(j).getClass())
						.newInstance(johannWindisch.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (johannWindisch.getBirthYear() != null)
			this.birthYear = johannWindisch.getBirthYear().getClass()
					.getDeclaredConstructor(johannWindisch.getBirthYear().getClass())
					.newInstance(johannWindisch.getBirthYear());
		this.characterOffset = johannWindisch.getCharacterOffset();
		this.characterOnset = johannWindisch.getCharacterOnset();
		if (johannWindisch.getPosition() != null)
			this.position = johannWindisch.getPosition().getClass()
					.getDeclaredConstructor(johannWindisch.getPosition().getClass())
					.newInstance(johannWindisch.getPosition());
		for (int j = 0; j < johannWindisch.getTeams().size(); j++) {
			if (johannWindisch.getTeams().get(j) != null) {
				teams.add(johannWindisch.getTeams().get(j).getClass()
						.getDeclaredConstructor(johannWindisch.getTeams().get(j).getClass())
						.newInstance(johannWindisch.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JohannWindisch [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
