...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IOfirMizrahi;
...
public class OfirMizrahi implements IOfirMizrahi {
	...
	public OfirMizrahi(OfirMizrahi ofirMizrahi) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = ofirMizrahi.getAnnotationID();
		for (int j = 0; j < ofirMizrahi.getBirthPlaces().size(); j++) {
			if (ofirMizrahi.getBirthPlaces().get(j) != null) {
				birthPlaces.add(ofirMizrahi.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(ofirMizrahi.getBirthPlaces().get(j).getClass())
						.newInstance(ofirMizrahi.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (ofirMizrahi.getBirthYear() != null)
			this.birthYear = ofirMizrahi.getBirthYear().getClass()
					.getDeclaredConstructor(ofirMizrahi.getBirthYear().getClass())
					.newInstance(ofirMizrahi.getBirthYear());
		this.characterOffset = ofirMizrahi.getCharacterOffset();
		this.characterOnset = ofirMizrahi.getCharacterOnset();
		if (ofirMizrahi.getPosition() != null)
			this.position = ofirMizrahi.getPosition().getClass()
					.getDeclaredConstructor(ofirMizrahi.getPosition().getClass())
					.newInstance(ofirMizrahi.getPosition());
		for (int j = 0; j < ofirMizrahi.getTeams().size(); j++) {
			if (ofirMizrahi.getTeams().get(j) != null) {
				teams.add(ofirMizrahi.getTeams().get(j).getClass()
						.getDeclaredConstructor(ofirMizrahi.getTeams().get(j).getClass())
						.newInstance(ofirMizrahi.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "OfirMizrahi [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
