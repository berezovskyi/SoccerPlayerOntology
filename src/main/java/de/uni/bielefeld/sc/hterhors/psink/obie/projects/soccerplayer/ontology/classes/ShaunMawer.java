...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IShaunMawer;
...
public class ShaunMawer implements IShaunMawer {
	...
	public ShaunMawer(ShaunMawer shaunMawer) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = shaunMawer.getAnnotationID();
		for (int j = 0; j < shaunMawer.getBirthPlaces().size(); j++) {
			if (shaunMawer.getBirthPlaces().get(j) != null) {
				birthPlaces.add(shaunMawer.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(shaunMawer.getBirthPlaces().get(j).getClass())
						.newInstance(shaunMawer.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (shaunMawer.getBirthYear() != null)
			this.birthYear = shaunMawer.getBirthYear().getClass()
					.getDeclaredConstructor(shaunMawer.getBirthYear().getClass())
					.newInstance(shaunMawer.getBirthYear());
		this.characterOffset = shaunMawer.getCharacterOffset();
		this.characterOnset = shaunMawer.getCharacterOnset();
		if (shaunMawer.getPosition() != null)
			this.position = shaunMawer.getPosition().getClass()
					.getDeclaredConstructor(shaunMawer.getPosition().getClass()).newInstance(shaunMawer.getPosition());
		for (int j = 0; j < shaunMawer.getTeams().size(); j++) {
			if (shaunMawer.getTeams().get(j) != null) {
				teams.add(shaunMawer.getTeams().get(j).getClass()
						.getDeclaredConstructor(shaunMawer.getTeams().get(j).getClass())
						.newInstance(shaunMawer.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ShaunMawer [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
