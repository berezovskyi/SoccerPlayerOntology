...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJoeReader;
...
public class JoeReader implements IJoeReader {
	...
	public JoeReader(JoeReader joeReader) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = joeReader.getAnnotationID();
		for (int j = 0; j < joeReader.getBirthPlaces().size(); j++) {
			if (joeReader.getBirthPlaces().get(j) != null) {
				birthPlaces.add(joeReader.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(joeReader.getBirthPlaces().get(j).getClass())
						.newInstance(joeReader.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (joeReader.getBirthYear() != null)
			this.birthYear = joeReader.getBirthYear().getClass()
					.getDeclaredConstructor(joeReader.getBirthYear().getClass()).newInstance(joeReader.getBirthYear());
		this.characterOffset = joeReader.getCharacterOffset();
		this.characterOnset = joeReader.getCharacterOnset();
		if (joeReader.getPosition() != null)
			this.position = joeReader.getPosition().getClass()
					.getDeclaredConstructor(joeReader.getPosition().getClass()).newInstance(joeReader.getPosition());
		for (int j = 0; j < joeReader.getTeams().size(); j++) {
			if (joeReader.getTeams().get(j) != null) {
				teams.add(joeReader.getTeams().get(j).getClass()
						.getDeclaredConstructor(joeReader.getTeams().get(j).getClass())
						.newInstance(joeReader.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JoeReader [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
