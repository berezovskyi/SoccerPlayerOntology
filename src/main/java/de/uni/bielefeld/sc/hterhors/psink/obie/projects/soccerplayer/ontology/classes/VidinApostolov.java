...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IVidinApostolov;
...
public class VidinApostolov implements IVidinApostolov {
	...
	public VidinApostolov(VidinApostolov vidinApostolov) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = vidinApostolov.getAnnotationID();
		for (int j = 0; j < vidinApostolov.getBirthPlaces().size(); j++) {
			if (vidinApostolov.getBirthPlaces().get(j) != null) {
				birthPlaces.add(vidinApostolov.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(vidinApostolov.getBirthPlaces().get(j).getClass())
						.newInstance(vidinApostolov.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (vidinApostolov.getBirthYear() != null)
			this.birthYear = vidinApostolov.getBirthYear().getClass()
					.getDeclaredConstructor(vidinApostolov.getBirthYear().getClass())
					.newInstance(vidinApostolov.getBirthYear());
		this.characterOffset = vidinApostolov.getCharacterOffset();
		this.characterOnset = vidinApostolov.getCharacterOnset();
		if (vidinApostolov.getPosition() != null)
			this.position = vidinApostolov.getPosition().getClass()
					.getDeclaredConstructor(vidinApostolov.getPosition().getClass())
					.newInstance(vidinApostolov.getPosition());
		for (int j = 0; j < vidinApostolov.getTeams().size(); j++) {
			if (vidinApostolov.getTeams().get(j) != null) {
				teams.add(vidinApostolov.getTeams().get(j).getClass()
						.getDeclaredConstructor(vidinApostolov.getTeams().get(j).getClass())
						.newInstance(vidinApostolov.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "VidinApostolov [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
