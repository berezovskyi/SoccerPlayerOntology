...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBenOlney;
...
public class BenOlney implements IBenOlney {
	...
	public BenOlney(BenOlney benOlney) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = benOlney.getAnnotationID();
		for (int j = 0; j < benOlney.getBirthPlaces().size(); j++) {
			if (benOlney.getBirthPlaces().get(j) != null) {
				birthPlaces.add(benOlney.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(benOlney.getBirthPlaces().get(j).getClass())
						.newInstance(benOlney.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (benOlney.getBirthYear() != null)
			this.birthYear = benOlney.getBirthYear().getClass()
					.getDeclaredConstructor(benOlney.getBirthYear().getClass()).newInstance(benOlney.getBirthYear());
		this.characterOffset = benOlney.getCharacterOffset();
		this.characterOnset = benOlney.getCharacterOnset();
		if (benOlney.getPosition() != null)
			this.position = benOlney.getPosition().getClass().getDeclaredConstructor(benOlney.getPosition().getClass())
					.newInstance(benOlney.getPosition());
		for (int j = 0; j < benOlney.getTeams().size(); j++) {
			if (benOlney.getTeams().get(j) != null) {
				teams.add(benOlney.getTeams().get(j).getClass()
						.getDeclaredConstructor(benOlney.getTeams().get(j).getClass())
						.newInstance(benOlney.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BenOlney [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
