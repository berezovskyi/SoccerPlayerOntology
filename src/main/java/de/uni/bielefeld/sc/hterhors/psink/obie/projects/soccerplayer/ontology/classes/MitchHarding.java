...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMitchHarding;
...
public class MitchHarding implements IMitchHarding {
	...
	public MitchHarding(MitchHarding mitchHarding) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = mitchHarding.getAnnotationID();
		for (int j = 0; j < mitchHarding.getBirthPlaces().size(); j++) {
			if (mitchHarding.getBirthPlaces().get(j) != null) {
				birthPlaces.add(mitchHarding.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(mitchHarding.getBirthPlaces().get(j).getClass())
						.newInstance(mitchHarding.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (mitchHarding.getBirthYear() != null)
			this.birthYear = mitchHarding.getBirthYear().getClass()
					.getDeclaredConstructor(mitchHarding.getBirthYear().getClass())
					.newInstance(mitchHarding.getBirthYear());
		this.characterOffset = mitchHarding.getCharacterOffset();
		this.characterOnset = mitchHarding.getCharacterOnset();
		if (mitchHarding.getPosition() != null)
			this.position = mitchHarding.getPosition().getClass()
					.getDeclaredConstructor(mitchHarding.getPosition().getClass())
					.newInstance(mitchHarding.getPosition());
		for (int j = 0; j < mitchHarding.getTeams().size(); j++) {
			if (mitchHarding.getTeams().get(j) != null) {
				teams.add(mitchHarding.getTeams().get(j).getClass()
						.getDeclaredConstructor(mitchHarding.getTeams().get(j).getClass())
						.newInstance(mitchHarding.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MitchHarding [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
