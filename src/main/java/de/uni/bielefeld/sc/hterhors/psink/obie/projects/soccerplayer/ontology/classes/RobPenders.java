...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRobPenders;
...
public class RobPenders implements IRobPenders {
	...
	public RobPenders(RobPenders robPenders) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = robPenders.getAnnotationID();
		for (int j = 0; j < robPenders.getBirthPlaces().size(); j++) {
			if (robPenders.getBirthPlaces().get(j) != null) {
				birthPlaces.add(robPenders.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(robPenders.getBirthPlaces().get(j).getClass())
						.newInstance(robPenders.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (robPenders.getBirthYear() != null)
			this.birthYear = robPenders.getBirthYear().getClass()
					.getDeclaredConstructor(robPenders.getBirthYear().getClass())
					.newInstance(robPenders.getBirthYear());
		this.characterOffset = robPenders.getCharacterOffset();
		this.characterOnset = robPenders.getCharacterOnset();
		if (robPenders.getPosition() != null)
			this.position = robPenders.getPosition().getClass()
					.getDeclaredConstructor(robPenders.getPosition().getClass()).newInstance(robPenders.getPosition());
		for (int j = 0; j < robPenders.getTeams().size(); j++) {
			if (robPenders.getTeams().get(j) != null) {
				teams.add(robPenders.getTeams().get(j).getClass()
						.getDeclaredConstructor(robPenders.getTeams().get(j).getClass())
						.newInstance(robPenders.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RobPenders [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
