...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISophusHansen;
...
public class SophusHansen implements ISophusHansen {
	...
	public SophusHansen(SophusHansen sophusHansen) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = sophusHansen.getAnnotationID();
		for (int j = 0; j < sophusHansen.getBirthPlaces().size(); j++) {
			if (sophusHansen.getBirthPlaces().get(j) != null) {
				birthPlaces.add(sophusHansen.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(sophusHansen.getBirthPlaces().get(j).getClass())
						.newInstance(sophusHansen.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (sophusHansen.getBirthYear() != null)
			this.birthYear = sophusHansen.getBirthYear().getClass()
					.getDeclaredConstructor(sophusHansen.getBirthYear().getClass())
					.newInstance(sophusHansen.getBirthYear());
		this.characterOffset = sophusHansen.getCharacterOffset();
		this.characterOnset = sophusHansen.getCharacterOnset();
		if (sophusHansen.getPosition() != null)
			this.position = sophusHansen.getPosition().getClass()
					.getDeclaredConstructor(sophusHansen.getPosition().getClass())
					.newInstance(sophusHansen.getPosition());
		for (int j = 0; j < sophusHansen.getTeams().size(); j++) {
			if (sophusHansen.getTeams().get(j) != null) {
				teams.add(sophusHansen.getTeams().get(j).getClass()
						.getDeclaredConstructor(sophusHansen.getTeams().get(j).getClass())
						.newInstance(sophusHansen.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SophusHansen [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
