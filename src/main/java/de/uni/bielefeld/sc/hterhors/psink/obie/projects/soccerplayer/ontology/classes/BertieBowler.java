...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBertieBowler;
...
public class BertieBowler implements IBertieBowler {
	...
	public BertieBowler(BertieBowler bertieBowler) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = bertieBowler.getAnnotationID();
		for (int j = 0; j < bertieBowler.getBirthPlaces().size(); j++) {
			if (bertieBowler.getBirthPlaces().get(j) != null) {
				birthPlaces.add(bertieBowler.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(bertieBowler.getBirthPlaces().get(j).getClass())
						.newInstance(bertieBowler.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (bertieBowler.getBirthYear() != null)
			this.birthYear = bertieBowler.getBirthYear().getClass()
					.getDeclaredConstructor(bertieBowler.getBirthYear().getClass())
					.newInstance(bertieBowler.getBirthYear());
		this.characterOffset = bertieBowler.getCharacterOffset();
		this.characterOnset = bertieBowler.getCharacterOnset();
		if (bertieBowler.getPosition() != null)
			this.position = bertieBowler.getPosition().getClass()
					.getDeclaredConstructor(bertieBowler.getPosition().getClass())
					.newInstance(bertieBowler.getPosition());
		for (int j = 0; j < bertieBowler.getTeams().size(); j++) {
			if (bertieBowler.getTeams().get(j) != null) {
				teams.add(bertieBowler.getTeams().get(j).getClass()
						.getDeclaredConstructor(bertieBowler.getTeams().get(j).getClass())
						.newInstance(bertieBowler.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BertieBowler [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
