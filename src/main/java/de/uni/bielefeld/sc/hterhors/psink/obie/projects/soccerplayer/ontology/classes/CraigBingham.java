...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ICraigBingham;
...
public class CraigBingham implements ICraigBingham {
	...
	public CraigBingham(CraigBingham craigBingham) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = craigBingham.getAnnotationID();
		for (int j = 0; j < craigBingham.getBirthPlaces().size(); j++) {
			if (craigBingham.getBirthPlaces().get(j) != null) {
				birthPlaces.add(craigBingham.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(craigBingham.getBirthPlaces().get(j).getClass())
						.newInstance(craigBingham.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (craigBingham.getBirthYear() != null)
			this.birthYear = craigBingham.getBirthYear().getClass()
					.getDeclaredConstructor(craigBingham.getBirthYear().getClass())
					.newInstance(craigBingham.getBirthYear());
		this.characterOffset = craigBingham.getCharacterOffset();
		this.characterOnset = craigBingham.getCharacterOnset();
		if (craigBingham.getPosition() != null)
			this.position = craigBingham.getPosition().getClass()
					.getDeclaredConstructor(craigBingham.getPosition().getClass())
					.newInstance(craigBingham.getPosition());
		for (int j = 0; j < craigBingham.getTeams().size(); j++) {
			if (craigBingham.getTeams().get(j) != null) {
				teams.add(craigBingham.getTeams().get(j).getClass()
						.getDeclaredConstructor(craigBingham.getTeams().get(j).getClass())
						.newInstance(craigBingham.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "CraigBingham [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
