...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ITihomirNaydenov;
...
public class TihomirNaydenov implements ITihomirNaydenov {
	...
	public TihomirNaydenov(TihomirNaydenov tihomirNaydenov) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = tihomirNaydenov.getAnnotationID();
		for (int j = 0; j < tihomirNaydenov.getBirthPlaces().size(); j++) {
			if (tihomirNaydenov.getBirthPlaces().get(j) != null) {
				birthPlaces.add(tihomirNaydenov.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(tihomirNaydenov.getBirthPlaces().get(j).getClass())
						.newInstance(tihomirNaydenov.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (tihomirNaydenov.getBirthYear() != null)
			this.birthYear = tihomirNaydenov.getBirthYear().getClass()
					.getDeclaredConstructor(tihomirNaydenov.getBirthYear().getClass())
					.newInstance(tihomirNaydenov.getBirthYear());
		this.characterOffset = tihomirNaydenov.getCharacterOffset();
		this.characterOnset = tihomirNaydenov.getCharacterOnset();
		if (tihomirNaydenov.getPosition() != null)
			this.position = tihomirNaydenov.getPosition().getClass()
					.getDeclaredConstructor(tihomirNaydenov.getPosition().getClass())
					.newInstance(tihomirNaydenov.getPosition());
		for (int j = 0; j < tihomirNaydenov.getTeams().size(); j++) {
			if (tihomirNaydenov.getTeams().get(j) != null) {
				teams.add(tihomirNaydenov.getTeams().get(j).getClass()
						.getDeclaredConstructor(tihomirNaydenov.getTeams().get(j).getClass())
						.newInstance(tihomirNaydenov.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "TihomirNaydenov [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
