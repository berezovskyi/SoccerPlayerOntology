...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IGaryRiddell;
...
public class GaryRiddell implements IGaryRiddell {
	...
	public GaryRiddell(GaryRiddell garyRiddell) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = garyRiddell.getAnnotationID();
		for (int j = 0; j < garyRiddell.getBirthPlaces().size(); j++) {
			if (garyRiddell.getBirthPlaces().get(j) != null) {
				birthPlaces.add(garyRiddell.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(garyRiddell.getBirthPlaces().get(j).getClass())
						.newInstance(garyRiddell.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (garyRiddell.getBirthYear() != null)
			this.birthYear = garyRiddell.getBirthYear().getClass()
					.getDeclaredConstructor(garyRiddell.getBirthYear().getClass())
					.newInstance(garyRiddell.getBirthYear());
		this.characterOffset = garyRiddell.getCharacterOffset();
		this.characterOnset = garyRiddell.getCharacterOnset();
		if (garyRiddell.getPosition() != null)
			this.position = garyRiddell.getPosition().getClass()
					.getDeclaredConstructor(garyRiddell.getPosition().getClass())
					.newInstance(garyRiddell.getPosition());
		for (int j = 0; j < garyRiddell.getTeams().size(); j++) {
			if (garyRiddell.getTeams().get(j) != null) {
				teams.add(garyRiddell.getTeams().get(j).getClass()
						.getDeclaredConstructor(garyRiddell.getTeams().get(j).getClass())
						.newInstance(garyRiddell.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "GaryRiddell [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
