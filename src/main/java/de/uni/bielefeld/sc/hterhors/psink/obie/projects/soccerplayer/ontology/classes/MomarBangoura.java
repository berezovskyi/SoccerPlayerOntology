...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMomarBangoura;
...
public class MomarBangoura implements IMomarBangoura {
	...
	public MomarBangoura(MomarBangoura momarBangoura) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = momarBangoura.getAnnotationID();
		for (int j = 0; j < momarBangoura.getBirthPlaces().size(); j++) {
			if (momarBangoura.getBirthPlaces().get(j) != null) {
				birthPlaces.add(momarBangoura.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(momarBangoura.getBirthPlaces().get(j).getClass())
						.newInstance(momarBangoura.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (momarBangoura.getBirthYear() != null)
			this.birthYear = momarBangoura.getBirthYear().getClass()
					.getDeclaredConstructor(momarBangoura.getBirthYear().getClass())
					.newInstance(momarBangoura.getBirthYear());
		this.characterOffset = momarBangoura.getCharacterOffset();
		this.characterOnset = momarBangoura.getCharacterOnset();
		if (momarBangoura.getPosition() != null)
			this.position = momarBangoura.getPosition().getClass()
					.getDeclaredConstructor(momarBangoura.getPosition().getClass())
					.newInstance(momarBangoura.getPosition());
		for (int j = 0; j < momarBangoura.getTeams().size(); j++) {
			if (momarBangoura.getTeams().get(j) != null) {
				teams.add(momarBangoura.getTeams().get(j).getClass()
						.getDeclaredConstructor(momarBangoura.getTeams().get(j).getClass())
						.newInstance(momarBangoura.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MomarBangoura [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
