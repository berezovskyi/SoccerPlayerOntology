...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJimGallacher;
...
public class JimGallacher implements IJimGallacher {
	...
	public JimGallacher(JimGallacher jimGallacher) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = jimGallacher.getAnnotationID();
		for (int j = 0; j < jimGallacher.getBirthPlaces().size(); j++) {
			if (jimGallacher.getBirthPlaces().get(j) != null) {
				birthPlaces.add(jimGallacher.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(jimGallacher.getBirthPlaces().get(j).getClass())
						.newInstance(jimGallacher.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (jimGallacher.getBirthYear() != null)
			this.birthYear = jimGallacher.getBirthYear().getClass()
					.getDeclaredConstructor(jimGallacher.getBirthYear().getClass())
					.newInstance(jimGallacher.getBirthYear());
		this.characterOffset = jimGallacher.getCharacterOffset();
		this.characterOnset = jimGallacher.getCharacterOnset();
		if (jimGallacher.getPosition() != null)
			this.position = jimGallacher.getPosition().getClass()
					.getDeclaredConstructor(jimGallacher.getPosition().getClass())
					.newInstance(jimGallacher.getPosition());
		for (int j = 0; j < jimGallacher.getTeams().size(); j++) {
			if (jimGallacher.getTeams().get(j) != null) {
				teams.add(jimGallacher.getTeams().get(j).getClass()
						.getDeclaredConstructor(jimGallacher.getTeams().get(j).getClass())
						.newInstance(jimGallacher.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JimGallacher [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
