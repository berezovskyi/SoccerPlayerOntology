...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDineshSinghFootballer;
...
public class DineshSinghFootballer implements IDineshSinghFootballer {
	...
	public DineshSinghFootballer(DineshSinghFootballer dineshSinghFootballer)
			throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
			NoSuchMethodException, SecurityException {
		this.annotationID = dineshSinghFootballer.getAnnotationID();
		for (int j = 0; j < dineshSinghFootballer.getBirthPlaces().size(); j++) {
			if (dineshSinghFootballer.getBirthPlaces().get(j) != null) {
				birthPlaces.add(dineshSinghFootballer.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(dineshSinghFootballer.getBirthPlaces().get(j).getClass())
						.newInstance(dineshSinghFootballer.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (dineshSinghFootballer.getBirthYear() != null)
			this.birthYear = dineshSinghFootballer.getBirthYear().getClass()
					.getDeclaredConstructor(dineshSinghFootballer.getBirthYear().getClass())
					.newInstance(dineshSinghFootballer.getBirthYear());
		this.characterOffset = dineshSinghFootballer.getCharacterOffset();
		this.characterOnset = dineshSinghFootballer.getCharacterOnset();
		if (dineshSinghFootballer.getPosition() != null)
			this.position = dineshSinghFootballer.getPosition().getClass()
					.getDeclaredConstructor(dineshSinghFootballer.getPosition().getClass())
					.newInstance(dineshSinghFootballer.getPosition());
		for (int j = 0; j < dineshSinghFootballer.getTeams().size(); j++) {
			if (dineshSinghFootballer.getTeams().get(j) != null) {
				teams.add(dineshSinghFootballer.getTeams().get(j).getClass()
						.getDeclaredConstructor(dineshSinghFootballer.getTeams().get(j).getClass())
						.newInstance(dineshSinghFootballer.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DineshSinghFootballer [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
