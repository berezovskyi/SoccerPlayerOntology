...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ICharlesVandenWouwer;
...
public class CharlesVandenWouwer implements ICharlesVandenWouwer {
	...
	public CharlesVandenWouwer(CharlesVandenWouwer charlesVandenWouwer)
			throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
			NoSuchMethodException, SecurityException {
		this.annotationID = charlesVandenWouwer.getAnnotationID();
		for (int j = 0; j < charlesVandenWouwer.getBirthPlaces().size(); j++) {
			if (charlesVandenWouwer.getBirthPlaces().get(j) != null) {
				birthPlaces.add(charlesVandenWouwer.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(charlesVandenWouwer.getBirthPlaces().get(j).getClass())
						.newInstance(charlesVandenWouwer.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (charlesVandenWouwer.getBirthYear() != null)
			this.birthYear = charlesVandenWouwer.getBirthYear().getClass()
					.getDeclaredConstructor(charlesVandenWouwer.getBirthYear().getClass())
					.newInstance(charlesVandenWouwer.getBirthYear());
		this.characterOffset = charlesVandenWouwer.getCharacterOffset();
		this.characterOnset = charlesVandenWouwer.getCharacterOnset();
		if (charlesVandenWouwer.getPosition() != null)
			this.position = charlesVandenWouwer.getPosition().getClass()
					.getDeclaredConstructor(charlesVandenWouwer.getPosition().getClass())
					.newInstance(charlesVandenWouwer.getPosition());
		for (int j = 0; j < charlesVandenWouwer.getTeams().size(); j++) {
			if (charlesVandenWouwer.getTeams().get(j) != null) {
				teams.add(charlesVandenWouwer.getTeams().get(j).getClass()
						.getDeclaredConstructor(charlesVandenWouwer.getTeams().get(j).getClass())
						.newInstance(charlesVandenWouwer.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "CharlesVandenWouwer [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
