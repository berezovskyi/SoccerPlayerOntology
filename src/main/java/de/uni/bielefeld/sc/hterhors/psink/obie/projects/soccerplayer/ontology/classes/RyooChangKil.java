...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRyooChangKil;
...
public class RyooChangKil implements IRyooChangKil {
	...
	public RyooChangKil(RyooChangKil ryooChangKil) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = ryooChangKil.getAnnotationID();
		for (int j = 0; j < ryooChangKil.getBirthPlaces().size(); j++) {
			if (ryooChangKil.getBirthPlaces().get(j) != null) {
				birthPlaces.add(ryooChangKil.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(ryooChangKil.getBirthPlaces().get(j).getClass())
						.newInstance(ryooChangKil.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (ryooChangKil.getBirthYear() != null)
			this.birthYear = ryooChangKil.getBirthYear().getClass()
					.getDeclaredConstructor(ryooChangKil.getBirthYear().getClass())
					.newInstance(ryooChangKil.getBirthYear());
		this.characterOffset = ryooChangKil.getCharacterOffset();
		this.characterOnset = ryooChangKil.getCharacterOnset();
		if (ryooChangKil.getPosition() != null)
			this.position = ryooChangKil.getPosition().getClass()
					.getDeclaredConstructor(ryooChangKil.getPosition().getClass())
					.newInstance(ryooChangKil.getPosition());
		for (int j = 0; j < ryooChangKil.getTeams().size(); j++) {
			if (ryooChangKil.getTeams().get(j) != null) {
				teams.add(ryooChangKil.getTeams().get(j).getClass()
						.getDeclaredConstructor(ryooChangKil.getTeams().get(j).getClass())
						.newInstance(ryooChangKil.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RyooChangKil [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
