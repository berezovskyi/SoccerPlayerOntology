...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IParkSunYong;
...
public class ParkSunYong implements IParkSunYong {
	...
	public ParkSunYong(ParkSunYong parkSunYong) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = parkSunYong.getAnnotationID();
		for (int j = 0; j < parkSunYong.getBirthPlaces().size(); j++) {
			if (parkSunYong.getBirthPlaces().get(j) != null) {
				birthPlaces.add(parkSunYong.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(parkSunYong.getBirthPlaces().get(j).getClass())
						.newInstance(parkSunYong.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (parkSunYong.getBirthYear() != null)
			this.birthYear = parkSunYong.getBirthYear().getClass()
					.getDeclaredConstructor(parkSunYong.getBirthYear().getClass())
					.newInstance(parkSunYong.getBirthYear());
		this.characterOffset = parkSunYong.getCharacterOffset();
		this.characterOnset = parkSunYong.getCharacterOnset();
		if (parkSunYong.getPosition() != null)
			this.position = parkSunYong.getPosition().getClass()
					.getDeclaredConstructor(parkSunYong.getPosition().getClass())
					.newInstance(parkSunYong.getPosition());
		for (int j = 0; j < parkSunYong.getTeams().size(); j++) {
			if (parkSunYong.getTeams().get(j) != null) {
				teams.add(parkSunYong.getTeams().get(j).getClass()
						.getDeclaredConstructor(parkSunYong.getTeams().get(j).getClass())
						.newInstance(parkSunYong.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ParkSunYong [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
