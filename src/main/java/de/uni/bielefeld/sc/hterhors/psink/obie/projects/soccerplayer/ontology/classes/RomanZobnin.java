...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRomanZobnin;
...
public class RomanZobnin implements IRomanZobnin {
	...
	public RomanZobnin(RomanZobnin romanZobnin) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = romanZobnin.getAnnotationID();
		for (int j = 0; j < romanZobnin.getBirthPlaces().size(); j++) {
			if (romanZobnin.getBirthPlaces().get(j) != null) {
				birthPlaces.add(romanZobnin.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(romanZobnin.getBirthPlaces().get(j).getClass())
						.newInstance(romanZobnin.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (romanZobnin.getBirthYear() != null)
			this.birthYear = romanZobnin.getBirthYear().getClass()
					.getDeclaredConstructor(romanZobnin.getBirthYear().getClass())
					.newInstance(romanZobnin.getBirthYear());
		this.characterOffset = romanZobnin.getCharacterOffset();
		this.characterOnset = romanZobnin.getCharacterOnset();
		if (romanZobnin.getPosition() != null)
			this.position = romanZobnin.getPosition().getClass()
					.getDeclaredConstructor(romanZobnin.getPosition().getClass())
					.newInstance(romanZobnin.getPosition());
		for (int j = 0; j < romanZobnin.getTeams().size(); j++) {
			if (romanZobnin.getTeams().get(j) != null) {
				teams.add(romanZobnin.getTeams().get(j).getClass()
						.getDeclaredConstructor(romanZobnin.getTeams().get(j).getClass())
						.newInstance(romanZobnin.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RomanZobnin [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
