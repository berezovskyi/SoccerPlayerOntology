...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAllanDevanney;
...
public class AllanDevanney implements IAllanDevanney {
	...
	public AllanDevanney(AllanDevanney allanDevanney) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = allanDevanney.getAnnotationID();
		for (int j = 0; j < allanDevanney.getBirthPlaces().size(); j++) {
			if (allanDevanney.getBirthPlaces().get(j) != null) {
				birthPlaces.add(allanDevanney.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(allanDevanney.getBirthPlaces().get(j).getClass())
						.newInstance(allanDevanney.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (allanDevanney.getBirthYear() != null)
			this.birthYear = allanDevanney.getBirthYear().getClass()
					.getDeclaredConstructor(allanDevanney.getBirthYear().getClass())
					.newInstance(allanDevanney.getBirthYear());
		this.characterOffset = allanDevanney.getCharacterOffset();
		this.characterOnset = allanDevanney.getCharacterOnset();
		if (allanDevanney.getPosition() != null)
			this.position = allanDevanney.getPosition().getClass()
					.getDeclaredConstructor(allanDevanney.getPosition().getClass())
					.newInstance(allanDevanney.getPosition());
		for (int j = 0; j < allanDevanney.getTeams().size(); j++) {
			if (allanDevanney.getTeams().get(j) != null) {
				teams.add(allanDevanney.getTeams().get(j).getClass()
						.getDeclaredConstructor(allanDevanney.getTeams().get(j).getClass())
						.newInstance(allanDevanney.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AllanDevanney [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
