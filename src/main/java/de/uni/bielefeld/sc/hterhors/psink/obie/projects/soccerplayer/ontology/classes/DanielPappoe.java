...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDanielPappoe;
...
public class DanielPappoe implements IDanielPappoe {
	...
	public DanielPappoe(DanielPappoe danielPappoe) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = danielPappoe.getAnnotationID();
		for (int j = 0; j < danielPappoe.getBirthPlaces().size(); j++) {
			if (danielPappoe.getBirthPlaces().get(j) != null) {
				birthPlaces.add(danielPappoe.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(danielPappoe.getBirthPlaces().get(j).getClass())
						.newInstance(danielPappoe.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (danielPappoe.getBirthYear() != null)
			this.birthYear = danielPappoe.getBirthYear().getClass()
					.getDeclaredConstructor(danielPappoe.getBirthYear().getClass())
					.newInstance(danielPappoe.getBirthYear());
		this.characterOffset = danielPappoe.getCharacterOffset();
		this.characterOnset = danielPappoe.getCharacterOnset();
		if (danielPappoe.getPosition() != null)
			this.position = danielPappoe.getPosition().getClass()
					.getDeclaredConstructor(danielPappoe.getPosition().getClass())
					.newInstance(danielPappoe.getPosition());
		for (int j = 0; j < danielPappoe.getTeams().size(); j++) {
			if (danielPappoe.getTeams().get(j) != null) {
				teams.add(danielPappoe.getTeams().get(j).getClass()
						.getDeclaredConstructor(danielPappoe.getTeams().get(j).getClass())
						.newInstance(danielPappoe.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DanielPappoe [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
