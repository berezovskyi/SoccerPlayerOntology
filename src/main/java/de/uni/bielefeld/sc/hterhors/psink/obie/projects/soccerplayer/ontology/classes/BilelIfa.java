...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBilelIfa;
...
public class BilelIfa implements IBilelIfa {
	...
	public BilelIfa(BilelIfa bilelIfa) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = bilelIfa.getAnnotationID();
		for (int j = 0; j < bilelIfa.getBirthPlaces().size(); j++) {
			if (bilelIfa.getBirthPlaces().get(j) != null) {
				birthPlaces.add(bilelIfa.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(bilelIfa.getBirthPlaces().get(j).getClass())
						.newInstance(bilelIfa.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (bilelIfa.getBirthYear() != null)
			this.birthYear = bilelIfa.getBirthYear().getClass()
					.getDeclaredConstructor(bilelIfa.getBirthYear().getClass()).newInstance(bilelIfa.getBirthYear());
		this.characterOffset = bilelIfa.getCharacterOffset();
		this.characterOnset = bilelIfa.getCharacterOnset();
		if (bilelIfa.getPosition() != null)
			this.position = bilelIfa.getPosition().getClass().getDeclaredConstructor(bilelIfa.getPosition().getClass())
					.newInstance(bilelIfa.getPosition());
		for (int j = 0; j < bilelIfa.getTeams().size(); j++) {
			if (bilelIfa.getTeams().get(j) != null) {
				teams.add(bilelIfa.getTeams().get(j).getClass()
						.getDeclaredConstructor(bilelIfa.getTeams().get(j).getClass())
						.newInstance(bilelIfa.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BilelIfa [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
