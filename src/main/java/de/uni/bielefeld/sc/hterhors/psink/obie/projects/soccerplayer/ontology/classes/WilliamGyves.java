...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IWilliamGyves;
...
public class WilliamGyves implements IWilliamGyves {
	...
	public WilliamGyves(WilliamGyves williamGyves) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = williamGyves.getAnnotationID();
		for (int j = 0; j < williamGyves.getBirthPlaces().size(); j++) {
			if (williamGyves.getBirthPlaces().get(j) != null) {
				birthPlaces.add(williamGyves.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(williamGyves.getBirthPlaces().get(j).getClass())
						.newInstance(williamGyves.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (williamGyves.getBirthYear() != null)
			this.birthYear = williamGyves.getBirthYear().getClass()
					.getDeclaredConstructor(williamGyves.getBirthYear().getClass())
					.newInstance(williamGyves.getBirthYear());
		this.characterOffset = williamGyves.getCharacterOffset();
		this.characterOnset = williamGyves.getCharacterOnset();
		if (williamGyves.getPosition() != null)
			this.position = williamGyves.getPosition().getClass()
					.getDeclaredConstructor(williamGyves.getPosition().getClass())
					.newInstance(williamGyves.getPosition());
		for (int j = 0; j < williamGyves.getTeams().size(); j++) {
			if (williamGyves.getTeams().get(j) != null) {
				teams.add(williamGyves.getTeams().get(j).getClass()
						.getDeclaredConstructor(williamGyves.getTeams().get(j).getClass())
						.newInstance(williamGyves.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "WilliamGyves [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
