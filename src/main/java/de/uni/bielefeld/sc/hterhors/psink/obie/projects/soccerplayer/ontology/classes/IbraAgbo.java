...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IIbraAgbo;
...
public class IbraAgbo implements IIbraAgbo {
	...
	public IbraAgbo(IbraAgbo ibraAgbo) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = ibraAgbo.getAnnotationID();
		for (int j = 0; j < ibraAgbo.getBirthPlaces().size(); j++) {
			if (ibraAgbo.getBirthPlaces().get(j) != null) {
				birthPlaces.add(ibraAgbo.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(ibraAgbo.getBirthPlaces().get(j).getClass())
						.newInstance(ibraAgbo.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (ibraAgbo.getBirthYear() != null)
			this.birthYear = ibraAgbo.getBirthYear().getClass()
					.getDeclaredConstructor(ibraAgbo.getBirthYear().getClass()).newInstance(ibraAgbo.getBirthYear());
		this.characterOffset = ibraAgbo.getCharacterOffset();
		this.characterOnset = ibraAgbo.getCharacterOnset();
		if (ibraAgbo.getPosition() != null)
			this.position = ibraAgbo.getPosition().getClass().getDeclaredConstructor(ibraAgbo.getPosition().getClass())
					.newInstance(ibraAgbo.getPosition());
		for (int j = 0; j < ibraAgbo.getTeams().size(); j++) {
			if (ibraAgbo.getTeams().get(j) != null) {
				teams.add(ibraAgbo.getTeams().get(j).getClass()
						.getDeclaredConstructor(ibraAgbo.getTeams().get(j).getClass())
						.newInstance(ibraAgbo.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "IbraAgbo [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
