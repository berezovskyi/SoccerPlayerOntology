...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IArthurCowell;
...
public class ArthurCowell implements IArthurCowell {
	...
	public ArthurCowell(ArthurCowell arthurCowell) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = arthurCowell.getAnnotationID();
		for (int j = 0; j < arthurCowell.getBirthPlaces().size(); j++) {
			if (arthurCowell.getBirthPlaces().get(j) != null) {
				birthPlaces.add(arthurCowell.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(arthurCowell.getBirthPlaces().get(j).getClass())
						.newInstance(arthurCowell.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (arthurCowell.getBirthYear() != null)
			this.birthYear = arthurCowell.getBirthYear().getClass()
					.getDeclaredConstructor(arthurCowell.getBirthYear().getClass())
					.newInstance(arthurCowell.getBirthYear());
		this.characterOffset = arthurCowell.getCharacterOffset();
		this.characterOnset = arthurCowell.getCharacterOnset();
		if (arthurCowell.getPosition() != null)
			this.position = arthurCowell.getPosition().getClass()
					.getDeclaredConstructor(arthurCowell.getPosition().getClass())
					.newInstance(arthurCowell.getPosition());
		for (int j = 0; j < arthurCowell.getTeams().size(); j++) {
			if (arthurCowell.getTeams().get(j) != null) {
				teams.add(arthurCowell.getTeams().get(j).getClass()
						.getDeclaredConstructor(arthurCowell.getTeams().get(j).getClass())
						.newInstance(arthurCowell.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ArthurCowell [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
