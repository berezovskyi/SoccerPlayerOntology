...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRudyGetzinger;
...
public class RudyGetzinger implements IRudyGetzinger {
	...
	public RudyGetzinger(RudyGetzinger rudyGetzinger) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = rudyGetzinger.getAnnotationID();
		for (int j = 0; j < rudyGetzinger.getBirthPlaces().size(); j++) {
			if (rudyGetzinger.getBirthPlaces().get(j) != null) {
				birthPlaces.add(rudyGetzinger.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(rudyGetzinger.getBirthPlaces().get(j).getClass())
						.newInstance(rudyGetzinger.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (rudyGetzinger.getBirthYear() != null)
			this.birthYear = rudyGetzinger.getBirthYear().getClass()
					.getDeclaredConstructor(rudyGetzinger.getBirthYear().getClass())
					.newInstance(rudyGetzinger.getBirthYear());
		this.characterOffset = rudyGetzinger.getCharacterOffset();
		this.characterOnset = rudyGetzinger.getCharacterOnset();
		if (rudyGetzinger.getPosition() != null)
			this.position = rudyGetzinger.getPosition().getClass()
					.getDeclaredConstructor(rudyGetzinger.getPosition().getClass())
					.newInstance(rudyGetzinger.getPosition());
		for (int j = 0; j < rudyGetzinger.getTeams().size(); j++) {
			if (rudyGetzinger.getTeams().get(j) != null) {
				teams.add(rudyGetzinger.getTeams().get(j).getClass()
						.getDeclaredConstructor(rudyGetzinger.getTeams().get(j).getClass())
						.newInstance(rudyGetzinger.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RudyGetzinger [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
