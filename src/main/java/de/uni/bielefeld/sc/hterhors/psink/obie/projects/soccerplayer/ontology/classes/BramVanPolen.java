...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBramVanPolen;
...
public class BramVanPolen implements IBramVanPolen {
	...
	public BramVanPolen(BramVanPolen bramVanPolen) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = bramVanPolen.getAnnotationID();
		for (int j = 0; j < bramVanPolen.getBirthPlaces().size(); j++) {
			if (bramVanPolen.getBirthPlaces().get(j) != null) {
				birthPlaces.add(bramVanPolen.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(bramVanPolen.getBirthPlaces().get(j).getClass())
						.newInstance(bramVanPolen.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (bramVanPolen.getBirthYear() != null)
			this.birthYear = bramVanPolen.getBirthYear().getClass()
					.getDeclaredConstructor(bramVanPolen.getBirthYear().getClass())
					.newInstance(bramVanPolen.getBirthYear());
		this.characterOffset = bramVanPolen.getCharacterOffset();
		this.characterOnset = bramVanPolen.getCharacterOnset();
		if (bramVanPolen.getPosition() != null)
			this.position = bramVanPolen.getPosition().getClass()
					.getDeclaredConstructor(bramVanPolen.getPosition().getClass())
					.newInstance(bramVanPolen.getPosition());
		for (int j = 0; j < bramVanPolen.getTeams().size(); j++) {
			if (bramVanPolen.getTeams().get(j) != null) {
				teams.add(bramVanPolen.getTeams().get(j).getClass()
						.getDeclaredConstructor(bramVanPolen.getTeams().get(j).getClass())
						.newInstance(bramVanPolen.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BramVanPolen [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
