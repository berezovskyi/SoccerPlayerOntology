...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILeonTol;
...
public class LeonTol implements ILeonTol {
	...
	public LeonTol(LeonTol leonTol) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = leonTol.getAnnotationID();
		for (int j = 0; j < leonTol.getBirthPlaces().size(); j++) {
			if (leonTol.getBirthPlaces().get(j) != null) {
				birthPlaces.add(leonTol.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(leonTol.getBirthPlaces().get(j).getClass())
						.newInstance(leonTol.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (leonTol.getBirthYear() != null)
			this.birthYear = leonTol.getBirthYear().getClass().getDeclaredConstructor(leonTol.getBirthYear().getClass())
					.newInstance(leonTol.getBirthYear());
		this.characterOffset = leonTol.getCharacterOffset();
		this.characterOnset = leonTol.getCharacterOnset();
		if (leonTol.getPosition() != null)
			this.position = leonTol.getPosition().getClass().getDeclaredConstructor(leonTol.getPosition().getClass())
					.newInstance(leonTol.getPosition());
		for (int j = 0; j < leonTol.getTeams().size(); j++) {
			if (leonTol.getTeams().get(j) != null) {
				teams.add(leonTol.getTeams().get(j).getClass()
						.getDeclaredConstructor(leonTol.getTeams().get(j).getClass())
						.newInstance(leonTol.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LeonTol [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
