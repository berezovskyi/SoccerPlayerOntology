...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJohnCoxFootballer;
...
public class JohnCoxFootballer implements IJohnCoxFootballer {
	...
	public JohnCoxFootballer(JohnCoxFootballer johnCoxFootballer) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = johnCoxFootballer.getAnnotationID();
		for (int j = 0; j < johnCoxFootballer.getBirthPlaces().size(); j++) {
			if (johnCoxFootballer.getBirthPlaces().get(j) != null) {
				birthPlaces.add(johnCoxFootballer.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(johnCoxFootballer.getBirthPlaces().get(j).getClass())
						.newInstance(johnCoxFootballer.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (johnCoxFootballer.getBirthYear() != null)
			this.birthYear = johnCoxFootballer.getBirthYear().getClass()
					.getDeclaredConstructor(johnCoxFootballer.getBirthYear().getClass())
					.newInstance(johnCoxFootballer.getBirthYear());
		this.characterOffset = johnCoxFootballer.getCharacterOffset();
		this.characterOnset = johnCoxFootballer.getCharacterOnset();
		if (johnCoxFootballer.getPosition() != null)
			this.position = johnCoxFootballer.getPosition().getClass()
					.getDeclaredConstructor(johnCoxFootballer.getPosition().getClass())
					.newInstance(johnCoxFootballer.getPosition());
		for (int j = 0; j < johnCoxFootballer.getTeams().size(); j++) {
			if (johnCoxFootballer.getTeams().get(j) != null) {
				teams.add(johnCoxFootballer.getTeams().get(j).getClass()
						.getDeclaredConstructor(johnCoxFootballer.getTeams().get(j).getClass())
						.newInstance(johnCoxFootballer.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JohnCoxFootballer [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
