...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IFredFurniss;
...
public class FredFurniss implements IFredFurniss {
	...
	public FredFurniss(FredFurniss fredFurniss) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = fredFurniss.getAnnotationID();
		for (int j = 0; j < fredFurniss.getBirthPlaces().size(); j++) {
			if (fredFurniss.getBirthPlaces().get(j) != null) {
				birthPlaces.add(fredFurniss.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(fredFurniss.getBirthPlaces().get(j).getClass())
						.newInstance(fredFurniss.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (fredFurniss.getBirthYear() != null)
			this.birthYear = fredFurniss.getBirthYear().getClass()
					.getDeclaredConstructor(fredFurniss.getBirthYear().getClass())
					.newInstance(fredFurniss.getBirthYear());
		this.characterOffset = fredFurniss.getCharacterOffset();
		this.characterOnset = fredFurniss.getCharacterOnset();
		if (fredFurniss.getPosition() != null)
			this.position = fredFurniss.getPosition().getClass()
					.getDeclaredConstructor(fredFurniss.getPosition().getClass())
					.newInstance(fredFurniss.getPosition());
		for (int j = 0; j < fredFurniss.getTeams().size(); j++) {
			if (fredFurniss.getTeams().get(j) != null) {
				teams.add(fredFurniss.getTeams().get(j).getClass()
						.getDeclaredConstructor(fredFurniss.getTeams().get(j).getClass())
						.newInstance(fredFurniss.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "FredFurniss [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
