...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IWaleedAlJasem;
...
public class WaleedAlJasem implements IWaleedAlJasem {
	...
	public WaleedAlJasem(WaleedAlJasem waleedAlJasem) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = waleedAlJasem.getAnnotationID();
		for (int j = 0; j < waleedAlJasem.getBirthPlaces().size(); j++) {
			if (waleedAlJasem.getBirthPlaces().get(j) != null) {
				birthPlaces.add(waleedAlJasem.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(waleedAlJasem.getBirthPlaces().get(j).getClass())
						.newInstance(waleedAlJasem.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (waleedAlJasem.getBirthYear() != null)
			this.birthYear = waleedAlJasem.getBirthYear().getClass()
					.getDeclaredConstructor(waleedAlJasem.getBirthYear().getClass())
					.newInstance(waleedAlJasem.getBirthYear());
		this.characterOffset = waleedAlJasem.getCharacterOffset();
		this.characterOnset = waleedAlJasem.getCharacterOnset();
		if (waleedAlJasem.getPosition() != null)
			this.position = waleedAlJasem.getPosition().getClass()
					.getDeclaredConstructor(waleedAlJasem.getPosition().getClass())
					.newInstance(waleedAlJasem.getPosition());
		for (int j = 0; j < waleedAlJasem.getTeams().size(); j++) {
			if (waleedAlJasem.getTeams().get(j) != null) {
				teams.add(waleedAlJasem.getTeams().get(j).getClass()
						.getDeclaredConstructor(waleedAlJasem.getTeams().get(j).getClass())
						.newInstance(waleedAlJasem.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "WaleedAlJasem [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
