...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMarioMedina;
...
public class MarioMedina implements IMarioMedina {
	...
	public MarioMedina(MarioMedina marioMedina) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = marioMedina.getAnnotationID();
		for (int j = 0; j < marioMedina.getBirthPlaces().size(); j++) {
			if (marioMedina.getBirthPlaces().get(j) != null) {
				birthPlaces.add(marioMedina.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(marioMedina.getBirthPlaces().get(j).getClass())
						.newInstance(marioMedina.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (marioMedina.getBirthYear() != null)
			this.birthYear = marioMedina.getBirthYear().getClass()
					.getDeclaredConstructor(marioMedina.getBirthYear().getClass())
					.newInstance(marioMedina.getBirthYear());
		this.characterOffset = marioMedina.getCharacterOffset();
		this.characterOnset = marioMedina.getCharacterOnset();
		if (marioMedina.getPosition() != null)
			this.position = marioMedina.getPosition().getClass()
					.getDeclaredConstructor(marioMedina.getPosition().getClass())
					.newInstance(marioMedina.getPosition());
		for (int j = 0; j < marioMedina.getTeams().size(); j++) {
			if (marioMedina.getTeams().get(j) != null) {
				teams.add(marioMedina.getTeams().get(j).getClass()
						.getDeclaredConstructor(marioMedina.getTeams().get(j).getClass())
						.newInstance(marioMedina.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MarioMedina [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
