...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILarbiElHadi;
...
public class LarbiElHadi implements ILarbiElHadi {
	...
	public LarbiElHadi(LarbiElHadi larbiElHadi) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = larbiElHadi.getAnnotationID();
		for (int j = 0; j < larbiElHadi.getBirthPlaces().size(); j++) {
			if (larbiElHadi.getBirthPlaces().get(j) != null) {
				birthPlaces.add(larbiElHadi.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(larbiElHadi.getBirthPlaces().get(j).getClass())
						.newInstance(larbiElHadi.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (larbiElHadi.getBirthYear() != null)
			this.birthYear = larbiElHadi.getBirthYear().getClass()
					.getDeclaredConstructor(larbiElHadi.getBirthYear().getClass())
					.newInstance(larbiElHadi.getBirthYear());
		this.characterOffset = larbiElHadi.getCharacterOffset();
		this.characterOnset = larbiElHadi.getCharacterOnset();
		if (larbiElHadi.getPosition() != null)
			this.position = larbiElHadi.getPosition().getClass()
					.getDeclaredConstructor(larbiElHadi.getPosition().getClass())
					.newInstance(larbiElHadi.getPosition());
		for (int j = 0; j < larbiElHadi.getTeams().size(); j++) {
			if (larbiElHadi.getTeams().get(j) != null) {
				teams.add(larbiElHadi.getTeams().get(j).getClass()
						.getDeclaredConstructor(larbiElHadi.getTeams().get(j).getClass())
						.newInstance(larbiElHadi.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LarbiElHadi [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
