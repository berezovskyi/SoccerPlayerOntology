...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRobertoLeopardi;
...
public class RobertoLeopardi implements IRobertoLeopardi {
	...
	public RobertoLeopardi(RobertoLeopardi robertoLeopardi) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = robertoLeopardi.getAnnotationID();
		for (int j = 0; j < robertoLeopardi.getBirthPlaces().size(); j++) {
			if (robertoLeopardi.getBirthPlaces().get(j) != null) {
				birthPlaces.add(robertoLeopardi.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(robertoLeopardi.getBirthPlaces().get(j).getClass())
						.newInstance(robertoLeopardi.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (robertoLeopardi.getBirthYear() != null)
			this.birthYear = robertoLeopardi.getBirthYear().getClass()
					.getDeclaredConstructor(robertoLeopardi.getBirthYear().getClass())
					.newInstance(robertoLeopardi.getBirthYear());
		this.characterOffset = robertoLeopardi.getCharacterOffset();
		this.characterOnset = robertoLeopardi.getCharacterOnset();
		if (robertoLeopardi.getPosition() != null)
			this.position = robertoLeopardi.getPosition().getClass()
					.getDeclaredConstructor(robertoLeopardi.getPosition().getClass())
					.newInstance(robertoLeopardi.getPosition());
		for (int j = 0; j < robertoLeopardi.getTeams().size(); j++) {
			if (robertoLeopardi.getTeams().get(j) != null) {
				teams.add(robertoLeopardi.getTeams().get(j).getClass()
						.getDeclaredConstructor(robertoLeopardi.getTeams().get(j).getClass())
						.newInstance(robertoLeopardi.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RobertoLeopardi [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
