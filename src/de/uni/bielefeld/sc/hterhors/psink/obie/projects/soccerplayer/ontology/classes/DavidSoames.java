...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDavidSoames;
...
public class DavidSoames implements IDavidSoames {
	...
	public DavidSoames(DavidSoames davidSoames) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = davidSoames.getAnnotationID();
		for (int j = 0; j < davidSoames.getBirthPlaces().size(); j++) {
			if (davidSoames.getBirthPlaces().get(j) != null) {
				birthPlaces.add(davidSoames.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(davidSoames.getBirthPlaces().get(j).getClass())
						.newInstance(davidSoames.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (davidSoames.getBirthYear() != null)
			this.birthYear = davidSoames.getBirthYear().getClass()
					.getDeclaredConstructor(davidSoames.getBirthYear().getClass())
					.newInstance(davidSoames.getBirthYear());
		this.characterOffset = davidSoames.getCharacterOffset();
		this.characterOnset = davidSoames.getCharacterOnset();
		if (davidSoames.getPosition() != null)
			this.position = davidSoames.getPosition().getClass()
					.getDeclaredConstructor(davidSoames.getPosition().getClass())
					.newInstance(davidSoames.getPosition());
		for (int j = 0; j < davidSoames.getTeams().size(); j++) {
			if (davidSoames.getTeams().get(j) != null) {
				teams.add(davidSoames.getTeams().get(j).getClass()
						.getDeclaredConstructor(davidSoames.getTeams().get(j).getClass())
						.newInstance(davidSoames.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DavidSoames [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
