...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ICameronLancaster;
...
public class CameronLancaster implements ICameronLancaster {
	...
	public CameronLancaster(CameronLancaster cameronLancaster) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = cameronLancaster.getAnnotationID();
		for (int j = 0; j < cameronLancaster.getBirthPlaces().size(); j++) {
			if (cameronLancaster.getBirthPlaces().get(j) != null) {
				birthPlaces.add(cameronLancaster.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(cameronLancaster.getBirthPlaces().get(j).getClass())
						.newInstance(cameronLancaster.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (cameronLancaster.getBirthYear() != null)
			this.birthYear = cameronLancaster.getBirthYear().getClass()
					.getDeclaredConstructor(cameronLancaster.getBirthYear().getClass())
					.newInstance(cameronLancaster.getBirthYear());
		this.characterOffset = cameronLancaster.getCharacterOffset();
		this.characterOnset = cameronLancaster.getCharacterOnset();
		if (cameronLancaster.getPosition() != null)
			this.position = cameronLancaster.getPosition().getClass()
					.getDeclaredConstructor(cameronLancaster.getPosition().getClass())
					.newInstance(cameronLancaster.getPosition());
		for (int j = 0; j < cameronLancaster.getTeams().size(); j++) {
			if (cameronLancaster.getTeams().get(j) != null) {
				teams.add(cameronLancaster.getTeams().get(j).getClass()
						.getDeclaredConstructor(cameronLancaster.getTeams().get(j).getClass())
						.newInstance(cameronLancaster.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "CameronLancaster [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
