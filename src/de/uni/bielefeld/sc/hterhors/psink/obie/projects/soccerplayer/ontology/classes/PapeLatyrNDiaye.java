...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IPapeLatyrNDiaye;
...
public class PapeLatyrNDiaye implements IPapeLatyrNDiaye {
	...
	public PapeLatyrNDiaye(PapeLatyrNDiaye papeLatyrNDiaye) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = papeLatyrNDiaye.getAnnotationID();
		for (int j = 0; j < papeLatyrNDiaye.getBirthPlaces().size(); j++) {
			if (papeLatyrNDiaye.getBirthPlaces().get(j) != null) {
				birthPlaces.add(papeLatyrNDiaye.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(papeLatyrNDiaye.getBirthPlaces().get(j).getClass())
						.newInstance(papeLatyrNDiaye.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (papeLatyrNDiaye.getBirthYear() != null)
			this.birthYear = papeLatyrNDiaye.getBirthYear().getClass()
					.getDeclaredConstructor(papeLatyrNDiaye.getBirthYear().getClass())
					.newInstance(papeLatyrNDiaye.getBirthYear());
		this.characterOffset = papeLatyrNDiaye.getCharacterOffset();
		this.characterOnset = papeLatyrNDiaye.getCharacterOnset();
		if (papeLatyrNDiaye.getPosition() != null)
			this.position = papeLatyrNDiaye.getPosition().getClass()
					.getDeclaredConstructor(papeLatyrNDiaye.getPosition().getClass())
					.newInstance(papeLatyrNDiaye.getPosition());
		for (int j = 0; j < papeLatyrNDiaye.getTeams().size(); j++) {
			if (papeLatyrNDiaye.getTeams().get(j) != null) {
				teams.add(papeLatyrNDiaye.getTeams().get(j).getClass()
						.getDeclaredConstructor(papeLatyrNDiaye.getTeams().get(j).getClass())
						.newInstance(papeLatyrNDiaye.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "PapeLatyrNDiaye [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
