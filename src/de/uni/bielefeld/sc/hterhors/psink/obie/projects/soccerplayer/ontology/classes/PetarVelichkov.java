...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IPetarVelichkov;
...
public class PetarVelichkov implements IPetarVelichkov {
	...
	public PetarVelichkov(PetarVelichkov petarVelichkov) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = petarVelichkov.getAnnotationID();
		for (int j = 0; j < petarVelichkov.getBirthPlaces().size(); j++) {
			if (petarVelichkov.getBirthPlaces().get(j) != null) {
				birthPlaces.add(petarVelichkov.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(petarVelichkov.getBirthPlaces().get(j).getClass())
						.newInstance(petarVelichkov.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (petarVelichkov.getBirthYear() != null)
			this.birthYear = petarVelichkov.getBirthYear().getClass()
					.getDeclaredConstructor(petarVelichkov.getBirthYear().getClass())
					.newInstance(petarVelichkov.getBirthYear());
		this.characterOffset = petarVelichkov.getCharacterOffset();
		this.characterOnset = petarVelichkov.getCharacterOnset();
		if (petarVelichkov.getPosition() != null)
			this.position = petarVelichkov.getPosition().getClass()
					.getDeclaredConstructor(petarVelichkov.getPosition().getClass())
					.newInstance(petarVelichkov.getPosition());
		for (int j = 0; j < petarVelichkov.getTeams().size(); j++) {
			if (petarVelichkov.getTeams().get(j) != null) {
				teams.add(petarVelichkov.getTeams().get(j).getClass()
						.getDeclaredConstructor(petarVelichkov.getTeams().get(j).getClass())
						.newInstance(petarVelichkov.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "PetarVelichkov [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
