...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDanielMudau;
...
public class DanielMudau implements IDanielMudau {
	...
	public DanielMudau(DanielMudau danielMudau) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = danielMudau.getAnnotationID();
		for (int j = 0; j < danielMudau.getBirthPlaces().size(); j++) {
			if (danielMudau.getBirthPlaces().get(j) != null) {
				birthPlaces.add(danielMudau.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(danielMudau.getBirthPlaces().get(j).getClass())
						.newInstance(danielMudau.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (danielMudau.getBirthYear() != null)
			this.birthYear = danielMudau.getBirthYear().getClass()
					.getDeclaredConstructor(danielMudau.getBirthYear().getClass())
					.newInstance(danielMudau.getBirthYear());
		this.characterOffset = danielMudau.getCharacterOffset();
		this.characterOnset = danielMudau.getCharacterOnset();
		if (danielMudau.getPosition() != null)
			this.position = danielMudau.getPosition().getClass()
					.getDeclaredConstructor(danielMudau.getPosition().getClass())
					.newInstance(danielMudau.getPosition());
		for (int j = 0; j < danielMudau.getTeams().size(); j++) {
			if (danielMudau.getTeams().get(j) != null) {
				teams.add(danielMudau.getTeams().get(j).getClass()
						.getDeclaredConstructor(danielMudau.getTeams().get(j).getClass())
						.newInstance(danielMudau.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DanielMudau [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
