...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IErnestMilton;
...
public class ErnestMilton implements IErnestMilton {
	...
	public ErnestMilton(ErnestMilton ernestMilton) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = ernestMilton.getAnnotationID();
		for (int j = 0; j < ernestMilton.getBirthPlaces().size(); j++) {
			if (ernestMilton.getBirthPlaces().get(j) != null) {
				birthPlaces.add(ernestMilton.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(ernestMilton.getBirthPlaces().get(j).getClass())
						.newInstance(ernestMilton.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (ernestMilton.getBirthYear() != null)
			this.birthYear = ernestMilton.getBirthYear().getClass()
					.getDeclaredConstructor(ernestMilton.getBirthYear().getClass())
					.newInstance(ernestMilton.getBirthYear());
		this.characterOffset = ernestMilton.getCharacterOffset();
		this.characterOnset = ernestMilton.getCharacterOnset();
		if (ernestMilton.getPosition() != null)
			this.position = ernestMilton.getPosition().getClass()
					.getDeclaredConstructor(ernestMilton.getPosition().getClass())
					.newInstance(ernestMilton.getPosition());
		for (int j = 0; j < ernestMilton.getTeams().size(); j++) {
			if (ernestMilton.getTeams().get(j) != null) {
				teams.add(ernestMilton.getTeams().get(j).getClass()
						.getDeclaredConstructor(ernestMilton.getTeams().get(j).getClass())
						.newInstance(ernestMilton.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ErnestMilton [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
