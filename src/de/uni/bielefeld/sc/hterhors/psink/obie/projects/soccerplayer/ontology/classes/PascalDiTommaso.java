...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IPascalDiTommaso;
...
public class PascalDiTommaso implements IPascalDiTommaso {
	...
	public PascalDiTommaso(PascalDiTommaso pascalDiTommaso) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = pascalDiTommaso.getAnnotationID();
		for (int j = 0; j < pascalDiTommaso.getBirthPlaces().size(); j++) {
			if (pascalDiTommaso.getBirthPlaces().get(j) != null) {
				birthPlaces.add(pascalDiTommaso.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(pascalDiTommaso.getBirthPlaces().get(j).getClass())
						.newInstance(pascalDiTommaso.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (pascalDiTommaso.getBirthYear() != null)
			this.birthYear = pascalDiTommaso.getBirthYear().getClass()
					.getDeclaredConstructor(pascalDiTommaso.getBirthYear().getClass())
					.newInstance(pascalDiTommaso.getBirthYear());
		this.characterOffset = pascalDiTommaso.getCharacterOffset();
		this.characterOnset = pascalDiTommaso.getCharacterOnset();
		if (pascalDiTommaso.getPosition() != null)
			this.position = pascalDiTommaso.getPosition().getClass()
					.getDeclaredConstructor(pascalDiTommaso.getPosition().getClass())
					.newInstance(pascalDiTommaso.getPosition());
		for (int j = 0; j < pascalDiTommaso.getTeams().size(); j++) {
			if (pascalDiTommaso.getTeams().get(j) != null) {
				teams.add(pascalDiTommaso.getTeams().get(j).getClass()
						.getDeclaredConstructor(pascalDiTommaso.getTeams().get(j).getClass())
						.newInstance(pascalDiTommaso.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "PascalDiTommaso [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
