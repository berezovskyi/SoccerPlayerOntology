...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IHugoLepe;
...
public class HugoLepe implements IHugoLepe {
	...
	public HugoLepe(HugoLepe hugoLepe) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = hugoLepe.getAnnotationID();
		for (int j = 0; j < hugoLepe.getBirthPlaces().size(); j++) {
			if (hugoLepe.getBirthPlaces().get(j) != null) {
				birthPlaces.add(hugoLepe.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(hugoLepe.getBirthPlaces().get(j).getClass())
						.newInstance(hugoLepe.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (hugoLepe.getBirthYear() != null)
			this.birthYear = hugoLepe.getBirthYear().getClass()
					.getDeclaredConstructor(hugoLepe.getBirthYear().getClass()).newInstance(hugoLepe.getBirthYear());
		this.characterOffset = hugoLepe.getCharacterOffset();
		this.characterOnset = hugoLepe.getCharacterOnset();
		if (hugoLepe.getPosition() != null)
			this.position = hugoLepe.getPosition().getClass().getDeclaredConstructor(hugoLepe.getPosition().getClass())
					.newInstance(hugoLepe.getPosition());
		for (int j = 0; j < hugoLepe.getTeams().size(); j++) {
			if (hugoLepe.getTeams().get(j) != null) {
				teams.add(hugoLepe.getTeams().get(j).getClass()
						.getDeclaredConstructor(hugoLepe.getTeams().get(j).getClass())
						.newInstance(hugoLepe.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "HugoLepe [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
