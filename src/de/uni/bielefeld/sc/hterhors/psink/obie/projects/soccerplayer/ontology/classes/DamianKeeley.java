...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDamianKeeley;
...
public class DamianKeeley implements IDamianKeeley {
	...
	public DamianKeeley(DamianKeeley damianKeeley) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = damianKeeley.getAnnotationID();
		for (int j = 0; j < damianKeeley.getBirthPlaces().size(); j++) {
			if (damianKeeley.getBirthPlaces().get(j) != null) {
				birthPlaces.add(damianKeeley.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(damianKeeley.getBirthPlaces().get(j).getClass())
						.newInstance(damianKeeley.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (damianKeeley.getBirthYear() != null)
			this.birthYear = damianKeeley.getBirthYear().getClass()
					.getDeclaredConstructor(damianKeeley.getBirthYear().getClass())
					.newInstance(damianKeeley.getBirthYear());
		this.characterOffset = damianKeeley.getCharacterOffset();
		this.characterOnset = damianKeeley.getCharacterOnset();
		if (damianKeeley.getPosition() != null)
			this.position = damianKeeley.getPosition().getClass()
					.getDeclaredConstructor(damianKeeley.getPosition().getClass())
					.newInstance(damianKeeley.getPosition());
		for (int j = 0; j < damianKeeley.getTeams().size(); j++) {
			if (damianKeeley.getTeams().get(j) != null) {
				teams.add(damianKeeley.getTeams().get(j).getClass()
						.getDeclaredConstructor(damianKeeley.getTeams().get(j).getClass())
						.newInstance(damianKeeley.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DamianKeeley [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
