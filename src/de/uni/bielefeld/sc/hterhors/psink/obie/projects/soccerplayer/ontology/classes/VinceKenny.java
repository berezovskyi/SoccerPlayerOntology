...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IVinceKenny;
...
public class VinceKenny implements IVinceKenny {
	...
	public VinceKenny(VinceKenny vinceKenny) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = vinceKenny.getAnnotationID();
		for (int j = 0; j < vinceKenny.getBirthPlaces().size(); j++) {
			if (vinceKenny.getBirthPlaces().get(j) != null) {
				birthPlaces.add(vinceKenny.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(vinceKenny.getBirthPlaces().get(j).getClass())
						.newInstance(vinceKenny.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (vinceKenny.getBirthYear() != null)
			this.birthYear = vinceKenny.getBirthYear().getClass()
					.getDeclaredConstructor(vinceKenny.getBirthYear().getClass())
					.newInstance(vinceKenny.getBirthYear());
		this.characterOffset = vinceKenny.getCharacterOffset();
		this.characterOnset = vinceKenny.getCharacterOnset();
		if (vinceKenny.getPosition() != null)
			this.position = vinceKenny.getPosition().getClass()
					.getDeclaredConstructor(vinceKenny.getPosition().getClass()).newInstance(vinceKenny.getPosition());
		for (int j = 0; j < vinceKenny.getTeams().size(); j++) {
			if (vinceKenny.getTeams().get(j) != null) {
				teams.add(vinceKenny.getTeams().get(j).getClass()
						.getDeclaredConstructor(vinceKenny.getTeams().get(j).getClass())
						.newInstance(vinceKenny.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "VinceKenny [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
