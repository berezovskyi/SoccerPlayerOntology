...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IQuiterioOlmedo;
...
public class QuiterioOlmedo implements IQuiterioOlmedo {
	...
	public QuiterioOlmedo(QuiterioOlmedo quiterioOlmedo) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = quiterioOlmedo.getAnnotationID();
		for (int j = 0; j < quiterioOlmedo.getBirthPlaces().size(); j++) {
			if (quiterioOlmedo.getBirthPlaces().get(j) != null) {
				birthPlaces.add(quiterioOlmedo.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(quiterioOlmedo.getBirthPlaces().get(j).getClass())
						.newInstance(quiterioOlmedo.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (quiterioOlmedo.getBirthYear() != null)
			this.birthYear = quiterioOlmedo.getBirthYear().getClass()
					.getDeclaredConstructor(quiterioOlmedo.getBirthYear().getClass())
					.newInstance(quiterioOlmedo.getBirthYear());
		this.characterOffset = quiterioOlmedo.getCharacterOffset();
		this.characterOnset = quiterioOlmedo.getCharacterOnset();
		if (quiterioOlmedo.getPosition() != null)
			this.position = quiterioOlmedo.getPosition().getClass()
					.getDeclaredConstructor(quiterioOlmedo.getPosition().getClass())
					.newInstance(quiterioOlmedo.getPosition());
		for (int j = 0; j < quiterioOlmedo.getTeams().size(); j++) {
			if (quiterioOlmedo.getTeams().get(j) != null) {
				teams.add(quiterioOlmedo.getTeams().get(j).getClass()
						.getDeclaredConstructor(quiterioOlmedo.getTeams().get(j).getClass())
						.newInstance(quiterioOlmedo.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "QuiterioOlmedo [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
