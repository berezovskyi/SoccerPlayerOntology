...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IWalterBuchananFootballer;
...
public class WalterBuchananFootballer implements IWalterBuchananFootballer {
	...
	public WalterBuchananFootballer(WalterBuchananFootballer walterBuchananFootballer)
			throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
			NoSuchMethodException, SecurityException {
		this.annotationID = walterBuchananFootballer.getAnnotationID();
		for (int j = 0; j < walterBuchananFootballer.getBirthPlaces().size(); j++) {
			if (walterBuchananFootballer.getBirthPlaces().get(j) != null) {
				birthPlaces.add(walterBuchananFootballer.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(walterBuchananFootballer.getBirthPlaces().get(j).getClass())
						.newInstance(walterBuchananFootballer.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (walterBuchananFootballer.getBirthYear() != null)
			this.birthYear = walterBuchananFootballer.getBirthYear().getClass()
					.getDeclaredConstructor(walterBuchananFootballer.getBirthYear().getClass())
					.newInstance(walterBuchananFootballer.getBirthYear());
		...
		if (walterBuchananFootballer.getPosition() != null)
			this.position = walterBuchananFootballer.getPosition().getClass()
					.getDeclaredConstructor(walterBuchananFootballer.getPosition().getClass())
					.newInstance(walterBuchananFootballer.getPosition());
		for (int j = 0; j < walterBuchananFootballer.getTeams().size(); j++) {
			if (walterBuchananFootballer.getTeams().get(j) != null) {
				teams.add(walterBuchananFootballer.getTeams().get(j).getClass()
						.getDeclaredConstructor(walterBuchananFootballer.getTeams().get(j).getClass())
						.newInstance(walterBuchananFootballer.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "WalterBuchananFootballer [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
