...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAlanBuck;
...
public class AlanBuck implements IAlanBuck {
	...
	public AlanBuck(AlanBuck alanBuck) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = alanBuck.getAnnotationID();
		for (int j = 0; j < alanBuck.getBirthPlaces().size(); j++) {
			if (alanBuck.getBirthPlaces().get(j) != null) {
				birthPlaces.add(alanBuck.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(alanBuck.getBirthPlaces().get(j).getClass())
						.newInstance(alanBuck.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (alanBuck.getBirthYear() != null)
			this.birthYear = alanBuck.getBirthYear().getClass()
					.getDeclaredConstructor(alanBuck.getBirthYear().getClass()).newInstance(alanBuck.getBirthYear());
		this.characterOffset = alanBuck.getCharacterOffset();
		this.characterOnset = alanBuck.getCharacterOnset();
		if (alanBuck.getPosition() != null)
			this.position = alanBuck.getPosition().getClass().getDeclaredConstructor(alanBuck.getPosition().getClass())
					.newInstance(alanBuck.getPosition());
		for (int j = 0; j < alanBuck.getTeams().size(); j++) {
			if (alanBuck.getTeams().get(j) != null) {
				teams.add(alanBuck.getTeams().get(j).getClass()
						.getDeclaredConstructor(alanBuck.getTeams().get(j).getClass())
						.newInstance(alanBuck.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AlanBuck [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
