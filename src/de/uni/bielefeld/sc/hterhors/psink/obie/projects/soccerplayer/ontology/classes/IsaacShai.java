...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IIsaacShai;
...
public class IsaacShai implements IIsaacShai {
	...
	public IsaacShai(IsaacShai isaacShai) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = isaacShai.getAnnotationID();
		for (int j = 0; j < isaacShai.getBirthPlaces().size(); j++) {
			if (isaacShai.getBirthPlaces().get(j) != null) {
				birthPlaces.add(isaacShai.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(isaacShai.getBirthPlaces().get(j).getClass())
						.newInstance(isaacShai.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (isaacShai.getBirthYear() != null)
			this.birthYear = isaacShai.getBirthYear().getClass()
					.getDeclaredConstructor(isaacShai.getBirthYear().getClass()).newInstance(isaacShai.getBirthYear());
		this.characterOffset = isaacShai.getCharacterOffset();
		this.characterOnset = isaacShai.getCharacterOnset();
		if (isaacShai.getPosition() != null)
			this.position = isaacShai.getPosition().getClass()
					.getDeclaredConstructor(isaacShai.getPosition().getClass()).newInstance(isaacShai.getPosition());
		for (int j = 0; j < isaacShai.getTeams().size(); j++) {
			if (isaacShai.getTeams().get(j) != null) {
				teams.add(isaacShai.getTeams().get(j).getClass()
						.getDeclaredConstructor(isaacShai.getTeams().get(j).getClass())
						.newInstance(isaacShai.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "IsaacShai [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
