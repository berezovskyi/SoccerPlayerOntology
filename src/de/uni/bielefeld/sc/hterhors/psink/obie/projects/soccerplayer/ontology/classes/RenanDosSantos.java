...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRenanDosSantos;
...
public class RenanDosSantos implements IRenanDosSantos {
	...
	public RenanDosSantos(RenanDosSantos renanDosSantos) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = renanDosSantos.getAnnotationID();
		for (int j = 0; j < renanDosSantos.getBirthPlaces().size(); j++) {
			if (renanDosSantos.getBirthPlaces().get(j) != null) {
				birthPlaces.add(renanDosSantos.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(renanDosSantos.getBirthPlaces().get(j).getClass())
						.newInstance(renanDosSantos.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (renanDosSantos.getBirthYear() != null)
			this.birthYear = renanDosSantos.getBirthYear().getClass()
					.getDeclaredConstructor(renanDosSantos.getBirthYear().getClass())
					.newInstance(renanDosSantos.getBirthYear());
		this.characterOffset = renanDosSantos.getCharacterOffset();
		this.characterOnset = renanDosSantos.getCharacterOnset();
		if (renanDosSantos.getPosition() != null)
			this.position = renanDosSantos.getPosition().getClass()
					.getDeclaredConstructor(renanDosSantos.getPosition().getClass())
					.newInstance(renanDosSantos.getPosition());
		for (int j = 0; j < renanDosSantos.getTeams().size(); j++) {
			if (renanDosSantos.getTeams().get(j) != null) {
				teams.add(renanDosSantos.getTeams().get(j).getClass()
						.getDeclaredConstructor(renanDosSantos.getTeams().get(j).getClass())
						.newInstance(renanDosSantos.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RenanDosSantos [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
