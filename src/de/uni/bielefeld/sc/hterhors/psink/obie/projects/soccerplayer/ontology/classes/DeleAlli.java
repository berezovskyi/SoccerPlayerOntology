...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDeleAlli;
...
public class DeleAlli implements IDeleAlli {
	...
	public DeleAlli(DeleAlli deleAlli) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = deleAlli.getAnnotationID();
		for (int j = 0; j < deleAlli.getBirthPlaces().size(); j++) {
			if (deleAlli.getBirthPlaces().get(j) != null) {
				birthPlaces.add(deleAlli.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(deleAlli.getBirthPlaces().get(j).getClass())
						.newInstance(deleAlli.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (deleAlli.getBirthYear() != null)
			this.birthYear = deleAlli.getBirthYear().getClass()
					.getDeclaredConstructor(deleAlli.getBirthYear().getClass()).newInstance(deleAlli.getBirthYear());
		this.characterOffset = deleAlli.getCharacterOffset();
		this.characterOnset = deleAlli.getCharacterOnset();
		if (deleAlli.getPosition() != null)
			this.position = deleAlli.getPosition().getClass().getDeclaredConstructor(deleAlli.getPosition().getClass())
					.newInstance(deleAlli.getPosition());
		for (int j = 0; j < deleAlli.getTeams().size(); j++) {
			if (deleAlli.getTeams().get(j) != null) {
				teams.add(deleAlli.getTeams().get(j).getClass()
						.getDeclaredConstructor(deleAlli.getTeams().get(j).getClass())
						.newInstance(deleAlli.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DeleAlli [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
