...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJakeCuenca;
...
public class JakeCuenca implements IJakeCuenca {
	...
	public JakeCuenca(JakeCuenca jakeCuenca) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = jakeCuenca.getAnnotationID();
		for (int j = 0; j < jakeCuenca.getBirthPlaces().size(); j++) {
			if (jakeCuenca.getBirthPlaces().get(j) != null) {
				birthPlaces.add(jakeCuenca.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(jakeCuenca.getBirthPlaces().get(j).getClass())
						.newInstance(jakeCuenca.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (jakeCuenca.getBirthYear() != null)
			this.birthYear = jakeCuenca.getBirthYear().getClass()
					.getDeclaredConstructor(jakeCuenca.getBirthYear().getClass())
					.newInstance(jakeCuenca.getBirthYear());
		this.characterOffset = jakeCuenca.getCharacterOffset();
		this.characterOnset = jakeCuenca.getCharacterOnset();
		if (jakeCuenca.getPosition() != null)
			this.position = jakeCuenca.getPosition().getClass()
					.getDeclaredConstructor(jakeCuenca.getPosition().getClass()).newInstance(jakeCuenca.getPosition());
		for (int j = 0; j < jakeCuenca.getTeams().size(); j++) {
			if (jakeCuenca.getTeams().get(j) != null) {
				teams.add(jakeCuenca.getTeams().get(j).getClass()
						.getDeclaredConstructor(jakeCuenca.getTeams().get(j).getClass())
						.newInstance(jakeCuenca.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JakeCuenca [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
