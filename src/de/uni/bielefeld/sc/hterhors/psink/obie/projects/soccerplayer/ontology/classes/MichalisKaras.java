...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMichalisKaras;
...
public class MichalisKaras implements IMichalisKaras {
	...
	public MichalisKaras(MichalisKaras michalisKaras) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = michalisKaras.getAnnotationID();
		for (int j = 0; j < michalisKaras.getBirthPlaces().size(); j++) {
			if (michalisKaras.getBirthPlaces().get(j) != null) {
				birthPlaces.add(michalisKaras.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(michalisKaras.getBirthPlaces().get(j).getClass())
						.newInstance(michalisKaras.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (michalisKaras.getBirthYear() != null)
			this.birthYear = michalisKaras.getBirthYear().getClass()
					.getDeclaredConstructor(michalisKaras.getBirthYear().getClass())
					.newInstance(michalisKaras.getBirthYear());
		this.characterOffset = michalisKaras.getCharacterOffset();
		this.characterOnset = michalisKaras.getCharacterOnset();
		if (michalisKaras.getPosition() != null)
			this.position = michalisKaras.getPosition().getClass()
					.getDeclaredConstructor(michalisKaras.getPosition().getClass())
					.newInstance(michalisKaras.getPosition());
		for (int j = 0; j < michalisKaras.getTeams().size(); j++) {
			if (michalisKaras.getTeams().get(j) != null) {
				teams.add(michalisKaras.getTeams().get(j).getClass()
						.getDeclaredConstructor(michalisKaras.getTeams().get(j).getClass())
						.newInstance(michalisKaras.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MichalisKaras [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
