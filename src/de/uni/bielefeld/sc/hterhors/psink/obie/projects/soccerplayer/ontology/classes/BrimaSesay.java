...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBrimaSesay;
...
public class BrimaSesay implements IBrimaSesay {
	...
	public BrimaSesay(BrimaSesay brimaSesay) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = brimaSesay.getAnnotationID();
		for (int j = 0; j < brimaSesay.getBirthPlaces().size(); j++) {
			if (brimaSesay.getBirthPlaces().get(j) != null) {
				birthPlaces.add(brimaSesay.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(brimaSesay.getBirthPlaces().get(j).getClass())
						.newInstance(brimaSesay.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (brimaSesay.getBirthYear() != null)
			this.birthYear = brimaSesay.getBirthYear().getClass()
					.getDeclaredConstructor(brimaSesay.getBirthYear().getClass())
					.newInstance(brimaSesay.getBirthYear());
		this.characterOffset = brimaSesay.getCharacterOffset();
		this.characterOnset = brimaSesay.getCharacterOnset();
		if (brimaSesay.getPosition() != null)
			this.position = brimaSesay.getPosition().getClass()
					.getDeclaredConstructor(brimaSesay.getPosition().getClass()).newInstance(brimaSesay.getPosition());
		for (int j = 0; j < brimaSesay.getTeams().size(); j++) {
			if (brimaSesay.getTeams().get(j) != null) {
				teams.add(brimaSesay.getTeams().get(j).getClass()
						.getDeclaredConstructor(brimaSesay.getTeams().get(j).getClass())
						.newInstance(brimaSesay.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BrimaSesay [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
