...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAnthonyWordsworth;
...
public class AnthonyWordsworth implements IAnthonyWordsworth {
	...
	public AnthonyWordsworth(AnthonyWordsworth anthonyWordsworth) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = anthonyWordsworth.getAnnotationID();
		for (int j = 0; j < anthonyWordsworth.getBirthPlaces().size(); j++) {
			if (anthonyWordsworth.getBirthPlaces().get(j) != null) {
				birthPlaces.add(anthonyWordsworth.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(anthonyWordsworth.getBirthPlaces().get(j).getClass())
						.newInstance(anthonyWordsworth.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (anthonyWordsworth.getBirthYear() != null)
			this.birthYear = anthonyWordsworth.getBirthYear().getClass()
					.getDeclaredConstructor(anthonyWordsworth.getBirthYear().getClass())
					.newInstance(anthonyWordsworth.getBirthYear());
		this.characterOffset = anthonyWordsworth.getCharacterOffset();
		this.characterOnset = anthonyWordsworth.getCharacterOnset();
		if (anthonyWordsworth.getPosition() != null)
			this.position = anthonyWordsworth.getPosition().getClass()
					.getDeclaredConstructor(anthonyWordsworth.getPosition().getClass())
					.newInstance(anthonyWordsworth.getPosition());
		for (int j = 0; j < anthonyWordsworth.getTeams().size(); j++) {
			if (anthonyWordsworth.getTeams().get(j) != null) {
				teams.add(anthonyWordsworth.getTeams().get(j).getClass()
						.getDeclaredConstructor(anthonyWordsworth.getTeams().get(j).getClass())
						.newInstance(anthonyWordsworth.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AnthonyWordsworth [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
