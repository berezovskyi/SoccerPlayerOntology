...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAbdelazizSouleimani;
...
public class AbdelazizSouleimani implements IAbdelazizSouleimani {
	...
	public AbdelazizSouleimani(AbdelazizSouleimani abdelazizSouleimani)
			throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
			NoSuchMethodException, SecurityException {
		this.annotationID = abdelazizSouleimani.getAnnotationID();
		for (int j = 0; j < abdelazizSouleimani.getBirthPlaces().size(); j++) {
			if (abdelazizSouleimani.getBirthPlaces().get(j) != null) {
				birthPlaces.add(abdelazizSouleimani.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(abdelazizSouleimani.getBirthPlaces().get(j).getClass())
						.newInstance(abdelazizSouleimani.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (abdelazizSouleimani.getBirthYear() != null)
			this.birthYear = abdelazizSouleimani.getBirthYear().getClass()
					.getDeclaredConstructor(abdelazizSouleimani.getBirthYear().getClass())
					.newInstance(abdelazizSouleimani.getBirthYear());
		this.characterOffset = abdelazizSouleimani.getCharacterOffset();
		this.characterOnset = abdelazizSouleimani.getCharacterOnset();
		if (abdelazizSouleimani.getPosition() != null)
			this.position = abdelazizSouleimani.getPosition().getClass()
					.getDeclaredConstructor(abdelazizSouleimani.getPosition().getClass())
					.newInstance(abdelazizSouleimani.getPosition());
		for (int j = 0; j < abdelazizSouleimani.getTeams().size(); j++) {
			if (abdelazizSouleimani.getTeams().get(j) != null) {
				teams.add(abdelazizSouleimani.getTeams().get(j).getClass()
						.getDeclaredConstructor(abdelazizSouleimani.getTeams().get(j).getClass())
						.newInstance(abdelazizSouleimani.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AbdelazizSouleimani [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
