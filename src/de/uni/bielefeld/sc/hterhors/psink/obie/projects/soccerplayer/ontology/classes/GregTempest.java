...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IGregTempest;
...
public class GregTempest implements IGregTempest {
	...
	public GregTempest(GregTempest gregTempest) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = gregTempest.getAnnotationID();
		for (int j = 0; j < gregTempest.getBirthPlaces().size(); j++) {
			if (gregTempest.getBirthPlaces().get(j) != null) {
				birthPlaces.add(gregTempest.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(gregTempest.getBirthPlaces().get(j).getClass())
						.newInstance(gregTempest.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (gregTempest.getBirthYear() != null)
			this.birthYear = gregTempest.getBirthYear().getClass()
					.getDeclaredConstructor(gregTempest.getBirthYear().getClass())
					.newInstance(gregTempest.getBirthYear());
		this.characterOffset = gregTempest.getCharacterOffset();
		this.characterOnset = gregTempest.getCharacterOnset();
		if (gregTempest.getPosition() != null)
			this.position = gregTempest.getPosition().getClass()
					.getDeclaredConstructor(gregTempest.getPosition().getClass())
					.newInstance(gregTempest.getPosition());
		for (int j = 0; j < gregTempest.getTeams().size(); j++) {
			if (gregTempest.getTeams().get(j) != null) {
				teams.add(gregTempest.getTeams().get(j).getClass()
						.getDeclaredConstructor(gregTempest.getTeams().get(j).getClass())
						.newInstance(gregTempest.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "GregTempest [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
