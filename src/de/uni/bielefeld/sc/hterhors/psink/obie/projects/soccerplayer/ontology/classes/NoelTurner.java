...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.INoelTurner;
...
public class NoelTurner implements INoelTurner {
	...
	public NoelTurner(NoelTurner noelTurner) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = noelTurner.getAnnotationID();
		for (int j = 0; j < noelTurner.getBirthPlaces().size(); j++) {
			if (noelTurner.getBirthPlaces().get(j) != null) {
				birthPlaces.add(noelTurner.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(noelTurner.getBirthPlaces().get(j).getClass())
						.newInstance(noelTurner.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (noelTurner.getBirthYear() != null)
			this.birthYear = noelTurner.getBirthYear().getClass()
					.getDeclaredConstructor(noelTurner.getBirthYear().getClass())
					.newInstance(noelTurner.getBirthYear());
		this.characterOffset = noelTurner.getCharacterOffset();
		this.characterOnset = noelTurner.getCharacterOnset();
		if (noelTurner.getPosition() != null)
			this.position = noelTurner.getPosition().getClass()
					.getDeclaredConstructor(noelTurner.getPosition().getClass()).newInstance(noelTurner.getPosition());
		for (int j = 0; j < noelTurner.getTeams().size(); j++) {
			if (noelTurner.getTeams().get(j) != null) {
				teams.add(noelTurner.getTeams().get(j).getClass()
						.getDeclaredConstructor(noelTurner.getTeams().get(j).getClass())
						.newInstance(noelTurner.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "NoelTurner [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
