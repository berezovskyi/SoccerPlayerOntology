...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IYussefAlSuwayed;
...
public class YussefAlSuwayed implements IYussefAlSuwayed {
	...
	public YussefAlSuwayed(YussefAlSuwayed yussefAlSuwayed) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = yussefAlSuwayed.getAnnotationID();
		for (int j = 0; j < yussefAlSuwayed.getBirthPlaces().size(); j++) {
			if (yussefAlSuwayed.getBirthPlaces().get(j) != null) {
				birthPlaces.add(yussefAlSuwayed.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(yussefAlSuwayed.getBirthPlaces().get(j).getClass())
						.newInstance(yussefAlSuwayed.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (yussefAlSuwayed.getBirthYear() != null)
			this.birthYear = yussefAlSuwayed.getBirthYear().getClass()
					.getDeclaredConstructor(yussefAlSuwayed.getBirthYear().getClass())
					.newInstance(yussefAlSuwayed.getBirthYear());
		this.characterOffset = yussefAlSuwayed.getCharacterOffset();
		this.characterOnset = yussefAlSuwayed.getCharacterOnset();
		if (yussefAlSuwayed.getPosition() != null)
			this.position = yussefAlSuwayed.getPosition().getClass()
					.getDeclaredConstructor(yussefAlSuwayed.getPosition().getClass())
					.newInstance(yussefAlSuwayed.getPosition());
		for (int j = 0; j < yussefAlSuwayed.getTeams().size(); j++) {
			if (yussefAlSuwayed.getTeams().get(j) != null) {
				teams.add(yussefAlSuwayed.getTeams().get(j).getClass()
						.getDeclaredConstructor(yussefAlSuwayed.getTeams().get(j).getClass())
						.newInstance(yussefAlSuwayed.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "YussefAlSuwayed [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
