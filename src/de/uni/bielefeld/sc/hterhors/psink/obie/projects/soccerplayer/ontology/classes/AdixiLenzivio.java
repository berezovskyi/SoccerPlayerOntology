...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAdixiLenzivio;
...
public class AdixiLenzivio implements IAdixiLenzivio {
	...
	public AdixiLenzivio(AdixiLenzivio adixiLenzivio) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = adixiLenzivio.getAnnotationID();
		for (int j = 0; j < adixiLenzivio.getBirthPlaces().size(); j++) {
			if (adixiLenzivio.getBirthPlaces().get(j) != null) {
				birthPlaces.add(adixiLenzivio.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(adixiLenzivio.getBirthPlaces().get(j).getClass())
						.newInstance(adixiLenzivio.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (adixiLenzivio.getBirthYear() != null)
			this.birthYear = adixiLenzivio.getBirthYear().getClass()
					.getDeclaredConstructor(adixiLenzivio.getBirthYear().getClass())
					.newInstance(adixiLenzivio.getBirthYear());
		this.characterOffset = adixiLenzivio.getCharacterOffset();
		this.characterOnset = adixiLenzivio.getCharacterOnset();
		if (adixiLenzivio.getPosition() != null)
			this.position = adixiLenzivio.getPosition().getClass()
					.getDeclaredConstructor(adixiLenzivio.getPosition().getClass())
					.newInstance(adixiLenzivio.getPosition());
		for (int j = 0; j < adixiLenzivio.getTeams().size(); j++) {
			if (adixiLenzivio.getTeams().get(j) != null) {
				teams.add(adixiLenzivio.getTeams().get(j).getClass()
						.getDeclaredConstructor(adixiLenzivio.getTeams().get(j).getClass())
						.newInstance(adixiLenzivio.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AdixiLenzivio [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
