...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IIvanDeyanov;
...
public class IvanDeyanov implements IIvanDeyanov {
	...
	public IvanDeyanov(IvanDeyanov ivanDeyanov) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = ivanDeyanov.getAnnotationID();
		for (int j = 0; j < ivanDeyanov.getBirthPlaces().size(); j++) {
			if (ivanDeyanov.getBirthPlaces().get(j) != null) {
				birthPlaces.add(ivanDeyanov.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(ivanDeyanov.getBirthPlaces().get(j).getClass())
						.newInstance(ivanDeyanov.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (ivanDeyanov.getBirthYear() != null)
			this.birthYear = ivanDeyanov.getBirthYear().getClass()
					.getDeclaredConstructor(ivanDeyanov.getBirthYear().getClass())
					.newInstance(ivanDeyanov.getBirthYear());
		this.characterOffset = ivanDeyanov.getCharacterOffset();
		this.characterOnset = ivanDeyanov.getCharacterOnset();
		if (ivanDeyanov.getPosition() != null)
			this.position = ivanDeyanov.getPosition().getClass()
					.getDeclaredConstructor(ivanDeyanov.getPosition().getClass())
					.newInstance(ivanDeyanov.getPosition());
		for (int j = 0; j < ivanDeyanov.getTeams().size(); j++) {
			if (ivanDeyanov.getTeams().get(j) != null) {
				teams.add(ivanDeyanov.getTeams().get(j).getClass()
						.getDeclaredConstructor(ivanDeyanov.getTeams().get(j).getClass())
						.newInstance(ivanDeyanov.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "IvanDeyanov [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
