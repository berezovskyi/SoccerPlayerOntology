...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ICharlieDeacon;
...
public class CharlieDeacon implements ICharlieDeacon {
	...
	public CharlieDeacon(CharlieDeacon charlieDeacon) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = charlieDeacon.getAnnotationID();
		for (int j = 0; j < charlieDeacon.getBirthPlaces().size(); j++) {
			if (charlieDeacon.getBirthPlaces().get(j) != null) {
				birthPlaces.add(charlieDeacon.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(charlieDeacon.getBirthPlaces().get(j).getClass())
						.newInstance(charlieDeacon.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (charlieDeacon.getBirthYear() != null)
			this.birthYear = charlieDeacon.getBirthYear().getClass()
					.getDeclaredConstructor(charlieDeacon.getBirthYear().getClass())
					.newInstance(charlieDeacon.getBirthYear());
		this.characterOffset = charlieDeacon.getCharacterOffset();
		this.characterOnset = charlieDeacon.getCharacterOnset();
		if (charlieDeacon.getPosition() != null)
			this.position = charlieDeacon.getPosition().getClass()
					.getDeclaredConstructor(charlieDeacon.getPosition().getClass())
					.newInstance(charlieDeacon.getPosition());
		for (int j = 0; j < charlieDeacon.getTeams().size(); j++) {
			if (charlieDeacon.getTeams().get(j) != null) {
				teams.add(charlieDeacon.getTeams().get(j).getClass()
						.getDeclaredConstructor(charlieDeacon.getTeams().get(j).getClass())
						.newInstance(charlieDeacon.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "CharlieDeacon [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
