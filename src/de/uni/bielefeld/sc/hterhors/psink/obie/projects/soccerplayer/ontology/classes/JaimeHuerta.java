...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJaimeHuerta;
...
public class JaimeHuerta implements IJaimeHuerta {
	...
	public JaimeHuerta(JaimeHuerta jaimeHuerta) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = jaimeHuerta.getAnnotationID();
		for (int j = 0; j < jaimeHuerta.getBirthPlaces().size(); j++) {
			if (jaimeHuerta.getBirthPlaces().get(j) != null) {
				birthPlaces.add(jaimeHuerta.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(jaimeHuerta.getBirthPlaces().get(j).getClass())
						.newInstance(jaimeHuerta.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (jaimeHuerta.getBirthYear() != null)
			this.birthYear = jaimeHuerta.getBirthYear().getClass()
					.getDeclaredConstructor(jaimeHuerta.getBirthYear().getClass())
					.newInstance(jaimeHuerta.getBirthYear());
		this.characterOffset = jaimeHuerta.getCharacterOffset();
		this.characterOnset = jaimeHuerta.getCharacterOnset();
		if (jaimeHuerta.getPosition() != null)
			this.position = jaimeHuerta.getPosition().getClass()
					.getDeclaredConstructor(jaimeHuerta.getPosition().getClass())
					.newInstance(jaimeHuerta.getPosition());
		for (int j = 0; j < jaimeHuerta.getTeams().size(); j++) {
			if (jaimeHuerta.getTeams().get(j) != null) {
				teams.add(jaimeHuerta.getTeams().get(j).getClass()
						.getDeclaredConstructor(jaimeHuerta.getTeams().get(j).getClass())
						.newInstance(jaimeHuerta.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JaimeHuerta [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
