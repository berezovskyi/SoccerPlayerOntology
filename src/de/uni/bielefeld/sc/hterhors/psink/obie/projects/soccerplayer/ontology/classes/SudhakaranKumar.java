...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISudhakaranKumar;
...
public class SudhakaranKumar implements ISudhakaranKumar {
	...
	public SudhakaranKumar(SudhakaranKumar sudhakaranKumar) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = sudhakaranKumar.getAnnotationID();
		for (int j = 0; j < sudhakaranKumar.getBirthPlaces().size(); j++) {
			if (sudhakaranKumar.getBirthPlaces().get(j) != null) {
				birthPlaces.add(sudhakaranKumar.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(sudhakaranKumar.getBirthPlaces().get(j).getClass())
						.newInstance(sudhakaranKumar.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (sudhakaranKumar.getBirthYear() != null)
			this.birthYear = sudhakaranKumar.getBirthYear().getClass()
					.getDeclaredConstructor(sudhakaranKumar.getBirthYear().getClass())
					.newInstance(sudhakaranKumar.getBirthYear());
		this.characterOffset = sudhakaranKumar.getCharacterOffset();
		this.characterOnset = sudhakaranKumar.getCharacterOnset();
		if (sudhakaranKumar.getPosition() != null)
			this.position = sudhakaranKumar.getPosition().getClass()
					.getDeclaredConstructor(sudhakaranKumar.getPosition().getClass())
					.newInstance(sudhakaranKumar.getPosition());
		for (int j = 0; j < sudhakaranKumar.getTeams().size(); j++) {
			if (sudhakaranKumar.getTeams().get(j) != null) {
				teams.add(sudhakaranKumar.getTeams().get(j).getClass()
						.getDeclaredConstructor(sudhakaranKumar.getTeams().get(j).getClass())
						.newInstance(sudhakaranKumar.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SudhakaranKumar [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
