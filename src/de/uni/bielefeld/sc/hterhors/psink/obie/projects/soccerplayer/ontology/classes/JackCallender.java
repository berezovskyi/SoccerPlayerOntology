...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJackCallender;
...
public class JackCallender implements IJackCallender {
	...
	public JackCallender(JackCallender jackCallender) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = jackCallender.getAnnotationID();
		for (int j = 0; j < jackCallender.getBirthPlaces().size(); j++) {
			if (jackCallender.getBirthPlaces().get(j) != null) {
				birthPlaces.add(jackCallender.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(jackCallender.getBirthPlaces().get(j).getClass())
						.newInstance(jackCallender.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (jackCallender.getBirthYear() != null)
			this.birthYear = jackCallender.getBirthYear().getClass()
					.getDeclaredConstructor(jackCallender.getBirthYear().getClass())
					.newInstance(jackCallender.getBirthYear());
		this.characterOffset = jackCallender.getCharacterOffset();
		this.characterOnset = jackCallender.getCharacterOnset();
		if (jackCallender.getPosition() != null)
			this.position = jackCallender.getPosition().getClass()
					.getDeclaredConstructor(jackCallender.getPosition().getClass())
					.newInstance(jackCallender.getPosition());
		for (int j = 0; j < jackCallender.getTeams().size(); j++) {
			if (jackCallender.getTeams().get(j) != null) {
				teams.add(jackCallender.getTeams().get(j).getClass()
						.getDeclaredConstructor(jackCallender.getTeams().get(j).getClass())
						.newInstance(jackCallender.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JackCallender [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
