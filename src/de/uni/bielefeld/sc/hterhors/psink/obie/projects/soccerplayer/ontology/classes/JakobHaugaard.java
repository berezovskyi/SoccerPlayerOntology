...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJakobHaugaard;
...
public class JakobHaugaard implements IJakobHaugaard {
	...
	public JakobHaugaard(JakobHaugaard jakobHaugaard) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = jakobHaugaard.getAnnotationID();
		for (int j = 0; j < jakobHaugaard.getBirthPlaces().size(); j++) {
			if (jakobHaugaard.getBirthPlaces().get(j) != null) {
				birthPlaces.add(jakobHaugaard.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(jakobHaugaard.getBirthPlaces().get(j).getClass())
						.newInstance(jakobHaugaard.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (jakobHaugaard.getBirthYear() != null)
			this.birthYear = jakobHaugaard.getBirthYear().getClass()
					.getDeclaredConstructor(jakobHaugaard.getBirthYear().getClass())
					.newInstance(jakobHaugaard.getBirthYear());
		this.characterOffset = jakobHaugaard.getCharacterOffset();
		this.characterOnset = jakobHaugaard.getCharacterOnset();
		if (jakobHaugaard.getPosition() != null)
			this.position = jakobHaugaard.getPosition().getClass()
					.getDeclaredConstructor(jakobHaugaard.getPosition().getClass())
					.newInstance(jakobHaugaard.getPosition());
		for (int j = 0; j < jakobHaugaard.getTeams().size(); j++) {
			if (jakobHaugaard.getTeams().get(j) != null) {
				teams.add(jakobHaugaard.getTeams().get(j).getClass()
						.getDeclaredConstructor(jakobHaugaard.getTeams().get(j).getClass())
						.newInstance(jakobHaugaard.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JakobHaugaard [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
