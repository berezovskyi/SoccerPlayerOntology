...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IHaroldKeenan;
...
public class HaroldKeenan implements IHaroldKeenan {
	...
	public HaroldKeenan(HaroldKeenan haroldKeenan) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = haroldKeenan.getAnnotationID();
		for (int j = 0; j < haroldKeenan.getBirthPlaces().size(); j++) {
			if (haroldKeenan.getBirthPlaces().get(j) != null) {
				birthPlaces.add(haroldKeenan.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(haroldKeenan.getBirthPlaces().get(j).getClass())
						.newInstance(haroldKeenan.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (haroldKeenan.getBirthYear() != null)
			this.birthYear = haroldKeenan.getBirthYear().getClass()
					.getDeclaredConstructor(haroldKeenan.getBirthYear().getClass())
					.newInstance(haroldKeenan.getBirthYear());
		this.characterOffset = haroldKeenan.getCharacterOffset();
		this.characterOnset = haroldKeenan.getCharacterOnset();
		if (haroldKeenan.getPosition() != null)
			this.position = haroldKeenan.getPosition().getClass()
					.getDeclaredConstructor(haroldKeenan.getPosition().getClass())
					.newInstance(haroldKeenan.getPosition());
		for (int j = 0; j < haroldKeenan.getTeams().size(); j++) {
			if (haroldKeenan.getTeams().get(j) != null) {
				teams.add(haroldKeenan.getTeams().get(j).getClass()
						.getDeclaredConstructor(haroldKeenan.getTeams().get(j).getClass())
						.newInstance(haroldKeenan.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "HaroldKeenan [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
