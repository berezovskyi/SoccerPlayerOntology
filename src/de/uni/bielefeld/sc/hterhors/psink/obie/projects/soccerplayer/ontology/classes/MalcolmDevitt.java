...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMalcolmDevitt;
...
public class MalcolmDevitt implements IMalcolmDevitt {
	...
	public MalcolmDevitt(MalcolmDevitt malcolmDevitt) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = malcolmDevitt.getAnnotationID();
		for (int j = 0; j < malcolmDevitt.getBirthPlaces().size(); j++) {
			if (malcolmDevitt.getBirthPlaces().get(j) != null) {
				birthPlaces.add(malcolmDevitt.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(malcolmDevitt.getBirthPlaces().get(j).getClass())
						.newInstance(malcolmDevitt.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (malcolmDevitt.getBirthYear() != null)
			this.birthYear = malcolmDevitt.getBirthYear().getClass()
					.getDeclaredConstructor(malcolmDevitt.getBirthYear().getClass())
					.newInstance(malcolmDevitt.getBirthYear());
		this.characterOffset = malcolmDevitt.getCharacterOffset();
		this.characterOnset = malcolmDevitt.getCharacterOnset();
		if (malcolmDevitt.getPosition() != null)
			this.position = malcolmDevitt.getPosition().getClass()
					.getDeclaredConstructor(malcolmDevitt.getPosition().getClass())
					.newInstance(malcolmDevitt.getPosition());
		for (int j = 0; j < malcolmDevitt.getTeams().size(); j++) {
			if (malcolmDevitt.getTeams().get(j) != null) {
				teams.add(malcolmDevitt.getTeams().get(j).getClass()
						.getDeclaredConstructor(malcolmDevitt.getTeams().get(j).getClass())
						.newInstance(malcolmDevitt.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MalcolmDevitt [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
