...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IUgoOkoye;
...
public class UgoOkoye implements IUgoOkoye {
	...
	public UgoOkoye(UgoOkoye ugoOkoye) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = ugoOkoye.getAnnotationID();
		for (int j = 0; j < ugoOkoye.getBirthPlaces().size(); j++) {
			if (ugoOkoye.getBirthPlaces().get(j) != null) {
				birthPlaces.add(ugoOkoye.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(ugoOkoye.getBirthPlaces().get(j).getClass())
						.newInstance(ugoOkoye.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (ugoOkoye.getBirthYear() != null)
			this.birthYear = ugoOkoye.getBirthYear().getClass()
					.getDeclaredConstructor(ugoOkoye.getBirthYear().getClass()).newInstance(ugoOkoye.getBirthYear());
		this.characterOffset = ugoOkoye.getCharacterOffset();
		this.characterOnset = ugoOkoye.getCharacterOnset();
		if (ugoOkoye.getPosition() != null)
			this.position = ugoOkoye.getPosition().getClass().getDeclaredConstructor(ugoOkoye.getPosition().getClass())
					.newInstance(ugoOkoye.getPosition());
		for (int j = 0; j < ugoOkoye.getTeams().size(); j++) {
			if (ugoOkoye.getTeams().get(j) != null) {
				teams.add(ugoOkoye.getTeams().get(j).getClass()
						.getDeclaredConstructor(ugoOkoye.getTeams().get(j).getClass())
						.newInstance(ugoOkoye.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "UgoOkoye [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
