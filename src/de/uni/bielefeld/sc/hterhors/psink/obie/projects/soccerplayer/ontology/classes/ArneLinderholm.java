...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IArneLinderholm;
...
public class ArneLinderholm implements IArneLinderholm {
	...
	public ArneLinderholm(ArneLinderholm arneLinderholm) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = arneLinderholm.getAnnotationID();
		for (int j = 0; j < arneLinderholm.getBirthPlaces().size(); j++) {
			if (arneLinderholm.getBirthPlaces().get(j) != null) {
				birthPlaces.add(arneLinderholm.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(arneLinderholm.getBirthPlaces().get(j).getClass())
						.newInstance(arneLinderholm.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (arneLinderholm.getBirthYear() != null)
			this.birthYear = arneLinderholm.getBirthYear().getClass()
					.getDeclaredConstructor(arneLinderholm.getBirthYear().getClass())
					.newInstance(arneLinderholm.getBirthYear());
		this.characterOffset = arneLinderholm.getCharacterOffset();
		this.characterOnset = arneLinderholm.getCharacterOnset();
		if (arneLinderholm.getPosition() != null)
			this.position = arneLinderholm.getPosition().getClass()
					.getDeclaredConstructor(arneLinderholm.getPosition().getClass())
					.newInstance(arneLinderholm.getPosition());
		for (int j = 0; j < arneLinderholm.getTeams().size(); j++) {
			if (arneLinderholm.getTeams().get(j) != null) {
				teams.add(arneLinderholm.getTeams().get(j).getClass()
						.getDeclaredConstructor(arneLinderholm.getTeams().get(j).getClass())
						.newInstance(arneLinderholm.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ArneLinderholm [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
