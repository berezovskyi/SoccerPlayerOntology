...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILucasMichelMendes;
...
public class LucasMichelMendes implements ILucasMichelMendes {
	...
	public LucasMichelMendes(LucasMichelMendes lucasMichelMendes) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = lucasMichelMendes.getAnnotationID();
		for (int j = 0; j < lucasMichelMendes.getBirthPlaces().size(); j++) {
			if (lucasMichelMendes.getBirthPlaces().get(j) != null) {
				birthPlaces.add(lucasMichelMendes.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(lucasMichelMendes.getBirthPlaces().get(j).getClass())
						.newInstance(lucasMichelMendes.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (lucasMichelMendes.getBirthYear() != null)
			this.birthYear = lucasMichelMendes.getBirthYear().getClass()
					.getDeclaredConstructor(lucasMichelMendes.getBirthYear().getClass())
					.newInstance(lucasMichelMendes.getBirthYear());
		this.characterOffset = lucasMichelMendes.getCharacterOffset();
		this.characterOnset = lucasMichelMendes.getCharacterOnset();
		if (lucasMichelMendes.getPosition() != null)
			this.position = lucasMichelMendes.getPosition().getClass()
					.getDeclaredConstructor(lucasMichelMendes.getPosition().getClass())
					.newInstance(lucasMichelMendes.getPosition());
		for (int j = 0; j < lucasMichelMendes.getTeams().size(); j++) {
			if (lucasMichelMendes.getTeams().get(j) != null) {
				teams.add(lucasMichelMendes.getTeams().get(j).getClass()
						.getDeclaredConstructor(lucasMichelMendes.getTeams().get(j).getClass())
						.newInstance(lucasMichelMendes.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LucasMichelMendes [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
