...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMartinLepa;
...
public class MartinLepa implements IMartinLepa {
	...
	public MartinLepa(MartinLepa martinLepa) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = martinLepa.getAnnotationID();
		for (int j = 0; j < martinLepa.getBirthPlaces().size(); j++) {
			if (martinLepa.getBirthPlaces().get(j) != null) {
				birthPlaces.add(martinLepa.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(martinLepa.getBirthPlaces().get(j).getClass())
						.newInstance(martinLepa.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (martinLepa.getBirthYear() != null)
			this.birthYear = martinLepa.getBirthYear().getClass()
					.getDeclaredConstructor(martinLepa.getBirthYear().getClass())
					.newInstance(martinLepa.getBirthYear());
		this.characterOffset = martinLepa.getCharacterOffset();
		this.characterOnset = martinLepa.getCharacterOnset();
		if (martinLepa.getPosition() != null)
			this.position = martinLepa.getPosition().getClass()
					.getDeclaredConstructor(martinLepa.getPosition().getClass()).newInstance(martinLepa.getPosition());
		for (int j = 0; j < martinLepa.getTeams().size(); j++) {
			if (martinLepa.getTeams().get(j) != null) {
				teams.add(martinLepa.getTeams().get(j).getClass()
						.getDeclaredConstructor(martinLepa.getTeams().get(j).getClass())
						.newInstance(martinLepa.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MartinLepa [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
