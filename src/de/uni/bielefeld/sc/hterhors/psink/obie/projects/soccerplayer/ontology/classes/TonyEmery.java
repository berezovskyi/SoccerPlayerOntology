...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ITonyEmery;
...
public class TonyEmery implements ITonyEmery {
	...
	public TonyEmery(TonyEmery tonyEmery) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = tonyEmery.getAnnotationID();
		for (int j = 0; j < tonyEmery.getBirthPlaces().size(); j++) {
			if (tonyEmery.getBirthPlaces().get(j) != null) {
				birthPlaces.add(tonyEmery.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(tonyEmery.getBirthPlaces().get(j).getClass())
						.newInstance(tonyEmery.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (tonyEmery.getBirthYear() != null)
			this.birthYear = tonyEmery.getBirthYear().getClass()
					.getDeclaredConstructor(tonyEmery.getBirthYear().getClass()).newInstance(tonyEmery.getBirthYear());
		this.characterOffset = tonyEmery.getCharacterOffset();
		this.characterOnset = tonyEmery.getCharacterOnset();
		if (tonyEmery.getPosition() != null)
			this.position = tonyEmery.getPosition().getClass()
					.getDeclaredConstructor(tonyEmery.getPosition().getClass()).newInstance(tonyEmery.getPosition());
		for (int j = 0; j < tonyEmery.getTeams().size(); j++) {
			if (tonyEmery.getTeams().get(j) != null) {
				teams.add(tonyEmery.getTeams().get(j).getClass()
						.getDeclaredConstructor(tonyEmery.getTeams().get(j).getClass())
						.newInstance(tonyEmery.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "TonyEmery [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
