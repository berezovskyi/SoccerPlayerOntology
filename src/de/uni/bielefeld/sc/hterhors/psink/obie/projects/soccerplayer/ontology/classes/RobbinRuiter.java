...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRobbinRuiter;
...
public class RobbinRuiter implements IRobbinRuiter {
	...
	public RobbinRuiter(RobbinRuiter robbinRuiter) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = robbinRuiter.getAnnotationID();
		for (int j = 0; j < robbinRuiter.getBirthPlaces().size(); j++) {
			if (robbinRuiter.getBirthPlaces().get(j) != null) {
				birthPlaces.add(robbinRuiter.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(robbinRuiter.getBirthPlaces().get(j).getClass())
						.newInstance(robbinRuiter.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (robbinRuiter.getBirthYear() != null)
			this.birthYear = robbinRuiter.getBirthYear().getClass()
					.getDeclaredConstructor(robbinRuiter.getBirthYear().getClass())
					.newInstance(robbinRuiter.getBirthYear());
		this.characterOffset = robbinRuiter.getCharacterOffset();
		this.characterOnset = robbinRuiter.getCharacterOnset();
		if (robbinRuiter.getPosition() != null)
			this.position = robbinRuiter.getPosition().getClass()
					.getDeclaredConstructor(robbinRuiter.getPosition().getClass())
					.newInstance(robbinRuiter.getPosition());
		for (int j = 0; j < robbinRuiter.getTeams().size(); j++) {
			if (robbinRuiter.getTeams().get(j) != null) {
				teams.add(robbinRuiter.getTeams().get(j).getClass()
						.getDeclaredConstructor(robbinRuiter.getTeams().get(j).getClass())
						.newInstance(robbinRuiter.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RobbinRuiter [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
