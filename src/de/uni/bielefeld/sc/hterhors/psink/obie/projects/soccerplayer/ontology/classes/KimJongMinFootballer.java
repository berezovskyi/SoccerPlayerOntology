...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IKimJongMinFootballer;
...
public class KimJongMinFootballer implements IKimJongMinFootballer {
	...
	public KimJongMinFootballer(KimJongMinFootballer kimJongMinFootballer)
			throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
			NoSuchMethodException, SecurityException {
		this.annotationID = kimJongMinFootballer.getAnnotationID();
		for (int j = 0; j < kimJongMinFootballer.getBirthPlaces().size(); j++) {
			if (kimJongMinFootballer.getBirthPlaces().get(j) != null) {
				birthPlaces.add(kimJongMinFootballer.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(kimJongMinFootballer.getBirthPlaces().get(j).getClass())
						.newInstance(kimJongMinFootballer.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (kimJongMinFootballer.getBirthYear() != null)
			this.birthYear = kimJongMinFootballer.getBirthYear().getClass()
					.getDeclaredConstructor(kimJongMinFootballer.getBirthYear().getClass())
					.newInstance(kimJongMinFootballer.getBirthYear());
		this.characterOffset = kimJongMinFootballer.getCharacterOffset();
		this.characterOnset = kimJongMinFootballer.getCharacterOnset();
		if (kimJongMinFootballer.getPosition() != null)
			this.position = kimJongMinFootballer.getPosition().getClass()
					.getDeclaredConstructor(kimJongMinFootballer.getPosition().getClass())
					.newInstance(kimJongMinFootballer.getPosition());
		for (int j = 0; j < kimJongMinFootballer.getTeams().size(); j++) {
			if (kimJongMinFootballer.getTeams().get(j) != null) {
				teams.add(kimJongMinFootballer.getTeams().get(j).getClass()
						.getDeclaredConstructor(kimJongMinFootballer.getTeams().get(j).getClass())
						.newInstance(kimJongMinFootballer.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "KimJongMinFootballer [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
