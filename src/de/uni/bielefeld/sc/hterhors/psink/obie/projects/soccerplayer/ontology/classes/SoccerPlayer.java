...
@AssignableSubClasses(get = { AbdelfettahRhiati.class, DanielBentley.class, AcurcioCarrelo.class, LahcenOuadani.class,
...
public class SoccerPlayer implements ISoccerPlayer {
	...
	public SoccerPlayer(SoccerPlayer soccerPlayer) throws InstantiationException, IllegalAccessException,
		...
		if (soccerPlayer.getPosition() != null)
			this.position = soccerPlayer.getPosition().getClass()
					.getDeclaredConstructor(soccerPlayer.getPosition().getClass())
					.newInstance(soccerPlayer.getPosition());
		for (int j = 0; j < soccerPlayer.getTeams().size(); j++) {
			if (soccerPlayer.getTeams().get(j) != null) {
				teams.add(soccerPlayer.getTeams().get(j).getClass()
						.getDeclaredConstructor(soccerPlayer.getTeams().get(j).getClass())
						.newInstance(soccerPlayer.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SoccerPlayer [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
