...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IKnutHansson;
...
public class KnutHansson implements IKnutHansson {
	...
	public KnutHansson(KnutHansson knutHansson) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = knutHansson.getAnnotationID();
		for (int j = 0; j < knutHansson.getBirthPlaces().size(); j++) {
			if (knutHansson.getBirthPlaces().get(j) != null) {
				birthPlaces.add(knutHansson.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(knutHansson.getBirthPlaces().get(j).getClass())
						.newInstance(knutHansson.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (knutHansson.getBirthYear() != null)
			this.birthYear = knutHansson.getBirthYear().getClass()
					.getDeclaredConstructor(knutHansson.getBirthYear().getClass())
					.newInstance(knutHansson.getBirthYear());
		this.characterOffset = knutHansson.getCharacterOffset();
		this.characterOnset = knutHansson.getCharacterOnset();
		if (knutHansson.getPosition() != null)
			this.position = knutHansson.getPosition().getClass()
					.getDeclaredConstructor(knutHansson.getPosition().getClass())
					.newInstance(knutHansson.getPosition());
		for (int j = 0; j < knutHansson.getTeams().size(); j++) {
			if (knutHansson.getTeams().get(j) != null) {
				teams.add(knutHansson.getTeams().get(j).getClass()
						.getDeclaredConstructor(knutHansson.getTeams().get(j).getClass())
						.newInstance(knutHansson.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "KnutHansson [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
