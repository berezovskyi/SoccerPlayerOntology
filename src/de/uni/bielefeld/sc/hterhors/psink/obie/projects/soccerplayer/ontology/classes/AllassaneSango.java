...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAllassaneSango;
...
public class AllassaneSango implements IAllassaneSango {
	...
	public AllassaneSango(AllassaneSango allassaneSango) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = allassaneSango.getAnnotationID();
		for (int j = 0; j < allassaneSango.getBirthPlaces().size(); j++) {
			if (allassaneSango.getBirthPlaces().get(j) != null) {
				birthPlaces.add(allassaneSango.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(allassaneSango.getBirthPlaces().get(j).getClass())
						.newInstance(allassaneSango.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (allassaneSango.getBirthYear() != null)
			this.birthYear = allassaneSango.getBirthYear().getClass()
					.getDeclaredConstructor(allassaneSango.getBirthYear().getClass())
					.newInstance(allassaneSango.getBirthYear());
		this.characterOffset = allassaneSango.getCharacterOffset();
		this.characterOnset = allassaneSango.getCharacterOnset();
		if (allassaneSango.getPosition() != null)
			this.position = allassaneSango.getPosition().getClass()
					.getDeclaredConstructor(allassaneSango.getPosition().getClass())
					.newInstance(allassaneSango.getPosition());
		for (int j = 0; j < allassaneSango.getTeams().size(); j++) {
			if (allassaneSango.getTeams().get(j) != null) {
				teams.add(allassaneSango.getTeams().get(j).getClass()
						.getDeclaredConstructor(allassaneSango.getTeams().get(j).getClass())
						.newInstance(allassaneSango.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AllassaneSango [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
