...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IGerLagendijk;
...
public class GerLagendijk implements IGerLagendijk {
	...
	public GerLagendijk(GerLagendijk gerLagendijk) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = gerLagendijk.getAnnotationID();
		for (int j = 0; j < gerLagendijk.getBirthPlaces().size(); j++) {
			if (gerLagendijk.getBirthPlaces().get(j) != null) {
				birthPlaces.add(gerLagendijk.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(gerLagendijk.getBirthPlaces().get(j).getClass())
						.newInstance(gerLagendijk.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (gerLagendijk.getBirthYear() != null)
			this.birthYear = gerLagendijk.getBirthYear().getClass()
					.getDeclaredConstructor(gerLagendijk.getBirthYear().getClass())
					.newInstance(gerLagendijk.getBirthYear());
		this.characterOffset = gerLagendijk.getCharacterOffset();
		this.characterOnset = gerLagendijk.getCharacterOnset();
		if (gerLagendijk.getPosition() != null)
			this.position = gerLagendijk.getPosition().getClass()
					.getDeclaredConstructor(gerLagendijk.getPosition().getClass())
					.newInstance(gerLagendijk.getPosition());
		for (int j = 0; j < gerLagendijk.getTeams().size(); j++) {
			if (gerLagendijk.getTeams().get(j) != null) {
				teams.add(gerLagendijk.getTeams().get(j).getClass()
						.getDeclaredConstructor(gerLagendijk.getTeams().get(j).getClass())
						.newInstance(gerLagendijk.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "GerLagendijk [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
