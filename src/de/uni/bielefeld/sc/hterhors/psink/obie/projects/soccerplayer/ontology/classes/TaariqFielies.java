...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ITaariqFielies;
...
public class TaariqFielies implements ITaariqFielies {
	...
	public TaariqFielies(TaariqFielies taariqFielies) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = taariqFielies.getAnnotationID();
		for (int j = 0; j < taariqFielies.getBirthPlaces().size(); j++) {
			if (taariqFielies.getBirthPlaces().get(j) != null) {
				birthPlaces.add(taariqFielies.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(taariqFielies.getBirthPlaces().get(j).getClass())
						.newInstance(taariqFielies.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (taariqFielies.getBirthYear() != null)
			this.birthYear = taariqFielies.getBirthYear().getClass()
					.getDeclaredConstructor(taariqFielies.getBirthYear().getClass())
					.newInstance(taariqFielies.getBirthYear());
		this.characterOffset = taariqFielies.getCharacterOffset();
		this.characterOnset = taariqFielies.getCharacterOnset();
		if (taariqFielies.getPosition() != null)
			this.position = taariqFielies.getPosition().getClass()
					.getDeclaredConstructor(taariqFielies.getPosition().getClass())
					.newInstance(taariqFielies.getPosition());
		for (int j = 0; j < taariqFielies.getTeams().size(); j++) {
			if (taariqFielies.getTeams().get(j) != null) {
				teams.add(taariqFielies.getTeams().get(j).getClass()
						.getDeclaredConstructor(taariqFielies.getTeams().get(j).getClass())
						.newInstance(taariqFielies.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "TaariqFielies [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
