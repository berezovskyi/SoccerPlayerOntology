...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IPrasenjitGhosh;
...
public class PrasenjitGhosh implements IPrasenjitGhosh {
	...
	public PrasenjitGhosh(PrasenjitGhosh prasenjitGhosh) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = prasenjitGhosh.getAnnotationID();
		for (int j = 0; j < prasenjitGhosh.getBirthPlaces().size(); j++) {
			if (prasenjitGhosh.getBirthPlaces().get(j) != null) {
				birthPlaces.add(prasenjitGhosh.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(prasenjitGhosh.getBirthPlaces().get(j).getClass())
						.newInstance(prasenjitGhosh.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (prasenjitGhosh.getBirthYear() != null)
			this.birthYear = prasenjitGhosh.getBirthYear().getClass()
					.getDeclaredConstructor(prasenjitGhosh.getBirthYear().getClass())
					.newInstance(prasenjitGhosh.getBirthYear());
		this.characterOffset = prasenjitGhosh.getCharacterOffset();
		this.characterOnset = prasenjitGhosh.getCharacterOnset();
		if (prasenjitGhosh.getPosition() != null)
			this.position = prasenjitGhosh.getPosition().getClass()
					.getDeclaredConstructor(prasenjitGhosh.getPosition().getClass())
					.newInstance(prasenjitGhosh.getPosition());
		for (int j = 0; j < prasenjitGhosh.getTeams().size(); j++) {
			if (prasenjitGhosh.getTeams().get(j) != null) {
				teams.add(prasenjitGhosh.getTeams().get(j).getClass()
						.getDeclaredConstructor(prasenjitGhosh.getTeams().get(j).getClass())
						.newInstance(prasenjitGhosh.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "PrasenjitGhosh [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
