...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJulianAguirreAgudelo;
...
public class JulianAguirreAgudelo implements IJulianAguirreAgudelo {
	...
	public JulianAguirreAgudelo(JulianAguirreAgudelo julianAguirreAgudelo)
			throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
			NoSuchMethodException, SecurityException {
		this.annotationID = julianAguirreAgudelo.getAnnotationID();
		for (int j = 0; j < julianAguirreAgudelo.getBirthPlaces().size(); j++) {
			if (julianAguirreAgudelo.getBirthPlaces().get(j) != null) {
				birthPlaces.add(julianAguirreAgudelo.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(julianAguirreAgudelo.getBirthPlaces().get(j).getClass())
						.newInstance(julianAguirreAgudelo.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (julianAguirreAgudelo.getBirthYear() != null)
			this.birthYear = julianAguirreAgudelo.getBirthYear().getClass()
					.getDeclaredConstructor(julianAguirreAgudelo.getBirthYear().getClass())
					.newInstance(julianAguirreAgudelo.getBirthYear());
		this.characterOffset = julianAguirreAgudelo.getCharacterOffset();
		this.characterOnset = julianAguirreAgudelo.getCharacterOnset();
		if (julianAguirreAgudelo.getPosition() != null)
			this.position = julianAguirreAgudelo.getPosition().getClass()
					.getDeclaredConstructor(julianAguirreAgudelo.getPosition().getClass())
					.newInstance(julianAguirreAgudelo.getPosition());
		for (int j = 0; j < julianAguirreAgudelo.getTeams().size(); j++) {
			if (julianAguirreAgudelo.getTeams().get(j) != null) {
				teams.add(julianAguirreAgudelo.getTeams().get(j).getClass()
						.getDeclaredConstructor(julianAguirreAgudelo.getTeams().get(j).getClass())
						.newInstance(julianAguirreAgudelo.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JulianAguirreAgudelo [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
