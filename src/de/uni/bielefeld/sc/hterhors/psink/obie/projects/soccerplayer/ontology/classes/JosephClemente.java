...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJosephClemente;
...
public class JosephClemente implements IJosephClemente {
	...
	public JosephClemente(JosephClemente josephClemente) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = josephClemente.getAnnotationID();
		for (int j = 0; j < josephClemente.getBirthPlaces().size(); j++) {
			if (josephClemente.getBirthPlaces().get(j) != null) {
				birthPlaces.add(josephClemente.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(josephClemente.getBirthPlaces().get(j).getClass())
						.newInstance(josephClemente.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (josephClemente.getBirthYear() != null)
			this.birthYear = josephClemente.getBirthYear().getClass()
					.getDeclaredConstructor(josephClemente.getBirthYear().getClass())
					.newInstance(josephClemente.getBirthYear());
		this.characterOffset = josephClemente.getCharacterOffset();
		this.characterOnset = josephClemente.getCharacterOnset();
		if (josephClemente.getPosition() != null)
			this.position = josephClemente.getPosition().getClass()
					.getDeclaredConstructor(josephClemente.getPosition().getClass())
					.newInstance(josephClemente.getPosition());
		for (int j = 0; j < josephClemente.getTeams().size(); j++) {
			if (josephClemente.getTeams().get(j) != null) {
				teams.add(josephClemente.getTeams().get(j).getClass()
						.getDeclaredConstructor(josephClemente.getTeams().get(j).getClass())
						.newInstance(josephClemente.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JosephClemente [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
