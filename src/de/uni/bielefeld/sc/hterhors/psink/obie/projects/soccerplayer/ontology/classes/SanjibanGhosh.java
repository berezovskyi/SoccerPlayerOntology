...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISanjibanGhosh;
...
public class SanjibanGhosh implements ISanjibanGhosh {
	...
	public SanjibanGhosh(SanjibanGhosh sanjibanGhosh) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = sanjibanGhosh.getAnnotationID();
		for (int j = 0; j < sanjibanGhosh.getBirthPlaces().size(); j++) {
			if (sanjibanGhosh.getBirthPlaces().get(j) != null) {
				birthPlaces.add(sanjibanGhosh.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(sanjibanGhosh.getBirthPlaces().get(j).getClass())
						.newInstance(sanjibanGhosh.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (sanjibanGhosh.getBirthYear() != null)
			this.birthYear = sanjibanGhosh.getBirthYear().getClass()
					.getDeclaredConstructor(sanjibanGhosh.getBirthYear().getClass())
					.newInstance(sanjibanGhosh.getBirthYear());
		this.characterOffset = sanjibanGhosh.getCharacterOffset();
		this.characterOnset = sanjibanGhosh.getCharacterOnset();
		if (sanjibanGhosh.getPosition() != null)
			this.position = sanjibanGhosh.getPosition().getClass()
					.getDeclaredConstructor(sanjibanGhosh.getPosition().getClass())
					.newInstance(sanjibanGhosh.getPosition());
		for (int j = 0; j < sanjibanGhosh.getTeams().size(); j++) {
			if (sanjibanGhosh.getTeams().get(j) != null) {
				teams.add(sanjibanGhosh.getTeams().get(j).getClass()
						.getDeclaredConstructor(sanjibanGhosh.getTeams().get(j).getClass())
						.newInstance(sanjibanGhosh.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SanjibanGhosh [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
