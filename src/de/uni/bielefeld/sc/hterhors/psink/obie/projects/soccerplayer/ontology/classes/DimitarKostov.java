...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDimitarKostov;
...
public class DimitarKostov implements IDimitarKostov {
	...
	public DimitarKostov(DimitarKostov dimitarKostov) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = dimitarKostov.getAnnotationID();
		for (int j = 0; j < dimitarKostov.getBirthPlaces().size(); j++) {
			if (dimitarKostov.getBirthPlaces().get(j) != null) {
				birthPlaces.add(dimitarKostov.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(dimitarKostov.getBirthPlaces().get(j).getClass())
						.newInstance(dimitarKostov.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (dimitarKostov.getBirthYear() != null)
			this.birthYear = dimitarKostov.getBirthYear().getClass()
					.getDeclaredConstructor(dimitarKostov.getBirthYear().getClass())
					.newInstance(dimitarKostov.getBirthYear());
		this.characterOffset = dimitarKostov.getCharacterOffset();
		this.characterOnset = dimitarKostov.getCharacterOnset();
		if (dimitarKostov.getPosition() != null)
			this.position = dimitarKostov.getPosition().getClass()
					.getDeclaredConstructor(dimitarKostov.getPosition().getClass())
					.newInstance(dimitarKostov.getPosition());
		for (int j = 0; j < dimitarKostov.getTeams().size(); j++) {
			if (dimitarKostov.getTeams().get(j) != null) {
				teams.add(dimitarKostov.getTeams().get(j).getClass()
						.getDeclaredConstructor(dimitarKostov.getTeams().get(j).getClass())
						.newInstance(dimitarKostov.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DimitarKostov [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
