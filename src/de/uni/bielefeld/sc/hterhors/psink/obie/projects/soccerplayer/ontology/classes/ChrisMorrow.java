...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IChrisMorrow;
...
public class ChrisMorrow implements IChrisMorrow {
	...
	public ChrisMorrow(ChrisMorrow chrisMorrow) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = chrisMorrow.getAnnotationID();
		for (int j = 0; j < chrisMorrow.getBirthPlaces().size(); j++) {
			if (chrisMorrow.getBirthPlaces().get(j) != null) {
				birthPlaces.add(chrisMorrow.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(chrisMorrow.getBirthPlaces().get(j).getClass())
						.newInstance(chrisMorrow.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (chrisMorrow.getBirthYear() != null)
			this.birthYear = chrisMorrow.getBirthYear().getClass()
					.getDeclaredConstructor(chrisMorrow.getBirthYear().getClass())
					.newInstance(chrisMorrow.getBirthYear());
		this.characterOffset = chrisMorrow.getCharacterOffset();
		this.characterOnset = chrisMorrow.getCharacterOnset();
		if (chrisMorrow.getPosition() != null)
			this.position = chrisMorrow.getPosition().getClass()
					.getDeclaredConstructor(chrisMorrow.getPosition().getClass())
					.newInstance(chrisMorrow.getPosition());
		for (int j = 0; j < chrisMorrow.getTeams().size(); j++) {
			if (chrisMorrow.getTeams().get(j) != null) {
				teams.add(chrisMorrow.getTeams().get(j).getClass()
						.getDeclaredConstructor(chrisMorrow.getTeams().get(j).getClass())
						.newInstance(chrisMorrow.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ChrisMorrow [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
