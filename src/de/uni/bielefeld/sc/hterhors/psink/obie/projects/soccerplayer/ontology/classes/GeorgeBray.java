...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IGeorgeBray;
...
public class GeorgeBray implements IGeorgeBray {
	...
	public GeorgeBray(GeorgeBray georgeBray) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = georgeBray.getAnnotationID();
		for (int j = 0; j < georgeBray.getBirthPlaces().size(); j++) {
			if (georgeBray.getBirthPlaces().get(j) != null) {
				birthPlaces.add(georgeBray.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(georgeBray.getBirthPlaces().get(j).getClass())
						.newInstance(georgeBray.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (georgeBray.getBirthYear() != null)
			this.birthYear = georgeBray.getBirthYear().getClass()
					.getDeclaredConstructor(georgeBray.getBirthYear().getClass())
					.newInstance(georgeBray.getBirthYear());
		this.characterOffset = georgeBray.getCharacterOffset();
		this.characterOnset = georgeBray.getCharacterOnset();
		if (georgeBray.getPosition() != null)
			this.position = georgeBray.getPosition().getClass()
					.getDeclaredConstructor(georgeBray.getPosition().getClass()).newInstance(georgeBray.getPosition());
		for (int j = 0; j < georgeBray.getTeams().size(); j++) {
			if (georgeBray.getTeams().get(j) != null) {
				teams.add(georgeBray.getTeams().get(j).getClass()
						.getDeclaredConstructor(georgeBray.getTeams().get(j).getClass())
						.newInstance(georgeBray.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "GeorgeBray [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
