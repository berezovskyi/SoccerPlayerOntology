...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMatthewCoad;
...
public class MatthewCoad implements IMatthewCoad {
	...
	public MatthewCoad(MatthewCoad matthewCoad) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = matthewCoad.getAnnotationID();
		for (int j = 0; j < matthewCoad.getBirthPlaces().size(); j++) {
			if (matthewCoad.getBirthPlaces().get(j) != null) {
				birthPlaces.add(matthewCoad.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(matthewCoad.getBirthPlaces().get(j).getClass())
						.newInstance(matthewCoad.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (matthewCoad.getBirthYear() != null)
			this.birthYear = matthewCoad.getBirthYear().getClass()
					.getDeclaredConstructor(matthewCoad.getBirthYear().getClass())
					.newInstance(matthewCoad.getBirthYear());
		this.characterOffset = matthewCoad.getCharacterOffset();
		this.characterOnset = matthewCoad.getCharacterOnset();
		if (matthewCoad.getPosition() != null)
			this.position = matthewCoad.getPosition().getClass()
					.getDeclaredConstructor(matthewCoad.getPosition().getClass())
					.newInstance(matthewCoad.getPosition());
		for (int j = 0; j < matthewCoad.getTeams().size(); j++) {
			if (matthewCoad.getTeams().get(j) != null) {
				teams.add(matthewCoad.getTeams().get(j).getClass()
						.getDeclaredConstructor(matthewCoad.getTeams().get(j).getClass())
						.newInstance(matthewCoad.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MatthewCoad [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
