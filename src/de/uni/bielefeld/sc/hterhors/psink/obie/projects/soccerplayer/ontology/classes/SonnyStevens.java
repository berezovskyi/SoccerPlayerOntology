...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISonnyStevens;
...
public class SonnyStevens implements ISonnyStevens {
	...
	public SonnyStevens(SonnyStevens sonnyStevens) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = sonnyStevens.getAnnotationID();
		for (int j = 0; j < sonnyStevens.getBirthPlaces().size(); j++) {
			if (sonnyStevens.getBirthPlaces().get(j) != null) {
				birthPlaces.add(sonnyStevens.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(sonnyStevens.getBirthPlaces().get(j).getClass())
						.newInstance(sonnyStevens.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (sonnyStevens.getBirthYear() != null)
			this.birthYear = sonnyStevens.getBirthYear().getClass()
					.getDeclaredConstructor(sonnyStevens.getBirthYear().getClass())
					.newInstance(sonnyStevens.getBirthYear());
		this.characterOffset = sonnyStevens.getCharacterOffset();
		this.characterOnset = sonnyStevens.getCharacterOnset();
		if (sonnyStevens.getPosition() != null)
			this.position = sonnyStevens.getPosition().getClass()
					.getDeclaredConstructor(sonnyStevens.getPosition().getClass())
					.newInstance(sonnyStevens.getPosition());
		for (int j = 0; j < sonnyStevens.getTeams().size(); j++) {
			if (sonnyStevens.getTeams().get(j) != null) {
				teams.add(sonnyStevens.getTeams().get(j).getClass()
						.getDeclaredConstructor(sonnyStevens.getTeams().get(j).getClass())
						.newInstance(sonnyStevens.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
