...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISamirSaid;
...
public class SamirSaid implements ISamirSaid {
	...
	public SamirSaid(SamirSaid samirSaid) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = samirSaid.getAnnotationID();
		for (int j = 0; j < samirSaid.getBirthPlaces().size(); j++) {
			if (samirSaid.getBirthPlaces().get(j) != null) {
				birthPlaces.add(samirSaid.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(samirSaid.getBirthPlaces().get(j).getClass())
						.newInstance(samirSaid.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (samirSaid.getBirthYear() != null)
			this.birthYear = samirSaid.getBirthYear().getClass()
					.getDeclaredConstructor(samirSaid.getBirthYear().getClass()).newInstance(samirSaid.getBirthYear());
		this.characterOffset = samirSaid.getCharacterOffset();
		this.characterOnset = samirSaid.getCharacterOnset();
		if (samirSaid.getPosition() != null)
			this.position = samirSaid.getPosition().getClass()
					.getDeclaredConstructor(samirSaid.getPosition().getClass()).newInstance(samirSaid.getPosition());
		for (int j = 0; j < samirSaid.getTeams().size(); j++) {
			if (samirSaid.getTeams().get(j) != null) {
				teams.add(samirSaid.getTeams().get(j).getClass()
						.getDeclaredConstructor(samirSaid.getTeams().get(j).getClass())
						.newInstance(samirSaid.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SamirSaid [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
