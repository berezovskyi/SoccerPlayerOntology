...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBenYagan;
...
public class BenYagan implements IBenYagan {
	...
	public BenYagan(BenYagan benYagan) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = benYagan.getAnnotationID();
		for (int j = 0; j < benYagan.getBirthPlaces().size(); j++) {
			if (benYagan.getBirthPlaces().get(j) != null) {
				birthPlaces.add(benYagan.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(benYagan.getBirthPlaces().get(j).getClass())
						.newInstance(benYagan.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (benYagan.getBirthYear() != null)
			this.birthYear = benYagan.getBirthYear().getClass()
					.getDeclaredConstructor(benYagan.getBirthYear().getClass()).newInstance(benYagan.getBirthYear());
		this.characterOffset = benYagan.getCharacterOffset();
		this.characterOnset = benYagan.getCharacterOnset();
		if (benYagan.getPosition() != null)
			this.position = benYagan.getPosition().getClass().getDeclaredConstructor(benYagan.getPosition().getClass())
					.newInstance(benYagan.getPosition());
		for (int j = 0; j < benYagan.getTeams().size(); j++) {
			if (benYagan.getTeams().get(j) != null) {
				teams.add(benYagan.getTeams().get(j).getClass()
						.getDeclaredConstructor(benYagan.getTeams().get(j).getClass())
						.newInstance(benYagan.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BenYagan [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
