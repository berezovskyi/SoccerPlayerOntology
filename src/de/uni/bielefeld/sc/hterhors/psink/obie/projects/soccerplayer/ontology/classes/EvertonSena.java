...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IEvertonSena;
...
public class EvertonSena implements IEvertonSena {
	...
	public EvertonSena(EvertonSena evertonSena) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = evertonSena.getAnnotationID();
		for (int j = 0; j < evertonSena.getBirthPlaces().size(); j++) {
			if (evertonSena.getBirthPlaces().get(j) != null) {
				birthPlaces.add(evertonSena.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(evertonSena.getBirthPlaces().get(j).getClass())
						.newInstance(evertonSena.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (evertonSena.getBirthYear() != null)
			this.birthYear = evertonSena.getBirthYear().getClass()
					.getDeclaredConstructor(evertonSena.getBirthYear().getClass())
					.newInstance(evertonSena.getBirthYear());
		this.characterOffset = evertonSena.getCharacterOffset();
		this.characterOnset = evertonSena.getCharacterOnset();
		if (evertonSena.getPosition() != null)
			this.position = evertonSena.getPosition().getClass()
					.getDeclaredConstructor(evertonSena.getPosition().getClass())
					.newInstance(evertonSena.getPosition());
		for (int j = 0; j < evertonSena.getTeams().size(); j++) {
			if (evertonSena.getTeams().get(j) != null) {
				teams.add(evertonSena.getTeams().get(j).getClass()
						.getDeclaredConstructor(evertonSena.getTeams().get(j).getClass())
						.newInstance(evertonSena.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "EvertonSena [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
