...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IHugoVillanueva;
...
public class HugoVillanueva implements IHugoVillanueva {
	...
	public HugoVillanueva(HugoVillanueva hugoVillanueva) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = hugoVillanueva.getAnnotationID();
		for (int j = 0; j < hugoVillanueva.getBirthPlaces().size(); j++) {
			if (hugoVillanueva.getBirthPlaces().get(j) != null) {
				birthPlaces.add(hugoVillanueva.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(hugoVillanueva.getBirthPlaces().get(j).getClass())
						.newInstance(hugoVillanueva.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (hugoVillanueva.getBirthYear() != null)
			this.birthYear = hugoVillanueva.getBirthYear().getClass()
					.getDeclaredConstructor(hugoVillanueva.getBirthYear().getClass())
					.newInstance(hugoVillanueva.getBirthYear());
		this.characterOffset = hugoVillanueva.getCharacterOffset();
		this.characterOnset = hugoVillanueva.getCharacterOnset();
		if (hugoVillanueva.getPosition() != null)
			this.position = hugoVillanueva.getPosition().getClass()
					.getDeclaredConstructor(hugoVillanueva.getPosition().getClass())
					.newInstance(hugoVillanueva.getPosition());
		for (int j = 0; j < hugoVillanueva.getTeams().size(); j++) {
			if (hugoVillanueva.getTeams().get(j) != null) {
				teams.add(hugoVillanueva.getTeams().get(j).getClass()
						.getDeclaredConstructor(hugoVillanueva.getTeams().get(j).getClass())
						.newInstance(hugoVillanueva.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "HugoVillanueva [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
