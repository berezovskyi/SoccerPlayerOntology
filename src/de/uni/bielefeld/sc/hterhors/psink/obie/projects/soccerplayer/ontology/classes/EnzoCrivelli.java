...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IEnzoCrivelli;
...
public class EnzoCrivelli implements IEnzoCrivelli {
	...
	public EnzoCrivelli(EnzoCrivelli enzoCrivelli) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = enzoCrivelli.getAnnotationID();
		for (int j = 0; j < enzoCrivelli.getBirthPlaces().size(); j++) {
			if (enzoCrivelli.getBirthPlaces().get(j) != null) {
				birthPlaces.add(enzoCrivelli.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(enzoCrivelli.getBirthPlaces().get(j).getClass())
						.newInstance(enzoCrivelli.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (enzoCrivelli.getBirthYear() != null)
			this.birthYear = enzoCrivelli.getBirthYear().getClass()
					.getDeclaredConstructor(enzoCrivelli.getBirthYear().getClass())
					.newInstance(enzoCrivelli.getBirthYear());
		this.characterOffset = enzoCrivelli.getCharacterOffset();
		this.characterOnset = enzoCrivelli.getCharacterOnset();
		if (enzoCrivelli.getPosition() != null)
			this.position = enzoCrivelli.getPosition().getClass()
					.getDeclaredConstructor(enzoCrivelli.getPosition().getClass())
					.newInstance(enzoCrivelli.getPosition());
		for (int j = 0; j < enzoCrivelli.getTeams().size(); j++) {
			if (enzoCrivelli.getTeams().get(j) != null) {
				teams.add(enzoCrivelli.getTeams().get(j).getClass()
						.getDeclaredConstructor(enzoCrivelli.getTeams().get(j).getClass())
						.newInstance(enzoCrivelli.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "EnzoCrivelli [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
