...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBiswajitBiswas;
...
public class BiswajitBiswas implements IBiswajitBiswas {
	...
	public BiswajitBiswas(BiswajitBiswas biswajitBiswas) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = biswajitBiswas.getAnnotationID();
		for (int j = 0; j < biswajitBiswas.getBirthPlaces().size(); j++) {
			if (biswajitBiswas.getBirthPlaces().get(j) != null) {
				birthPlaces.add(biswajitBiswas.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(biswajitBiswas.getBirthPlaces().get(j).getClass())
						.newInstance(biswajitBiswas.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (biswajitBiswas.getBirthYear() != null)
			this.birthYear = biswajitBiswas.getBirthYear().getClass()
					.getDeclaredConstructor(biswajitBiswas.getBirthYear().getClass())
					.newInstance(biswajitBiswas.getBirthYear());
		this.characterOffset = biswajitBiswas.getCharacterOffset();
		this.characterOnset = biswajitBiswas.getCharacterOnset();
		if (biswajitBiswas.getPosition() != null)
			this.position = biswajitBiswas.getPosition().getClass()
					.getDeclaredConstructor(biswajitBiswas.getPosition().getClass())
					.newInstance(biswajitBiswas.getPosition());
		for (int j = 0; j < biswajitBiswas.getTeams().size(); j++) {
			if (biswajitBiswas.getTeams().get(j) != null) {
				teams.add(biswajitBiswas.getTeams().get(j).getClass()
						.getDeclaredConstructor(biswajitBiswas.getTeams().get(j).getClass())
						.newInstance(biswajitBiswas.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BiswajitBiswas [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
