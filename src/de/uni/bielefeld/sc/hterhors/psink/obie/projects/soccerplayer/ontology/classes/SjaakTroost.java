...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISjaakTroost;
...
public class SjaakTroost implements ISjaakTroost {
	...
	public SjaakTroost(SjaakTroost sjaakTroost) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = sjaakTroost.getAnnotationID();
		for (int j = 0; j < sjaakTroost.getBirthPlaces().size(); j++) {
			if (sjaakTroost.getBirthPlaces().get(j) != null) {
				birthPlaces.add(sjaakTroost.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(sjaakTroost.getBirthPlaces().get(j).getClass())
						.newInstance(sjaakTroost.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (sjaakTroost.getBirthYear() != null)
			this.birthYear = sjaakTroost.getBirthYear().getClass()
					.getDeclaredConstructor(sjaakTroost.getBirthYear().getClass())
					.newInstance(sjaakTroost.getBirthYear());
		this.characterOffset = sjaakTroost.getCharacterOffset();
		this.characterOnset = sjaakTroost.getCharacterOnset();
		if (sjaakTroost.getPosition() != null)
			this.position = sjaakTroost.getPosition().getClass()
					.getDeclaredConstructor(sjaakTroost.getPosition().getClass())
					.newInstance(sjaakTroost.getPosition());
		for (int j = 0; j < sjaakTroost.getTeams().size(); j++) {
			if (sjaakTroost.getTeams().get(j) != null) {
				teams.add(sjaakTroost.getTeams().get(j).getClass()
						.getDeclaredConstructor(sjaakTroost.getTeams().get(j).getClass())
						.newInstance(sjaakTroost.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SjaakTroost [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
