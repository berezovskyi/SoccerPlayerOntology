...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAndreAnis;
...
public class AndreAnis implements IAndreAnis {
	...
	public AndreAnis(AndreAnis andreAnis) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = andreAnis.getAnnotationID();
		for (int j = 0; j < andreAnis.getBirthPlaces().size(); j++) {
			if (andreAnis.getBirthPlaces().get(j) != null) {
				birthPlaces.add(andreAnis.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(andreAnis.getBirthPlaces().get(j).getClass())
						.newInstance(andreAnis.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (andreAnis.getBirthYear() != null)
			this.birthYear = andreAnis.getBirthYear().getClass()
					.getDeclaredConstructor(andreAnis.getBirthYear().getClass()).newInstance(andreAnis.getBirthYear());
		this.characterOffset = andreAnis.getCharacterOffset();
		this.characterOnset = andreAnis.getCharacterOnset();
		if (andreAnis.getPosition() != null)
			this.position = andreAnis.getPosition().getClass()
					.getDeclaredConstructor(andreAnis.getPosition().getClass()).newInstance(andreAnis.getPosition());
		for (int j = 0; j < andreAnis.getTeams().size(); j++) {
			if (andreAnis.getTeams().get(j) != null) {
				teams.add(andreAnis.getTeams().get(j).getClass()
						.getDeclaredConstructor(andreAnis.getTeams().get(j).getClass())
						.newInstance(andreAnis.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AndreAnis [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
