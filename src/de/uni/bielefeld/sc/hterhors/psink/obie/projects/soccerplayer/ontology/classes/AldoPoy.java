...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAldoPoy;
...
public class AldoPoy implements IAldoPoy {
	...
	public AldoPoy(AldoPoy aldoPoy) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = aldoPoy.getAnnotationID();
		for (int j = 0; j < aldoPoy.getBirthPlaces().size(); j++) {
			if (aldoPoy.getBirthPlaces().get(j) != null) {
				birthPlaces.add(aldoPoy.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(aldoPoy.getBirthPlaces().get(j).getClass())
						.newInstance(aldoPoy.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (aldoPoy.getBirthYear() != null)
			this.birthYear = aldoPoy.getBirthYear().getClass().getDeclaredConstructor(aldoPoy.getBirthYear().getClass())
					.newInstance(aldoPoy.getBirthYear());
		this.characterOffset = aldoPoy.getCharacterOffset();
		this.characterOnset = aldoPoy.getCharacterOnset();
		if (aldoPoy.getPosition() != null)
			this.position = aldoPoy.getPosition().getClass().getDeclaredConstructor(aldoPoy.getPosition().getClass())
					.newInstance(aldoPoy.getPosition());
		for (int j = 0; j < aldoPoy.getTeams().size(); j++) {
			if (aldoPoy.getTeams().get(j) != null) {
				teams.add(aldoPoy.getTeams().get(j).getClass()
						.getDeclaredConstructor(aldoPoy.getTeams().get(j).getClass())
						.newInstance(aldoPoy.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AldoPoy [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
