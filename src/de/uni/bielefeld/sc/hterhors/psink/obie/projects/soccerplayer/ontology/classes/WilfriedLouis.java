...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IWilfriedLouis;
...
public class WilfriedLouis implements IWilfriedLouis {
	...
	public WilfriedLouis(WilfriedLouis wilfriedLouis) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = wilfriedLouis.getAnnotationID();
		for (int j = 0; j < wilfriedLouis.getBirthPlaces().size(); j++) {
			if (wilfriedLouis.getBirthPlaces().get(j) != null) {
				birthPlaces.add(wilfriedLouis.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(wilfriedLouis.getBirthPlaces().get(j).getClass())
						.newInstance(wilfriedLouis.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (wilfriedLouis.getBirthYear() != null)
			this.birthYear = wilfriedLouis.getBirthYear().getClass()
					.getDeclaredConstructor(wilfriedLouis.getBirthYear().getClass())
					.newInstance(wilfriedLouis.getBirthYear());
		this.characterOffset = wilfriedLouis.getCharacterOffset();
		this.characterOnset = wilfriedLouis.getCharacterOnset();
		if (wilfriedLouis.getPosition() != null)
			this.position = wilfriedLouis.getPosition().getClass()
					.getDeclaredConstructor(wilfriedLouis.getPosition().getClass())
					.newInstance(wilfriedLouis.getPosition());
		for (int j = 0; j < wilfriedLouis.getTeams().size(); j++) {
			if (wilfriedLouis.getTeams().get(j) != null) {
				teams.add(wilfriedLouis.getTeams().get(j).getClass()
						.getDeclaredConstructor(wilfriedLouis.getTeams().get(j).getClass())
						.newInstance(wilfriedLouis.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "WilfriedLouis [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
