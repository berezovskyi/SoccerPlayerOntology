...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAlbertEvers;
...
public class AlbertEvers implements IAlbertEvers {
	...
	public AlbertEvers(AlbertEvers albertEvers) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = albertEvers.getAnnotationID();
		for (int j = 0; j < albertEvers.getBirthPlaces().size(); j++) {
			if (albertEvers.getBirthPlaces().get(j) != null) {
				birthPlaces.add(albertEvers.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(albertEvers.getBirthPlaces().get(j).getClass())
						.newInstance(albertEvers.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (albertEvers.getBirthYear() != null)
			this.birthYear = albertEvers.getBirthYear().getClass()
					.getDeclaredConstructor(albertEvers.getBirthYear().getClass())
					.newInstance(albertEvers.getBirthYear());
		this.characterOffset = albertEvers.getCharacterOffset();
		this.characterOnset = albertEvers.getCharacterOnset();
		if (albertEvers.getPosition() != null)
			this.position = albertEvers.getPosition().getClass()
					.getDeclaredConstructor(albertEvers.getPosition().getClass())
					.newInstance(albertEvers.getPosition());
		for (int j = 0; j < albertEvers.getTeams().size(); j++) {
			if (albertEvers.getTeams().get(j) != null) {
				teams.add(albertEvers.getTeams().get(j).getClass()
						.getDeclaredConstructor(albertEvers.getTeams().get(j).getClass())
						.newInstance(albertEvers.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AlbertEvers [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
