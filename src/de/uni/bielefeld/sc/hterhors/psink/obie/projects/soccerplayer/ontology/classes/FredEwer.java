...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IFredEwer;
...
public class FredEwer implements IFredEwer {
	...
	public FredEwer(FredEwer fredEwer) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = fredEwer.getAnnotationID();
		for (int j = 0; j < fredEwer.getBirthPlaces().size(); j++) {
			if (fredEwer.getBirthPlaces().get(j) != null) {
				birthPlaces.add(fredEwer.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(fredEwer.getBirthPlaces().get(j).getClass())
						.newInstance(fredEwer.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (fredEwer.getBirthYear() != null)
			this.birthYear = fredEwer.getBirthYear().getClass()
					.getDeclaredConstructor(fredEwer.getBirthYear().getClass()).newInstance(fredEwer.getBirthYear());
		this.characterOffset = fredEwer.getCharacterOffset();
		this.characterOnset = fredEwer.getCharacterOnset();
		if (fredEwer.getPosition() != null)
			this.position = fredEwer.getPosition().getClass().getDeclaredConstructor(fredEwer.getPosition().getClass())
					.newInstance(fredEwer.getPosition());
		for (int j = 0; j < fredEwer.getTeams().size(); j++) {
			if (fredEwer.getTeams().get(j) != null) {
				teams.add(fredEwer.getTeams().get(j).getClass()
						.getDeclaredConstructor(fredEwer.getTeams().get(j).getClass())
						.newInstance(fredEwer.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "FredEwer [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
