...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRoddieMackenzie;
...
public class RoddieMackenzie implements IRoddieMackenzie {
	...
	public RoddieMackenzie(RoddieMackenzie roddieMackenzie) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = roddieMackenzie.getAnnotationID();
		for (int j = 0; j < roddieMackenzie.getBirthPlaces().size(); j++) {
			if (roddieMackenzie.getBirthPlaces().get(j) != null) {
				birthPlaces.add(roddieMackenzie.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(roddieMackenzie.getBirthPlaces().get(j).getClass())
						.newInstance(roddieMackenzie.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (roddieMackenzie.getBirthYear() != null)
			this.birthYear = roddieMackenzie.getBirthYear().getClass()
					.getDeclaredConstructor(roddieMackenzie.getBirthYear().getClass())
					.newInstance(roddieMackenzie.getBirthYear());
		this.characterOffset = roddieMackenzie.getCharacterOffset();
		this.characterOnset = roddieMackenzie.getCharacterOnset();
		if (roddieMackenzie.getPosition() != null)
			this.position = roddieMackenzie.getPosition().getClass()
					.getDeclaredConstructor(roddieMackenzie.getPosition().getClass())
					.newInstance(roddieMackenzie.getPosition());
		for (int j = 0; j < roddieMackenzie.getTeams().size(); j++) {
			if (roddieMackenzie.getTeams().get(j) != null) {
				teams.add(roddieMackenzie.getTeams().get(j).getClass()
						.getDeclaredConstructor(roddieMackenzie.getTeams().get(j).getClass())
						.newInstance(roddieMackenzie.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RoddieMackenzie [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
