...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJuanAlfonsoValle;
...
public class JuanAlfonsoValle implements IJuanAlfonsoValle {
	...
	public JuanAlfonsoValle(JuanAlfonsoValle juanAlfonsoValle) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = juanAlfonsoValle.getAnnotationID();
		for (int j = 0; j < juanAlfonsoValle.getBirthPlaces().size(); j++) {
			if (juanAlfonsoValle.getBirthPlaces().get(j) != null) {
				birthPlaces.add(juanAlfonsoValle.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(juanAlfonsoValle.getBirthPlaces().get(j).getClass())
						.newInstance(juanAlfonsoValle.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (juanAlfonsoValle.getBirthYear() != null)
			this.birthYear = juanAlfonsoValle.getBirthYear().getClass()
					.getDeclaredConstructor(juanAlfonsoValle.getBirthYear().getClass())
					.newInstance(juanAlfonsoValle.getBirthYear());
		this.characterOffset = juanAlfonsoValle.getCharacterOffset();
		this.characterOnset = juanAlfonsoValle.getCharacterOnset();
		if (juanAlfonsoValle.getPosition() != null)
			this.position = juanAlfonsoValle.getPosition().getClass()
					.getDeclaredConstructor(juanAlfonsoValle.getPosition().getClass())
					.newInstance(juanAlfonsoValle.getPosition());
		for (int j = 0; j < juanAlfonsoValle.getTeams().size(); j++) {
			if (juanAlfonsoValle.getTeams().get(j) != null) {
				teams.add(juanAlfonsoValle.getTeams().get(j).getClass()
						.getDeclaredConstructor(juanAlfonsoValle.getTeams().get(j).getClass())
						.newInstance(juanAlfonsoValle.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JuanAlfonsoValle [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
