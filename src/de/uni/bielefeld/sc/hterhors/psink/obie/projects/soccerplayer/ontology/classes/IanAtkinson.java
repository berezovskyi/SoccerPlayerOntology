...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IIanAtkinson;
...
public class IanAtkinson implements IIanAtkinson {
	...
	public IanAtkinson(IanAtkinson ianAtkinson) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = ianAtkinson.getAnnotationID();
		for (int j = 0; j < ianAtkinson.getBirthPlaces().size(); j++) {
			if (ianAtkinson.getBirthPlaces().get(j) != null) {
				birthPlaces.add(ianAtkinson.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(ianAtkinson.getBirthPlaces().get(j).getClass())
						.newInstance(ianAtkinson.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (ianAtkinson.getBirthYear() != null)
			this.birthYear = ianAtkinson.getBirthYear().getClass()
					.getDeclaredConstructor(ianAtkinson.getBirthYear().getClass())
					.newInstance(ianAtkinson.getBirthYear());
		this.characterOffset = ianAtkinson.getCharacterOffset();
		this.characterOnset = ianAtkinson.getCharacterOnset();
		if (ianAtkinson.getPosition() != null)
			this.position = ianAtkinson.getPosition().getClass()
					.getDeclaredConstructor(ianAtkinson.getPosition().getClass())
					.newInstance(ianAtkinson.getPosition());
		for (int j = 0; j < ianAtkinson.getTeams().size(); j++) {
			if (ianAtkinson.getTeams().get(j) != null) {
				teams.add(ianAtkinson.getTeams().get(j).getClass()
						.getDeclaredConstructor(ianAtkinson.getTeams().get(j).getClass())
						.newInstance(ianAtkinson.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "IanAtkinson [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
