...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRedouaneDrici;
...
public class RedouaneDrici implements IRedouaneDrici {
	...
	public RedouaneDrici(RedouaneDrici redouaneDrici) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = redouaneDrici.getAnnotationID();
		for (int j = 0; j < redouaneDrici.getBirthPlaces().size(); j++) {
			if (redouaneDrici.getBirthPlaces().get(j) != null) {
				birthPlaces.add(redouaneDrici.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(redouaneDrici.getBirthPlaces().get(j).getClass())
						.newInstance(redouaneDrici.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (redouaneDrici.getBirthYear() != null)
			this.birthYear = redouaneDrici.getBirthYear().getClass()
					.getDeclaredConstructor(redouaneDrici.getBirthYear().getClass())
					.newInstance(redouaneDrici.getBirthYear());
		this.characterOffset = redouaneDrici.getCharacterOffset();
		this.characterOnset = redouaneDrici.getCharacterOnset();
		if (redouaneDrici.getPosition() != null)
			this.position = redouaneDrici.getPosition().getClass()
					.getDeclaredConstructor(redouaneDrici.getPosition().getClass())
					.newInstance(redouaneDrici.getPosition());
		for (int j = 0; j < redouaneDrici.getTeams().size(); j++) {
			if (redouaneDrici.getTeams().get(j) != null) {
				teams.add(redouaneDrici.getTeams().get(j).getClass()
						.getDeclaredConstructor(redouaneDrici.getTeams().get(j).getClass())
						.newInstance(redouaneDrici.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RedouaneDrici [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
