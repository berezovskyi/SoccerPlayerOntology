...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IIvarEriksson;
...
public class IvarEriksson implements IIvarEriksson {
	...
	public IvarEriksson(IvarEriksson ivarEriksson) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = ivarEriksson.getAnnotationID();
		for (int j = 0; j < ivarEriksson.getBirthPlaces().size(); j++) {
			if (ivarEriksson.getBirthPlaces().get(j) != null) {
				birthPlaces.add(ivarEriksson.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(ivarEriksson.getBirthPlaces().get(j).getClass())
						.newInstance(ivarEriksson.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (ivarEriksson.getBirthYear() != null)
			this.birthYear = ivarEriksson.getBirthYear().getClass()
					.getDeclaredConstructor(ivarEriksson.getBirthYear().getClass())
					.newInstance(ivarEriksson.getBirthYear());
		this.characterOffset = ivarEriksson.getCharacterOffset();
		this.characterOnset = ivarEriksson.getCharacterOnset();
		if (ivarEriksson.getPosition() != null)
			this.position = ivarEriksson.getPosition().getClass()
					.getDeclaredConstructor(ivarEriksson.getPosition().getClass())
					.newInstance(ivarEriksson.getPosition());
		for (int j = 0; j < ivarEriksson.getTeams().size(); j++) {
			if (ivarEriksson.getTeams().get(j) != null) {
				teams.add(ivarEriksson.getTeams().get(j).getClass()
						.getDeclaredConstructor(ivarEriksson.getTeams().get(j).getClass())
						.newInstance(ivarEriksson.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "IvarEriksson [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
