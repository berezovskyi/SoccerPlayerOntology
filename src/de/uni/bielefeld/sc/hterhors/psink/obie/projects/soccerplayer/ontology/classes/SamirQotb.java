...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISamirQotb;
...
public class SamirQotb implements ISamirQotb {
	...
	public SamirQotb(SamirQotb samirQotb) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = samirQotb.getAnnotationID();
		for (int j = 0; j < samirQotb.getBirthPlaces().size(); j++) {
			if (samirQotb.getBirthPlaces().get(j) != null) {
				birthPlaces.add(samirQotb.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(samirQotb.getBirthPlaces().get(j).getClass())
						.newInstance(samirQotb.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (samirQotb.getBirthYear() != null)
			this.birthYear = samirQotb.getBirthYear().getClass()
					.getDeclaredConstructor(samirQotb.getBirthYear().getClass()).newInstance(samirQotb.getBirthYear());
		this.characterOffset = samirQotb.getCharacterOffset();
		this.characterOnset = samirQotb.getCharacterOnset();
		if (samirQotb.getPosition() != null)
			this.position = samirQotb.getPosition().getClass()
					.getDeclaredConstructor(samirQotb.getPosition().getClass()).newInstance(samirQotb.getPosition());
		for (int j = 0; j < samirQotb.getTeams().size(); j++) {
			if (samirQotb.getTeams().get(j) != null) {
				teams.add(samirQotb.getTeams().get(j).getClass()
						.getDeclaredConstructor(samirQotb.getTeams().get(j).getClass())
						.newInstance(samirQotb.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SamirQotb [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
