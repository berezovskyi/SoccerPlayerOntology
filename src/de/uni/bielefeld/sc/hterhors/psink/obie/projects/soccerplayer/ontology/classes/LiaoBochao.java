...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILiaoBochao;
...
public class LiaoBochao implements ILiaoBochao {
	...
	public LiaoBochao(LiaoBochao liaoBochao) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = liaoBochao.getAnnotationID();
		for (int j = 0; j < liaoBochao.getBirthPlaces().size(); j++) {
			if (liaoBochao.getBirthPlaces().get(j) != null) {
				birthPlaces.add(liaoBochao.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(liaoBochao.getBirthPlaces().get(j).getClass())
						.newInstance(liaoBochao.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (liaoBochao.getBirthYear() != null)
			this.birthYear = liaoBochao.getBirthYear().getClass()
					.getDeclaredConstructor(liaoBochao.getBirthYear().getClass())
					.newInstance(liaoBochao.getBirthYear());
		this.characterOffset = liaoBochao.getCharacterOffset();
		this.characterOnset = liaoBochao.getCharacterOnset();
		if (liaoBochao.getPosition() != null)
			this.position = liaoBochao.getPosition().getClass()
					.getDeclaredConstructor(liaoBochao.getPosition().getClass()).newInstance(liaoBochao.getPosition());
		for (int j = 0; j < liaoBochao.getTeams().size(); j++) {
			if (liaoBochao.getTeams().get(j) != null) {
				teams.add(liaoBochao.getTeams().get(j).getClass()
						.getDeclaredConstructor(liaoBochao.getTeams().get(j).getClass())
						.newInstance(liaoBochao.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LiaoBochao [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
