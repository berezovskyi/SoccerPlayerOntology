...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IZeferinoMartins;
...
public class ZeferinoMartins implements IZeferinoMartins {
	...
	public ZeferinoMartins(ZeferinoMartins zeferinoMartins) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = zeferinoMartins.getAnnotationID();
		for (int j = 0; j < zeferinoMartins.getBirthPlaces().size(); j++) {
			if (zeferinoMartins.getBirthPlaces().get(j) != null) {
				birthPlaces.add(zeferinoMartins.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(zeferinoMartins.getBirthPlaces().get(j).getClass())
						.newInstance(zeferinoMartins.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (zeferinoMartins.getBirthYear() != null)
			this.birthYear = zeferinoMartins.getBirthYear().getClass()
					.getDeclaredConstructor(zeferinoMartins.getBirthYear().getClass())
					.newInstance(zeferinoMartins.getBirthYear());
		this.characterOffset = zeferinoMartins.getCharacterOffset();
		this.characterOnset = zeferinoMartins.getCharacterOnset();
		if (zeferinoMartins.getPosition() != null)
			this.position = zeferinoMartins.getPosition().getClass()
					.getDeclaredConstructor(zeferinoMartins.getPosition().getClass())
					.newInstance(zeferinoMartins.getPosition());
		for (int j = 0; j < zeferinoMartins.getTeams().size(); j++) {
			if (zeferinoMartins.getTeams().get(j) != null) {
				teams.add(zeferinoMartins.getTeams().get(j).getClass()
						.getDeclaredConstructor(zeferinoMartins.getTeams().get(j).getClass())
						.newInstance(zeferinoMartins.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ZeferinoMartins [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
