...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILeeChangMyung;
...
public class LeeChangMyung implements ILeeChangMyung {
	...
	public LeeChangMyung(LeeChangMyung leeChangMyung) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = leeChangMyung.getAnnotationID();
		for (int j = 0; j < leeChangMyung.getBirthPlaces().size(); j++) {
			if (leeChangMyung.getBirthPlaces().get(j) != null) {
				birthPlaces.add(leeChangMyung.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(leeChangMyung.getBirthPlaces().get(j).getClass())
						.newInstance(leeChangMyung.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (leeChangMyung.getBirthYear() != null)
			this.birthYear = leeChangMyung.getBirthYear().getClass()
					.getDeclaredConstructor(leeChangMyung.getBirthYear().getClass())
					.newInstance(leeChangMyung.getBirthYear());
		this.characterOffset = leeChangMyung.getCharacterOffset();
		this.characterOnset = leeChangMyung.getCharacterOnset();
		if (leeChangMyung.getPosition() != null)
			this.position = leeChangMyung.getPosition().getClass()
					.getDeclaredConstructor(leeChangMyung.getPosition().getClass())
					.newInstance(leeChangMyung.getPosition());
		for (int j = 0; j < leeChangMyung.getTeams().size(); j++) {
			if (leeChangMyung.getTeams().get(j) != null) {
				teams.add(leeChangMyung.getTeams().get(j).getClass()
						.getDeclaredConstructor(leeChangMyung.getTeams().get(j).getClass())
						.newInstance(leeChangMyung.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LeeChangMyung [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
