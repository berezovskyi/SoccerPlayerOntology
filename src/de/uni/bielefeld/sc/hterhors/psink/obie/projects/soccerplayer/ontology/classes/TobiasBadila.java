...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ITobiasBadila;
...
public class TobiasBadila implements ITobiasBadila {
	...
	public TobiasBadila(TobiasBadila tobiasBadila) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = tobiasBadila.getAnnotationID();
		for (int j = 0; j < tobiasBadila.getBirthPlaces().size(); j++) {
			if (tobiasBadila.getBirthPlaces().get(j) != null) {
				birthPlaces.add(tobiasBadila.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(tobiasBadila.getBirthPlaces().get(j).getClass())
						.newInstance(tobiasBadila.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (tobiasBadila.getBirthYear() != null)
			this.birthYear = tobiasBadila.getBirthYear().getClass()
					.getDeclaredConstructor(tobiasBadila.getBirthYear().getClass())
					.newInstance(tobiasBadila.getBirthYear());
		this.characterOffset = tobiasBadila.getCharacterOffset();
		this.characterOnset = tobiasBadila.getCharacterOnset();
		if (tobiasBadila.getPosition() != null)
			this.position = tobiasBadila.getPosition().getClass()
					.getDeclaredConstructor(tobiasBadila.getPosition().getClass())
					.newInstance(tobiasBadila.getPosition());
		for (int j = 0; j < tobiasBadila.getTeams().size(); j++) {
			if (tobiasBadila.getTeams().get(j) != null) {
				teams.add(tobiasBadila.getTeams().get(j).getClass()
						.getDeclaredConstructor(tobiasBadila.getTeams().get(j).getClass())
						.newInstance(tobiasBadila.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "TobiasBadila [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
