...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAlbertIremonger;
...
public class AlbertIremonger implements IAlbertIremonger {
	...
	public AlbertIremonger(AlbertIremonger albertIremonger) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = albertIremonger.getAnnotationID();
		for (int j = 0; j < albertIremonger.getBirthPlaces().size(); j++) {
			if (albertIremonger.getBirthPlaces().get(j) != null) {
				birthPlaces.add(albertIremonger.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(albertIremonger.getBirthPlaces().get(j).getClass())
						.newInstance(albertIremonger.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (albertIremonger.getBirthYear() != null)
			this.birthYear = albertIremonger.getBirthYear().getClass()
					.getDeclaredConstructor(albertIremonger.getBirthYear().getClass())
					.newInstance(albertIremonger.getBirthYear());
		this.characterOffset = albertIremonger.getCharacterOffset();
		this.characterOnset = albertIremonger.getCharacterOnset();
		if (albertIremonger.getPosition() != null)
			this.position = albertIremonger.getPosition().getClass()
					.getDeclaredConstructor(albertIremonger.getPosition().getClass())
					.newInstance(albertIremonger.getPosition());
		for (int j = 0; j < albertIremonger.getTeams().size(); j++) {
			if (albertIremonger.getTeams().get(j) != null) {
				teams.add(albertIremonger.getTeams().get(j).getClass()
						.getDeclaredConstructor(albertIremonger.getTeams().get(j).getClass())
						.newInstance(albertIremonger.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AlbertIremonger [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
