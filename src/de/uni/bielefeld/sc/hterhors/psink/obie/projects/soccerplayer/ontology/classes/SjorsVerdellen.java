...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISjorsVerdellen;
...
public class SjorsVerdellen implements ISjorsVerdellen {
	...
	public SjorsVerdellen(SjorsVerdellen sjorsVerdellen) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = sjorsVerdellen.getAnnotationID();
		for (int j = 0; j < sjorsVerdellen.getBirthPlaces().size(); j++) {
			if (sjorsVerdellen.getBirthPlaces().get(j) != null) {
				birthPlaces.add(sjorsVerdellen.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(sjorsVerdellen.getBirthPlaces().get(j).getClass())
						.newInstance(sjorsVerdellen.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (sjorsVerdellen.getBirthYear() != null)
			this.birthYear = sjorsVerdellen.getBirthYear().getClass()
					.getDeclaredConstructor(sjorsVerdellen.getBirthYear().getClass())
					.newInstance(sjorsVerdellen.getBirthYear());
		this.characterOffset = sjorsVerdellen.getCharacterOffset();
		this.characterOnset = sjorsVerdellen.getCharacterOnset();
		if (sjorsVerdellen.getPosition() != null)
			this.position = sjorsVerdellen.getPosition().getClass()
					.getDeclaredConstructor(sjorsVerdellen.getPosition().getClass())
					.newInstance(sjorsVerdellen.getPosition());
		for (int j = 0; j < sjorsVerdellen.getTeams().size(); j++) {
			if (sjorsVerdellen.getTeams().get(j) != null) {
				teams.add(sjorsVerdellen.getTeams().get(j).getClass()
						.getDeclaredConstructor(sjorsVerdellen.getTeams().get(j).getClass())
						.newInstance(sjorsVerdellen.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SjorsVerdellen [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
