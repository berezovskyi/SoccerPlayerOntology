...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMiguelSegura;
...
public class MiguelSegura implements IMiguelSegura {
	...
	public MiguelSegura(MiguelSegura miguelSegura) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = miguelSegura.getAnnotationID();
		for (int j = 0; j < miguelSegura.getBirthPlaces().size(); j++) {
			if (miguelSegura.getBirthPlaces().get(j) != null) {
				birthPlaces.add(miguelSegura.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(miguelSegura.getBirthPlaces().get(j).getClass())
						.newInstance(miguelSegura.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (miguelSegura.getBirthYear() != null)
			this.birthYear = miguelSegura.getBirthYear().getClass()
					.getDeclaredConstructor(miguelSegura.getBirthYear().getClass())
					.newInstance(miguelSegura.getBirthYear());
		this.characterOffset = miguelSegura.getCharacterOffset();
		this.characterOnset = miguelSegura.getCharacterOnset();
		if (miguelSegura.getPosition() != null)
			this.position = miguelSegura.getPosition().getClass()
					.getDeclaredConstructor(miguelSegura.getPosition().getClass())
					.newInstance(miguelSegura.getPosition());
		for (int j = 0; j < miguelSegura.getTeams().size(); j++) {
			if (miguelSegura.getTeams().get(j) != null) {
				teams.add(miguelSegura.getTeams().get(j).getClass()
						.getDeclaredConstructor(miguelSegura.getTeams().get(j).getClass())
						.newInstance(miguelSegura.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MiguelSegura [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
