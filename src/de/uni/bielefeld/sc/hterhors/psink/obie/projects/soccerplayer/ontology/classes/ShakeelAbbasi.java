...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IShakeelAbbasi;
...
public class ShakeelAbbasi implements IShakeelAbbasi {
	...
	public ShakeelAbbasi(ShakeelAbbasi shakeelAbbasi) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = shakeelAbbasi.getAnnotationID();
		for (int j = 0; j < shakeelAbbasi.getBirthPlaces().size(); j++) {
			if (shakeelAbbasi.getBirthPlaces().get(j) != null) {
				birthPlaces.add(shakeelAbbasi.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(shakeelAbbasi.getBirthPlaces().get(j).getClass())
						.newInstance(shakeelAbbasi.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (shakeelAbbasi.getBirthYear() != null)
			this.birthYear = shakeelAbbasi.getBirthYear().getClass()
					.getDeclaredConstructor(shakeelAbbasi.getBirthYear().getClass())
					.newInstance(shakeelAbbasi.getBirthYear());
		this.characterOffset = shakeelAbbasi.getCharacterOffset();
		this.characterOnset = shakeelAbbasi.getCharacterOnset();
		if (shakeelAbbasi.getPosition() != null)
			this.position = shakeelAbbasi.getPosition().getClass()
					.getDeclaredConstructor(shakeelAbbasi.getPosition().getClass())
					.newInstance(shakeelAbbasi.getPosition());
		for (int j = 0; j < shakeelAbbasi.getTeams().size(); j++) {
			if (shakeelAbbasi.getTeams().get(j) != null) {
				teams.add(shakeelAbbasi.getTeams().get(j).getClass()
						.getDeclaredConstructor(shakeelAbbasi.getTeams().get(j).getClass())
						.newInstance(shakeelAbbasi.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ShakeelAbbasi [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
