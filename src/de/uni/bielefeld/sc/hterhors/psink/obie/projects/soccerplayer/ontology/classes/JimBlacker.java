...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJimBlacker;
...
public class JimBlacker implements IJimBlacker {
	...
	public JimBlacker(JimBlacker jimBlacker) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = jimBlacker.getAnnotationID();
		for (int j = 0; j < jimBlacker.getBirthPlaces().size(); j++) {
			if (jimBlacker.getBirthPlaces().get(j) != null) {
				birthPlaces.add(jimBlacker.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(jimBlacker.getBirthPlaces().get(j).getClass())
						.newInstance(jimBlacker.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (jimBlacker.getBirthYear() != null)
			this.birthYear = jimBlacker.getBirthYear().getClass()
					.getDeclaredConstructor(jimBlacker.getBirthYear().getClass())
					.newInstance(jimBlacker.getBirthYear());
		this.characterOffset = jimBlacker.getCharacterOffset();
		this.characterOnset = jimBlacker.getCharacterOnset();
		if (jimBlacker.getPosition() != null)
			this.position = jimBlacker.getPosition().getClass()
					.getDeclaredConstructor(jimBlacker.getPosition().getClass()).newInstance(jimBlacker.getPosition());
		for (int j = 0; j < jimBlacker.getTeams().size(); j++) {
			if (jimBlacker.getTeams().get(j) != null) {
				teams.add(jimBlacker.getTeams().get(j).getClass()
						.getDeclaredConstructor(jimBlacker.getTeams().get(j).getClass())
						.newInstance(jimBlacker.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JimBlacker [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
