...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJeongGiDong;
...
public class JeongGiDong implements IJeongGiDong {
	...
	public JeongGiDong(JeongGiDong jeongGiDong) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = jeongGiDong.getAnnotationID();
		for (int j = 0; j < jeongGiDong.getBirthPlaces().size(); j++) {
			if (jeongGiDong.getBirthPlaces().get(j) != null) {
				birthPlaces.add(jeongGiDong.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(jeongGiDong.getBirthPlaces().get(j).getClass())
						.newInstance(jeongGiDong.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (jeongGiDong.getBirthYear() != null)
			this.birthYear = jeongGiDong.getBirthYear().getClass()
					.getDeclaredConstructor(jeongGiDong.getBirthYear().getClass())
					.newInstance(jeongGiDong.getBirthYear());
		this.characterOffset = jeongGiDong.getCharacterOffset();
		this.characterOnset = jeongGiDong.getCharacterOnset();
		if (jeongGiDong.getPosition() != null)
			this.position = jeongGiDong.getPosition().getClass()
					.getDeclaredConstructor(jeongGiDong.getPosition().getClass())
					.newInstance(jeongGiDong.getPosition());
		for (int j = 0; j < jeongGiDong.getTeams().size(); j++) {
			if (jeongGiDong.getTeams().get(j) != null) {
				teams.add(jeongGiDong.getTeams().get(j).getClass()
						.getDeclaredConstructor(jeongGiDong.getTeams().get(j).getClass())
						.newInstance(jeongGiDong.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JeongGiDong [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
