...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IUrmasKaljend;
...
public class UrmasKaljend implements IUrmasKaljend {
	...
	public UrmasKaljend(UrmasKaljend urmasKaljend) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = urmasKaljend.getAnnotationID();
		for (int j = 0; j < urmasKaljend.getBirthPlaces().size(); j++) {
			if (urmasKaljend.getBirthPlaces().get(j) != null) {
				birthPlaces.add(urmasKaljend.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(urmasKaljend.getBirthPlaces().get(j).getClass())
						.newInstance(urmasKaljend.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (urmasKaljend.getBirthYear() != null)
			this.birthYear = urmasKaljend.getBirthYear().getClass()
					.getDeclaredConstructor(urmasKaljend.getBirthYear().getClass())
					.newInstance(urmasKaljend.getBirthYear());
		this.characterOffset = urmasKaljend.getCharacterOffset();
		this.characterOnset = urmasKaljend.getCharacterOnset();
		if (urmasKaljend.getPosition() != null)
			this.position = urmasKaljend.getPosition().getClass()
					.getDeclaredConstructor(urmasKaljend.getPosition().getClass())
					.newInstance(urmasKaljend.getPosition());
		for (int j = 0; j < urmasKaljend.getTeams().size(); j++) {
			if (urmasKaljend.getTeams().get(j) != null) {
				teams.add(urmasKaljend.getTeams().get(j).getClass()
						.getDeclaredConstructor(urmasKaljend.getTeams().get(j).getClass())
						.newInstance(urmasKaljend.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "UrmasKaljend [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
