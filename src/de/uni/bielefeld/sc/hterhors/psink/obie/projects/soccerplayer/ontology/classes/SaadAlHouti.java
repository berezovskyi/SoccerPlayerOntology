...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISaadAlHouti;
...
public class SaadAlHouti implements ISaadAlHouti {
	...
	public SaadAlHouti(SaadAlHouti saadAlHouti) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = saadAlHouti.getAnnotationID();
		for (int j = 0; j < saadAlHouti.getBirthPlaces().size(); j++) {
			if (saadAlHouti.getBirthPlaces().get(j) != null) {
				birthPlaces.add(saadAlHouti.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(saadAlHouti.getBirthPlaces().get(j).getClass())
						.newInstance(saadAlHouti.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (saadAlHouti.getBirthYear() != null)
			this.birthYear = saadAlHouti.getBirthYear().getClass()
					.getDeclaredConstructor(saadAlHouti.getBirthYear().getClass())
					.newInstance(saadAlHouti.getBirthYear());
		this.characterOffset = saadAlHouti.getCharacterOffset();
		this.characterOnset = saadAlHouti.getCharacterOnset();
		if (saadAlHouti.getPosition() != null)
			this.position = saadAlHouti.getPosition().getClass()
					.getDeclaredConstructor(saadAlHouti.getPosition().getClass())
					.newInstance(saadAlHouti.getPosition());
		for (int j = 0; j < saadAlHouti.getTeams().size(); j++) {
			if (saadAlHouti.getTeams().get(j) != null) {
				teams.add(saadAlHouti.getTeams().get(j).getClass()
						.getDeclaredConstructor(saadAlHouti.getTeams().get(j).getClass())
						.newInstance(saadAlHouti.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SaadAlHouti [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
