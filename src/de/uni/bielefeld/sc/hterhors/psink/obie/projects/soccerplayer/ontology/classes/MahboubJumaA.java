...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMahboubJumaA;
...
public class MahboubJumaA implements IMahboubJumaA {
	...
	public MahboubJumaA(MahboubJumaA mahboubJumaA) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = mahboubJumaA.getAnnotationID();
		for (int j = 0; j < mahboubJumaA.getBirthPlaces().size(); j++) {
			if (mahboubJumaA.getBirthPlaces().get(j) != null) {
				birthPlaces.add(mahboubJumaA.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(mahboubJumaA.getBirthPlaces().get(j).getClass())
						.newInstance(mahboubJumaA.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (mahboubJumaA.getBirthYear() != null)
			this.birthYear = mahboubJumaA.getBirthYear().getClass()
					.getDeclaredConstructor(mahboubJumaA.getBirthYear().getClass())
					.newInstance(mahboubJumaA.getBirthYear());
		this.characterOffset = mahboubJumaA.getCharacterOffset();
		this.characterOnset = mahboubJumaA.getCharacterOnset();
		if (mahboubJumaA.getPosition() != null)
			this.position = mahboubJumaA.getPosition().getClass()
					.getDeclaredConstructor(mahboubJumaA.getPosition().getClass())
					.newInstance(mahboubJumaA.getPosition());
		for (int j = 0; j < mahboubJumaA.getTeams().size(); j++) {
			if (mahboubJumaA.getTeams().get(j) != null) {
				teams.add(mahboubJumaA.getTeams().get(j).getClass()
						.getDeclaredConstructor(mahboubJumaA.getTeams().get(j).getClass())
						.newInstance(mahboubJumaA.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MahboubJumaA [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
