...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMiguelDavis;
...
public class MiguelDavis implements IMiguelDavis {
	...
	public MiguelDavis(MiguelDavis miguelDavis) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = miguelDavis.getAnnotationID();
		for (int j = 0; j < miguelDavis.getBirthPlaces().size(); j++) {
			if (miguelDavis.getBirthPlaces().get(j) != null) {
				birthPlaces.add(miguelDavis.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(miguelDavis.getBirthPlaces().get(j).getClass())
						.newInstance(miguelDavis.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (miguelDavis.getBirthYear() != null)
			this.birthYear = miguelDavis.getBirthYear().getClass()
					.getDeclaredConstructor(miguelDavis.getBirthYear().getClass())
					.newInstance(miguelDavis.getBirthYear());
		this.characterOffset = miguelDavis.getCharacterOffset();
		this.characterOnset = miguelDavis.getCharacterOnset();
		if (miguelDavis.getPosition() != null)
			this.position = miguelDavis.getPosition().getClass()
					.getDeclaredConstructor(miguelDavis.getPosition().getClass())
					.newInstance(miguelDavis.getPosition());
		for (int j = 0; j < miguelDavis.getTeams().size(); j++) {
			if (miguelDavis.getTeams().get(j) != null) {
				teams.add(miguelDavis.getTeams().get(j).getClass()
						.getDeclaredConstructor(miguelDavis.getTeams().get(j).getClass())
						.newInstance(miguelDavis.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MiguelDavis [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
