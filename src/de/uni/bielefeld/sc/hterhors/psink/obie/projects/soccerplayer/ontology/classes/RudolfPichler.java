...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRudolfPichler;
...
public class RudolfPichler implements IRudolfPichler {
	...
	public RudolfPichler(RudolfPichler rudolfPichler) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = rudolfPichler.getAnnotationID();
		for (int j = 0; j < rudolfPichler.getBirthPlaces().size(); j++) {
			if (rudolfPichler.getBirthPlaces().get(j) != null) {
				birthPlaces.add(rudolfPichler.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(rudolfPichler.getBirthPlaces().get(j).getClass())
						.newInstance(rudolfPichler.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (rudolfPichler.getBirthYear() != null)
			this.birthYear = rudolfPichler.getBirthYear().getClass()
					.getDeclaredConstructor(rudolfPichler.getBirthYear().getClass())
					.newInstance(rudolfPichler.getBirthYear());
		this.characterOffset = rudolfPichler.getCharacterOffset();
		this.characterOnset = rudolfPichler.getCharacterOnset();
		if (rudolfPichler.getPosition() != null)
			this.position = rudolfPichler.getPosition().getClass()
					.getDeclaredConstructor(rudolfPichler.getPosition().getClass())
					.newInstance(rudolfPichler.getPosition());
		for (int j = 0; j < rudolfPichler.getTeams().size(); j++) {
			if (rudolfPichler.getTeams().get(j) != null) {
				teams.add(rudolfPichler.getTeams().get(j).getClass()
						.getDeclaredConstructor(rudolfPichler.getTeams().get(j).getClass())
						.newInstance(rudolfPichler.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RudolfPichler [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
