...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRayPennick;
...
public class RayPennick implements IRayPennick {
	...
	public RayPennick(RayPennick rayPennick) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = rayPennick.getAnnotationID();
		for (int j = 0; j < rayPennick.getBirthPlaces().size(); j++) {
			if (rayPennick.getBirthPlaces().get(j) != null) {
				birthPlaces.add(rayPennick.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(rayPennick.getBirthPlaces().get(j).getClass())
						.newInstance(rayPennick.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (rayPennick.getBirthYear() != null)
			this.birthYear = rayPennick.getBirthYear().getClass()
					.getDeclaredConstructor(rayPennick.getBirthYear().getClass())
					.newInstance(rayPennick.getBirthYear());
		this.characterOffset = rayPennick.getCharacterOffset();
		this.characterOnset = rayPennick.getCharacterOnset();
		if (rayPennick.getPosition() != null)
			this.position = rayPennick.getPosition().getClass()
					.getDeclaredConstructor(rayPennick.getPosition().getClass()).newInstance(rayPennick.getPosition());
		for (int j = 0; j < rayPennick.getTeams().size(); j++) {
			if (rayPennick.getTeams().get(j) != null) {
				teams.add(rayPennick.getTeams().get(j).getClass()
						.getDeclaredConstructor(rayPennick.getTeams().get(j).getClass())
						.newInstance(rayPennick.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RayPennick [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
