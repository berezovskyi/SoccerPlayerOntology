...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IArthurSamson;
...
public class ArthurSamson implements IArthurSamson {
	...
	public ArthurSamson(ArthurSamson arthurSamson) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = arthurSamson.getAnnotationID();
		for (int j = 0; j < arthurSamson.getBirthPlaces().size(); j++) {
			if (arthurSamson.getBirthPlaces().get(j) != null) {
				birthPlaces.add(arthurSamson.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(arthurSamson.getBirthPlaces().get(j).getClass())
						.newInstance(arthurSamson.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (arthurSamson.getBirthYear() != null)
			this.birthYear = arthurSamson.getBirthYear().getClass()
					.getDeclaredConstructor(arthurSamson.getBirthYear().getClass())
					.newInstance(arthurSamson.getBirthYear());
		this.characterOffset = arthurSamson.getCharacterOffset();
		this.characterOnset = arthurSamson.getCharacterOnset();
		if (arthurSamson.getPosition() != null)
			this.position = arthurSamson.getPosition().getClass()
					.getDeclaredConstructor(arthurSamson.getPosition().getClass())
					.newInstance(arthurSamson.getPosition());
		for (int j = 0; j < arthurSamson.getTeams().size(); j++) {
			if (arthurSamson.getTeams().get(j) != null) {
				teams.add(arthurSamson.getTeams().get(j).getClass()
						.getDeclaredConstructor(arthurSamson.getTeams().get(j).getClass())
						.newInstance(arthurSamson.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ArthurSamson [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
