...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJimmyCuthbertson;
...
public class JimmyCuthbertson implements IJimmyCuthbertson {
	...
	public JimmyCuthbertson(JimmyCuthbertson jimmyCuthbertson) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = jimmyCuthbertson.getAnnotationID();
		for (int j = 0; j < jimmyCuthbertson.getBirthPlaces().size(); j++) {
			if (jimmyCuthbertson.getBirthPlaces().get(j) != null) {
				birthPlaces.add(jimmyCuthbertson.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(jimmyCuthbertson.getBirthPlaces().get(j).getClass())
						.newInstance(jimmyCuthbertson.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (jimmyCuthbertson.getBirthYear() != null)
			this.birthYear = jimmyCuthbertson.getBirthYear().getClass()
					.getDeclaredConstructor(jimmyCuthbertson.getBirthYear().getClass())
					.newInstance(jimmyCuthbertson.getBirthYear());
		this.characterOffset = jimmyCuthbertson.getCharacterOffset();
		this.characterOnset = jimmyCuthbertson.getCharacterOnset();
		if (jimmyCuthbertson.getPosition() != null)
			this.position = jimmyCuthbertson.getPosition().getClass()
					.getDeclaredConstructor(jimmyCuthbertson.getPosition().getClass())
					.newInstance(jimmyCuthbertson.getPosition());
		for (int j = 0; j < jimmyCuthbertson.getTeams().size(); j++) {
			if (jimmyCuthbertson.getTeams().get(j) != null) {
				teams.add(jimmyCuthbertson.getTeams().get(j).getClass()
						.getDeclaredConstructor(jimmyCuthbertson.getTeams().get(j).getClass())
						.newInstance(jimmyCuthbertson.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JimmyCuthbertson [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
