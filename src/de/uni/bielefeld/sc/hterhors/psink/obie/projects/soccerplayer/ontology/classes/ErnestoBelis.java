...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IErnestoBelis;
...
public class ErnestoBelis implements IErnestoBelis {
	...
	public ErnestoBelis(ErnestoBelis ernestoBelis) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = ernestoBelis.getAnnotationID();
		for (int j = 0; j < ernestoBelis.getBirthPlaces().size(); j++) {
			if (ernestoBelis.getBirthPlaces().get(j) != null) {
				birthPlaces.add(ernestoBelis.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(ernestoBelis.getBirthPlaces().get(j).getClass())
						.newInstance(ernestoBelis.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (ernestoBelis.getBirthYear() != null)
			this.birthYear = ernestoBelis.getBirthYear().getClass()
					.getDeclaredConstructor(ernestoBelis.getBirthYear().getClass())
					.newInstance(ernestoBelis.getBirthYear());
		this.characterOffset = ernestoBelis.getCharacterOffset();
		this.characterOnset = ernestoBelis.getCharacterOnset();
		if (ernestoBelis.getPosition() != null)
			this.position = ernestoBelis.getPosition().getClass()
					.getDeclaredConstructor(ernestoBelis.getPosition().getClass())
					.newInstance(ernestoBelis.getPosition());
		for (int j = 0; j < ernestoBelis.getTeams().size(); j++) {
			if (ernestoBelis.getTeams().get(j) != null) {
				teams.add(ernestoBelis.getTeams().get(j).getClass()
						.getDeclaredConstructor(ernestoBelis.getTeams().get(j).getClass())
						.newInstance(ernestoBelis.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "ErnestoBelis [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
