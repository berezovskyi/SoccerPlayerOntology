...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISamHignett;
...
public class SamHignett implements ISamHignett {
	...
	public SamHignett(SamHignett samHignett) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = samHignett.getAnnotationID();
		for (int j = 0; j < samHignett.getBirthPlaces().size(); j++) {
			if (samHignett.getBirthPlaces().get(j) != null) {
				birthPlaces.add(samHignett.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(samHignett.getBirthPlaces().get(j).getClass())
						.newInstance(samHignett.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (samHignett.getBirthYear() != null)
			this.birthYear = samHignett.getBirthYear().getClass()
					.getDeclaredConstructor(samHignett.getBirthYear().getClass())
					.newInstance(samHignett.getBirthYear());
		this.characterOffset = samHignett.getCharacterOffset();
		this.characterOnset = samHignett.getCharacterOnset();
		if (samHignett.getPosition() != null)
			this.position = samHignett.getPosition().getClass()
					.getDeclaredConstructor(samHignett.getPosition().getClass()).newInstance(samHignett.getPosition());
		for (int j = 0; j < samHignett.getTeams().size(); j++) {
			if (samHignett.getTeams().get(j) != null) {
				teams.add(samHignett.getTeams().get(j).getClass()
						.getDeclaredConstructor(samHignett.getTeams().get(j).getClass())
						.newInstance(samHignett.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SamHignett [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
