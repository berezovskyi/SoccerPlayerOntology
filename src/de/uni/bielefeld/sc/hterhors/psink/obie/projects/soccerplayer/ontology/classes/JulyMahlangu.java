...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IJulyMahlangu;
...
public class JulyMahlangu implements IJulyMahlangu {
	...
	public JulyMahlangu(JulyMahlangu julyMahlangu) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = julyMahlangu.getAnnotationID();
		for (int j = 0; j < julyMahlangu.getBirthPlaces().size(); j++) {
			if (julyMahlangu.getBirthPlaces().get(j) != null) {
				birthPlaces.add(julyMahlangu.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(julyMahlangu.getBirthPlaces().get(j).getClass())
						.newInstance(julyMahlangu.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (julyMahlangu.getBirthYear() != null)
			this.birthYear = julyMahlangu.getBirthYear().getClass()
					.getDeclaredConstructor(julyMahlangu.getBirthYear().getClass())
					.newInstance(julyMahlangu.getBirthYear());
		this.characterOffset = julyMahlangu.getCharacterOffset();
		this.characterOnset = julyMahlangu.getCharacterOnset();
		if (julyMahlangu.getPosition() != null)
			this.position = julyMahlangu.getPosition().getClass()
					.getDeclaredConstructor(julyMahlangu.getPosition().getClass())
					.newInstance(julyMahlangu.getPosition());
		for (int j = 0; j < julyMahlangu.getTeams().size(); j++) {
			if (julyMahlangu.getTeams().get(j) != null) {
				teams.add(julyMahlangu.getTeams().get(j).getClass()
						.getDeclaredConstructor(julyMahlangu.getTeams().get(j).getClass())
						.newInstance(julyMahlangu.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "JulyMahlangu [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
