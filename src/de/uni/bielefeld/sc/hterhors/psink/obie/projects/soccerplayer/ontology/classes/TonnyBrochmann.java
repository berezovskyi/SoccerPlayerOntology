...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ITonnyBrochmann;
...
public class TonnyBrochmann implements ITonnyBrochmann {
	...
	public TonnyBrochmann(TonnyBrochmann tonnyBrochmann) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = tonnyBrochmann.getAnnotationID();
		for (int j = 0; j < tonnyBrochmann.getBirthPlaces().size(); j++) {
			if (tonnyBrochmann.getBirthPlaces().get(j) != null) {
				birthPlaces.add(tonnyBrochmann.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(tonnyBrochmann.getBirthPlaces().get(j).getClass())
						.newInstance(tonnyBrochmann.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (tonnyBrochmann.getBirthYear() != null)
			this.birthYear = tonnyBrochmann.getBirthYear().getClass()
					.getDeclaredConstructor(tonnyBrochmann.getBirthYear().getClass())
					.newInstance(tonnyBrochmann.getBirthYear());
		this.characterOffset = tonnyBrochmann.getCharacterOffset();
		this.characterOnset = tonnyBrochmann.getCharacterOnset();
		if (tonnyBrochmann.getPosition() != null)
			this.position = tonnyBrochmann.getPosition().getClass()
					.getDeclaredConstructor(tonnyBrochmann.getPosition().getClass())
					.newInstance(tonnyBrochmann.getPosition());
		for (int j = 0; j < tonnyBrochmann.getTeams().size(); j++) {
			if (tonnyBrochmann.getTeams().get(j) != null) {
				teams.add(tonnyBrochmann.getTeams().get(j).getClass()
						.getDeclaredConstructor(tonnyBrochmann.getTeams().get(j).getClass())
						.newInstance(tonnyBrochmann.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "TonnyBrochmann [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
