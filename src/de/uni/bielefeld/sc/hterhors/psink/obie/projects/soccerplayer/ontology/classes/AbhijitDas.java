...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAbhijitDas;
...
public class AbhijitDas implements IAbhijitDas {
	...
	public AbhijitDas(AbhijitDas abhijitDas) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = abhijitDas.getAnnotationID();
		for (int j = 0; j < abhijitDas.getBirthPlaces().size(); j++) {
			if (abhijitDas.getBirthPlaces().get(j) != null) {
				birthPlaces.add(abhijitDas.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(abhijitDas.getBirthPlaces().get(j).getClass())
						.newInstance(abhijitDas.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (abhijitDas.getBirthYear() != null)
			this.birthYear = abhijitDas.getBirthYear().getClass()
					.getDeclaredConstructor(abhijitDas.getBirthYear().getClass())
					.newInstance(abhijitDas.getBirthYear());
		this.characterOffset = abhijitDas.getCharacterOffset();
		this.characterOnset = abhijitDas.getCharacterOnset();
		if (abhijitDas.getPosition() != null)
			this.position = abhijitDas.getPosition().getClass()
					.getDeclaredConstructor(abhijitDas.getPosition().getClass()).newInstance(abhijitDas.getPosition());
		for (int j = 0; j < abhijitDas.getTeams().size(); j++) {
			if (abhijitDas.getTeams().get(j) != null) {
				teams.add(abhijitDas.getTeams().get(j).getClass()
						.getDeclaredConstructor(abhijitDas.getTeams().get(j).getClass())
						.newInstance(abhijitDas.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AbhijitDas [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
