...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IFredBeardsley;
...
public class FredBeardsley implements IFredBeardsley {
	...
	public FredBeardsley(FredBeardsley fredBeardsley) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = fredBeardsley.getAnnotationID();
		for (int j = 0; j < fredBeardsley.getBirthPlaces().size(); j++) {
			if (fredBeardsley.getBirthPlaces().get(j) != null) {
				birthPlaces.add(fredBeardsley.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(fredBeardsley.getBirthPlaces().get(j).getClass())
						.newInstance(fredBeardsley.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (fredBeardsley.getBirthYear() != null)
			this.birthYear = fredBeardsley.getBirthYear().getClass()
					.getDeclaredConstructor(fredBeardsley.getBirthYear().getClass())
					.newInstance(fredBeardsley.getBirthYear());
		this.characterOffset = fredBeardsley.getCharacterOffset();
		this.characterOnset = fredBeardsley.getCharacterOnset();
		if (fredBeardsley.getPosition() != null)
			this.position = fredBeardsley.getPosition().getClass()
					.getDeclaredConstructor(fredBeardsley.getPosition().getClass())
					.newInstance(fredBeardsley.getPosition());
		for (int j = 0; j < fredBeardsley.getTeams().size(); j++) {
			if (fredBeardsley.getTeams().get(j) != null) {
				teams.add(fredBeardsley.getTeams().get(j).getClass()
						.getDeclaredConstructor(fredBeardsley.getTeams().get(j).getClass())
						.newInstance(fredBeardsley.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "FredBeardsley [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
