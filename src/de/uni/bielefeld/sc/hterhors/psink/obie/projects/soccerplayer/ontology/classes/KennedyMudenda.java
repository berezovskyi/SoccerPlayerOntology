...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IKennedyMudenda;
...
public class KennedyMudenda implements IKennedyMudenda {
	...
	public KennedyMudenda(KennedyMudenda kennedyMudenda) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = kennedyMudenda.getAnnotationID();
		for (int j = 0; j < kennedyMudenda.getBirthPlaces().size(); j++) {
			if (kennedyMudenda.getBirthPlaces().get(j) != null) {
				birthPlaces.add(kennedyMudenda.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(kennedyMudenda.getBirthPlaces().get(j).getClass())
						.newInstance(kennedyMudenda.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (kennedyMudenda.getBirthYear() != null)
			this.birthYear = kennedyMudenda.getBirthYear().getClass()
					.getDeclaredConstructor(kennedyMudenda.getBirthYear().getClass())
					.newInstance(kennedyMudenda.getBirthYear());
		this.characterOffset = kennedyMudenda.getCharacterOffset();
		this.characterOnset = kennedyMudenda.getCharacterOnset();
		if (kennedyMudenda.getPosition() != null)
			this.position = kennedyMudenda.getPosition().getClass()
					.getDeclaredConstructor(kennedyMudenda.getPosition().getClass())
					.newInstance(kennedyMudenda.getPosition());
		for (int j = 0; j < kennedyMudenda.getTeams().size(); j++) {
			if (kennedyMudenda.getTeams().get(j) != null) {
				teams.add(kennedyMudenda.getTeams().get(j).getClass()
						.getDeclaredConstructor(kennedyMudenda.getTeams().get(j).getClass())
						.newInstance(kennedyMudenda.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "KennedyMudenda [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
