...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDrissTrichard;
...
public class DrissTrichard implements IDrissTrichard {
	...
	public DrissTrichard(DrissTrichard drissTrichard) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = drissTrichard.getAnnotationID();
		for (int j = 0; j < drissTrichard.getBirthPlaces().size(); j++) {
			if (drissTrichard.getBirthPlaces().get(j) != null) {
				birthPlaces.add(drissTrichard.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(drissTrichard.getBirthPlaces().get(j).getClass())
						.newInstance(drissTrichard.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (drissTrichard.getBirthYear() != null)
			this.birthYear = drissTrichard.getBirthYear().getClass()
					.getDeclaredConstructor(drissTrichard.getBirthYear().getClass())
					.newInstance(drissTrichard.getBirthYear());
		this.characterOffset = drissTrichard.getCharacterOffset();
		this.characterOnset = drissTrichard.getCharacterOnset();
		if (drissTrichard.getPosition() != null)
			this.position = drissTrichard.getPosition().getClass()
					.getDeclaredConstructor(drissTrichard.getPosition().getClass())
					.newInstance(drissTrichard.getPosition());
		for (int j = 0; j < drissTrichard.getTeams().size(); j++) {
			if (drissTrichard.getTeams().get(j) != null) {
				teams.add(drissTrichard.getTeams().get(j).getClass()
						.getDeclaredConstructor(drissTrichard.getTeams().get(j).getClass())
						.newInstance(drissTrichard.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DrissTrichard [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
