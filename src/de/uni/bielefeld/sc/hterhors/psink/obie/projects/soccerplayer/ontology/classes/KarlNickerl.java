...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IKarlNickerl;
...
public class KarlNickerl implements IKarlNickerl {
	...
	public KarlNickerl(KarlNickerl karlNickerl) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = karlNickerl.getAnnotationID();
		for (int j = 0; j < karlNickerl.getBirthPlaces().size(); j++) {
			if (karlNickerl.getBirthPlaces().get(j) != null) {
				birthPlaces.add(karlNickerl.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(karlNickerl.getBirthPlaces().get(j).getClass())
						.newInstance(karlNickerl.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (karlNickerl.getBirthYear() != null)
			this.birthYear = karlNickerl.getBirthYear().getClass()
					.getDeclaredConstructor(karlNickerl.getBirthYear().getClass())
					.newInstance(karlNickerl.getBirthYear());
		this.characterOffset = karlNickerl.getCharacterOffset();
		this.characterOnset = karlNickerl.getCharacterOnset();
		if (karlNickerl.getPosition() != null)
			this.position = karlNickerl.getPosition().getClass()
					.getDeclaredConstructor(karlNickerl.getPosition().getClass())
					.newInstance(karlNickerl.getPosition());
		for (int j = 0; j < karlNickerl.getTeams().size(); j++) {
			if (karlNickerl.getTeams().get(j) != null) {
				teams.add(karlNickerl.getTeams().get(j).getClass()
						.getDeclaredConstructor(karlNickerl.getTeams().get(j).getClass())
						.newInstance(karlNickerl.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "KarlNickerl [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
