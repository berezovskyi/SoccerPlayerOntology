...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IRyanLloyd;
...
public class RyanLloyd implements IRyanLloyd {
	...
	public RyanLloyd(RyanLloyd ryanLloyd) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = ryanLloyd.getAnnotationID();
		for (int j = 0; j < ryanLloyd.getBirthPlaces().size(); j++) {
			if (ryanLloyd.getBirthPlaces().get(j) != null) {
				birthPlaces.add(ryanLloyd.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(ryanLloyd.getBirthPlaces().get(j).getClass())
						.newInstance(ryanLloyd.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (ryanLloyd.getBirthYear() != null)
			this.birthYear = ryanLloyd.getBirthYear().getClass()
					.getDeclaredConstructor(ryanLloyd.getBirthYear().getClass()).newInstance(ryanLloyd.getBirthYear());
		this.characterOffset = ryanLloyd.getCharacterOffset();
		this.characterOnset = ryanLloyd.getCharacterOnset();
		if (ryanLloyd.getPosition() != null)
			this.position = ryanLloyd.getPosition().getClass()
					.getDeclaredConstructor(ryanLloyd.getPosition().getClass()).newInstance(ryanLloyd.getPosition());
		for (int j = 0; j < ryanLloyd.getTeams().size(); j++) {
			if (ryanLloyd.getTeams().get(j) != null) {
				teams.add(ryanLloyd.getTeams().get(j).getClass()
						.getDeclaredConstructor(ryanLloyd.getTeams().get(j).getClass())
						.newInstance(ryanLloyd.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "RyanLloyd [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
