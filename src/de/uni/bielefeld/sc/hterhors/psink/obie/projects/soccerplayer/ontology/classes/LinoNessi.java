...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILinoNessi;
...
public class LinoNessi implements ILinoNessi {
	...
	public LinoNessi(LinoNessi linoNessi) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = linoNessi.getAnnotationID();
		for (int j = 0; j < linoNessi.getBirthPlaces().size(); j++) {
			if (linoNessi.getBirthPlaces().get(j) != null) {
				birthPlaces.add(linoNessi.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(linoNessi.getBirthPlaces().get(j).getClass())
						.newInstance(linoNessi.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (linoNessi.getBirthYear() != null)
			this.birthYear = linoNessi.getBirthYear().getClass()
					.getDeclaredConstructor(linoNessi.getBirthYear().getClass()).newInstance(linoNessi.getBirthYear());
		this.characterOffset = linoNessi.getCharacterOffset();
		this.characterOnset = linoNessi.getCharacterOnset();
		if (linoNessi.getPosition() != null)
			this.position = linoNessi.getPosition().getClass()
					.getDeclaredConstructor(linoNessi.getPosition().getClass()).newInstance(linoNessi.getPosition());
		for (int j = 0; j < linoNessi.getTeams().size(); j++) {
			if (linoNessi.getTeams().get(j) != null) {
				teams.add(linoNessi.getTeams().get(j).getClass()
						.getDeclaredConstructor(linoNessi.getTeams().get(j).getClass())
						.newInstance(linoNessi.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LinoNessi [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
