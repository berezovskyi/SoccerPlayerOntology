...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISerafimNeves;
...
public class SerafimNeves implements ISerafimNeves {
	...
	public SerafimNeves(SerafimNeves serafimNeves) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = serafimNeves.getAnnotationID();
		for (int j = 0; j < serafimNeves.getBirthPlaces().size(); j++) {
			if (serafimNeves.getBirthPlaces().get(j) != null) {
				birthPlaces.add(serafimNeves.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(serafimNeves.getBirthPlaces().get(j).getClass())
						.newInstance(serafimNeves.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (serafimNeves.getBirthYear() != null)
			this.birthYear = serafimNeves.getBirthYear().getClass()
					.getDeclaredConstructor(serafimNeves.getBirthYear().getClass())
					.newInstance(serafimNeves.getBirthYear());
		this.characterOffset = serafimNeves.getCharacterOffset();
		this.characterOnset = serafimNeves.getCharacterOnset();
		if (serafimNeves.getPosition() != null)
			this.position = serafimNeves.getPosition().getClass()
					.getDeclaredConstructor(serafimNeves.getPosition().getClass())
					.newInstance(serafimNeves.getPosition());
		for (int j = 0; j < serafimNeves.getTeams().size(); j++) {
			if (serafimNeves.getTeams().get(j) != null) {
				teams.add(serafimNeves.getTeams().get(j).getClass()
						.getDeclaredConstructor(serafimNeves.getTeams().get(j).getClass())
						.newInstance(serafimNeves.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SerafimNeves [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
