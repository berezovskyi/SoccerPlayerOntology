...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMasoudZeraei;
...
public class MasoudZeraei implements IMasoudZeraei {
	...
	public MasoudZeraei(MasoudZeraei masoudZeraei) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = masoudZeraei.getAnnotationID();
		for (int j = 0; j < masoudZeraei.getBirthPlaces().size(); j++) {
			if (masoudZeraei.getBirthPlaces().get(j) != null) {
				birthPlaces.add(masoudZeraei.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(masoudZeraei.getBirthPlaces().get(j).getClass())
						.newInstance(masoudZeraei.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (masoudZeraei.getBirthYear() != null)
			this.birthYear = masoudZeraei.getBirthYear().getClass()
					.getDeclaredConstructor(masoudZeraei.getBirthYear().getClass())
					.newInstance(masoudZeraei.getBirthYear());
		this.characterOffset = masoudZeraei.getCharacterOffset();
		this.characterOnset = masoudZeraei.getCharacterOnset();
		if (masoudZeraei.getPosition() != null)
			this.position = masoudZeraei.getPosition().getClass()
					.getDeclaredConstructor(masoudZeraei.getPosition().getClass())
					.newInstance(masoudZeraei.getPosition());
		for (int j = 0; j < masoudZeraei.getTeams().size(); j++) {
			if (masoudZeraei.getTeams().get(j) != null) {
				teams.add(masoudZeraei.getTeams().get(j).getClass()
						.getDeclaredConstructor(masoudZeraei.getTeams().get(j).getClass())
						.newInstance(masoudZeraei.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MasoudZeraei [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
