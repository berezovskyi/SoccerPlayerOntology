...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IKlausDecker;
...
public class KlausDecker implements IKlausDecker {
	...
	public KlausDecker(KlausDecker klausDecker) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = klausDecker.getAnnotationID();
		for (int j = 0; j < klausDecker.getBirthPlaces().size(); j++) {
			if (klausDecker.getBirthPlaces().get(j) != null) {
				birthPlaces.add(klausDecker.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(klausDecker.getBirthPlaces().get(j).getClass())
						.newInstance(klausDecker.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (klausDecker.getBirthYear() != null)
			this.birthYear = klausDecker.getBirthYear().getClass()
					.getDeclaredConstructor(klausDecker.getBirthYear().getClass())
					.newInstance(klausDecker.getBirthYear());
		this.characterOffset = klausDecker.getCharacterOffset();
		this.characterOnset = klausDecker.getCharacterOnset();
		if (klausDecker.getPosition() != null)
			this.position = klausDecker.getPosition().getClass()
					.getDeclaredConstructor(klausDecker.getPosition().getClass())
					.newInstance(klausDecker.getPosition());
		for (int j = 0; j < klausDecker.getTeams().size(); j++) {
			if (klausDecker.getTeams().get(j) != null) {
				teams.add(klausDecker.getTeams().get(j).getClass()
						.getDeclaredConstructor(klausDecker.getTeams().get(j).getClass())
						.newInstance(klausDecker.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "KlausDecker [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
