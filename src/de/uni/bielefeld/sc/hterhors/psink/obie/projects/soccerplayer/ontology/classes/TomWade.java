...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ITomWade;
...
public class TomWade implements ITomWade {
	...
	public TomWade(TomWade tomWade) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = tomWade.getAnnotationID();
		for (int j = 0; j < tomWade.getBirthPlaces().size(); j++) {
			if (tomWade.getBirthPlaces().get(j) != null) {
				birthPlaces.add(tomWade.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(tomWade.getBirthPlaces().get(j).getClass())
						.newInstance(tomWade.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (tomWade.getBirthYear() != null)
			this.birthYear = tomWade.getBirthYear().getClass().getDeclaredConstructor(tomWade.getBirthYear().getClass())
					.newInstance(tomWade.getBirthYear());
		this.characterOffset = tomWade.getCharacterOffset();
		this.characterOnset = tomWade.getCharacterOnset();
		if (tomWade.getPosition() != null)
			this.position = tomWade.getPosition().getClass().getDeclaredConstructor(tomWade.getPosition().getClass())
					.newInstance(tomWade.getPosition());
		for (int j = 0; j < tomWade.getTeams().size(); j++) {
			if (tomWade.getTeams().get(j) != null) {
				teams.add(tomWade.getTeams().get(j).getClass()
						.getDeclaredConstructor(tomWade.getTeams().get(j).getClass())
						.newInstance(tomWade.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "TomWade [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
