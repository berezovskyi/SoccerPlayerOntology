...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IBertFenwick;
...
public class BertFenwick implements IBertFenwick {
	...
	public BertFenwick(BertFenwick bertFenwick) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = bertFenwick.getAnnotationID();
		for (int j = 0; j < bertFenwick.getBirthPlaces().size(); j++) {
			if (bertFenwick.getBirthPlaces().get(j) != null) {
				birthPlaces.add(bertFenwick.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(bertFenwick.getBirthPlaces().get(j).getClass())
						.newInstance(bertFenwick.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (bertFenwick.getBirthYear() != null)
			this.birthYear = bertFenwick.getBirthYear().getClass()
					.getDeclaredConstructor(bertFenwick.getBirthYear().getClass())
					.newInstance(bertFenwick.getBirthYear());
		this.characterOffset = bertFenwick.getCharacterOffset();
		this.characterOnset = bertFenwick.getCharacterOnset();
		if (bertFenwick.getPosition() != null)
			this.position = bertFenwick.getPosition().getClass()
					.getDeclaredConstructor(bertFenwick.getPosition().getClass())
					.newInstance(bertFenwick.getPosition());
		for (int j = 0; j < bertFenwick.getTeams().size(); j++) {
			if (bertFenwick.getTeams().get(j) != null) {
				teams.add(bertFenwick.getTeams().get(j).getClass()
						.getDeclaredConstructor(bertFenwick.getTeams().get(j).getClass())
						.newInstance(bertFenwick.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "BertFenwick [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
