...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ITarmoSaks;
...
public class TarmoSaks implements ITarmoSaks {
	...
	public TarmoSaks(TarmoSaks tarmoSaks) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = tarmoSaks.getAnnotationID();
		for (int j = 0; j < tarmoSaks.getBirthPlaces().size(); j++) {
			if (tarmoSaks.getBirthPlaces().get(j) != null) {
				birthPlaces.add(tarmoSaks.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(tarmoSaks.getBirthPlaces().get(j).getClass())
						.newInstance(tarmoSaks.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (tarmoSaks.getBirthYear() != null)
			this.birthYear = tarmoSaks.getBirthYear().getClass()
					.getDeclaredConstructor(tarmoSaks.getBirthYear().getClass()).newInstance(tarmoSaks.getBirthYear());
		this.characterOffset = tarmoSaks.getCharacterOffset();
		this.characterOnset = tarmoSaks.getCharacterOnset();
		if (tarmoSaks.getPosition() != null)
			this.position = tarmoSaks.getPosition().getClass()
					.getDeclaredConstructor(tarmoSaks.getPosition().getClass()).newInstance(tarmoSaks.getPosition());
		for (int j = 0; j < tarmoSaks.getTeams().size(); j++) {
			if (tarmoSaks.getTeams().get(j) != null) {
				teams.add(tarmoSaks.getTeams().get(j).getClass()
						.getDeclaredConstructor(tarmoSaks.getTeams().get(j).getClass())
						.newInstance(tarmoSaks.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "TarmoSaks [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
