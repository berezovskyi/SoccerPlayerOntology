...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IHaJungWon;
...
public class HaJungWon implements IHaJungWon {
	...
	public HaJungWon(HaJungWon haJungWon) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = haJungWon.getAnnotationID();
		for (int j = 0; j < haJungWon.getBirthPlaces().size(); j++) {
			if (haJungWon.getBirthPlaces().get(j) != null) {
				birthPlaces.add(haJungWon.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(haJungWon.getBirthPlaces().get(j).getClass())
						.newInstance(haJungWon.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (haJungWon.getBirthYear() != null)
			this.birthYear = haJungWon.getBirthYear().getClass()
					.getDeclaredConstructor(haJungWon.getBirthYear().getClass()).newInstance(haJungWon.getBirthYear());
		this.characterOffset = haJungWon.getCharacterOffset();
		this.characterOnset = haJungWon.getCharacterOnset();
		if (haJungWon.getPosition() != null)
			this.position = haJungWon.getPosition().getClass()
					.getDeclaredConstructor(haJungWon.getPosition().getClass()).newInstance(haJungWon.getPosition());
		for (int j = 0; j < haJungWon.getTeams().size(); j++) {
			if (haJungWon.getTeams().get(j) != null) {
				teams.add(haJungWon.getTeams().get(j).getClass()
						.getDeclaredConstructor(haJungWon.getTeams().get(j).getClass())
						.newInstance(haJungWon.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "HaJungWon [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
