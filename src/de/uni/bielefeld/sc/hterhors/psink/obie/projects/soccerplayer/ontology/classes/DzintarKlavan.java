...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IDzintarKlavan;
...
public class DzintarKlavan implements IDzintarKlavan {
	...
	public DzintarKlavan(DzintarKlavan dzintarKlavan) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = dzintarKlavan.getAnnotationID();
		for (int j = 0; j < dzintarKlavan.getBirthPlaces().size(); j++) {
			if (dzintarKlavan.getBirthPlaces().get(j) != null) {
				birthPlaces.add(dzintarKlavan.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(dzintarKlavan.getBirthPlaces().get(j).getClass())
						.newInstance(dzintarKlavan.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (dzintarKlavan.getBirthYear() != null)
			this.birthYear = dzintarKlavan.getBirthYear().getClass()
					.getDeclaredConstructor(dzintarKlavan.getBirthYear().getClass())
					.newInstance(dzintarKlavan.getBirthYear());
		this.characterOffset = dzintarKlavan.getCharacterOffset();
		this.characterOnset = dzintarKlavan.getCharacterOnset();
		if (dzintarKlavan.getPosition() != null)
			this.position = dzintarKlavan.getPosition().getClass()
					.getDeclaredConstructor(dzintarKlavan.getPosition().getClass())
					.newInstance(dzintarKlavan.getPosition());
		for (int j = 0; j < dzintarKlavan.getTeams().size(); j++) {
			if (dzintarKlavan.getTeams().get(j) != null) {
				teams.add(dzintarKlavan.getTeams().get(j).getClass()
						.getDeclaredConstructor(dzintarKlavan.getTeams().get(j).getClass())
						.newInstance(dzintarKlavan.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "DzintarKlavan [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
