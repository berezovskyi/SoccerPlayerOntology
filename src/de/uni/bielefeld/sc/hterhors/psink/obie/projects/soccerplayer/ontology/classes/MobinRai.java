...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMobinRai;
...
public class MobinRai implements IMobinRai {
	...
	public MobinRai(MobinRai mobinRai) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = mobinRai.getAnnotationID();
		for (int j = 0; j < mobinRai.getBirthPlaces().size(); j++) {
			if (mobinRai.getBirthPlaces().get(j) != null) {
				birthPlaces.add(mobinRai.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(mobinRai.getBirthPlaces().get(j).getClass())
						.newInstance(mobinRai.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (mobinRai.getBirthYear() != null)
			this.birthYear = mobinRai.getBirthYear().getClass()
					.getDeclaredConstructor(mobinRai.getBirthYear().getClass()).newInstance(mobinRai.getBirthYear());
		this.characterOffset = mobinRai.getCharacterOffset();
		this.characterOnset = mobinRai.getCharacterOnset();
		if (mobinRai.getPosition() != null)
			this.position = mobinRai.getPosition().getClass().getDeclaredConstructor(mobinRai.getPosition().getClass())
					.newInstance(mobinRai.getPosition());
		for (int j = 0; j < mobinRai.getTeams().size(); j++) {
			if (mobinRai.getTeams().get(j) != null) {
				teams.add(mobinRai.getTeams().get(j).getClass()
						.getDeclaredConstructor(mobinRai.getTeams().get(j).getClass())
						.newInstance(mobinRai.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MobinRai [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
