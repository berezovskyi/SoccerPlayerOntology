...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILiamGray;
...
public class LiamGray implements ILiamGray {
	...
	public LiamGray(LiamGray liamGray) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = liamGray.getAnnotationID();
		for (int j = 0; j < liamGray.getBirthPlaces().size(); j++) {
			if (liamGray.getBirthPlaces().get(j) != null) {
				birthPlaces.add(liamGray.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(liamGray.getBirthPlaces().get(j).getClass())
						.newInstance(liamGray.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (liamGray.getBirthYear() != null)
			this.birthYear = liamGray.getBirthYear().getClass()
					.getDeclaredConstructor(liamGray.getBirthYear().getClass()).newInstance(liamGray.getBirthYear());
		this.characterOffset = liamGray.getCharacterOffset();
		this.characterOnset = liamGray.getCharacterOnset();
		if (liamGray.getPosition() != null)
			this.position = liamGray.getPosition().getClass().getDeclaredConstructor(liamGray.getPosition().getClass())
					.newInstance(liamGray.getPosition());
		for (int j = 0; j < liamGray.getTeams().size(); j++) {
			if (liamGray.getTeams().get(j) != null) {
				teams.add(liamGray.getTeams().get(j).getClass()
						.getDeclaredConstructor(liamGray.getTeams().get(j).getClass())
						.newInstance(liamGray.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LiamGray [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
