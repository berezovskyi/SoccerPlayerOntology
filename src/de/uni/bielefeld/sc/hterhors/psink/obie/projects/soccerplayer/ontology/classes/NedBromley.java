...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.INedBromley;
...
public class NedBromley implements INedBromley {
	...
	public NedBromley(NedBromley nedBromley) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = nedBromley.getAnnotationID();
		for (int j = 0; j < nedBromley.getBirthPlaces().size(); j++) {
			if (nedBromley.getBirthPlaces().get(j) != null) {
				birthPlaces.add(nedBromley.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(nedBromley.getBirthPlaces().get(j).getClass())
						.newInstance(nedBromley.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (nedBromley.getBirthYear() != null)
			this.birthYear = nedBromley.getBirthYear().getClass()
					.getDeclaredConstructor(nedBromley.getBirthYear().getClass())
					.newInstance(nedBromley.getBirthYear());
		this.characterOffset = nedBromley.getCharacterOffset();
		this.characterOnset = nedBromley.getCharacterOnset();
		if (nedBromley.getPosition() != null)
			this.position = nedBromley.getPosition().getClass()
					.getDeclaredConstructor(nedBromley.getPosition().getClass()).newInstance(nedBromley.getPosition());
		for (int j = 0; j < nedBromley.getTeams().size(); j++) {
			if (nedBromley.getTeams().get(j) != null) {
				teams.add(nedBromley.getTeams().get(j).getClass()
						.getDeclaredConstructor(nedBromley.getTeams().get(j).getClass())
						.newInstance(nedBromley.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "NedBromley [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
