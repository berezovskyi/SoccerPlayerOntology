...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMarcelinoVargas;
...
public class MarcelinoVargas implements IMarcelinoVargas {
	...
	public MarcelinoVargas(MarcelinoVargas marcelinoVargas) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = marcelinoVargas.getAnnotationID();
		for (int j = 0; j < marcelinoVargas.getBirthPlaces().size(); j++) {
			if (marcelinoVargas.getBirthPlaces().get(j) != null) {
				birthPlaces.add(marcelinoVargas.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(marcelinoVargas.getBirthPlaces().get(j).getClass())
						.newInstance(marcelinoVargas.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (marcelinoVargas.getBirthYear() != null)
			this.birthYear = marcelinoVargas.getBirthYear().getClass()
					.getDeclaredConstructor(marcelinoVargas.getBirthYear().getClass())
					.newInstance(marcelinoVargas.getBirthYear());
		this.characterOffset = marcelinoVargas.getCharacterOffset();
		this.characterOnset = marcelinoVargas.getCharacterOnset();
		if (marcelinoVargas.getPosition() != null)
			this.position = marcelinoVargas.getPosition().getClass()
					.getDeclaredConstructor(marcelinoVargas.getPosition().getClass())
					.newInstance(marcelinoVargas.getPosition());
		for (int j = 0; j < marcelinoVargas.getTeams().size(); j++) {
			if (marcelinoVargas.getTeams().get(j) != null) {
				teams.add(marcelinoVargas.getTeams().get(j).getClass()
						.getDeclaredConstructor(marcelinoVargas.getTeams().get(j).getClass())
						.newInstance(marcelinoVargas.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MarcelinoVargas [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
