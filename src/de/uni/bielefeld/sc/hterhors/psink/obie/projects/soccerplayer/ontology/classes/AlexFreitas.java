...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAlexFreitas;
...
public class AlexFreitas implements IAlexFreitas {
	...
	public AlexFreitas(AlexFreitas alexFreitas) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = alexFreitas.getAnnotationID();
		for (int j = 0; j < alexFreitas.getBirthPlaces().size(); j++) {
			if (alexFreitas.getBirthPlaces().get(j) != null) {
				birthPlaces.add(alexFreitas.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(alexFreitas.getBirthPlaces().get(j).getClass())
						.newInstance(alexFreitas.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (alexFreitas.getBirthYear() != null)
			this.birthYear = alexFreitas.getBirthYear().getClass()
					.getDeclaredConstructor(alexFreitas.getBirthYear().getClass())
					.newInstance(alexFreitas.getBirthYear());
		this.characterOffset = alexFreitas.getCharacterOffset();
		this.characterOnset = alexFreitas.getCharacterOnset();
		if (alexFreitas.getPosition() != null)
			this.position = alexFreitas.getPosition().getClass()
					.getDeclaredConstructor(alexFreitas.getPosition().getClass())
					.newInstance(alexFreitas.getPosition());
		for (int j = 0; j < alexFreitas.getTeams().size(); j++) {
			if (alexFreitas.getTeams().get(j) != null) {
				teams.add(alexFreitas.getTeams().get(j).getClass()
						.getDeclaredConstructor(alexFreitas.getTeams().get(j).getClass())
						.newInstance(alexFreitas.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AlexFreitas [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
