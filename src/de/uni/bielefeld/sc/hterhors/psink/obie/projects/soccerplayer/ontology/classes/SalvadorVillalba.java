...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISalvadorVillalba;
...
public class SalvadorVillalba implements ISalvadorVillalba {
	...
	public SalvadorVillalba(SalvadorVillalba salvadorVillalba) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = salvadorVillalba.getAnnotationID();
		for (int j = 0; j < salvadorVillalba.getBirthPlaces().size(); j++) {
			if (salvadorVillalba.getBirthPlaces().get(j) != null) {
				birthPlaces.add(salvadorVillalba.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(salvadorVillalba.getBirthPlaces().get(j).getClass())
						.newInstance(salvadorVillalba.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (salvadorVillalba.getBirthYear() != null)
			this.birthYear = salvadorVillalba.getBirthYear().getClass()
					.getDeclaredConstructor(salvadorVillalba.getBirthYear().getClass())
					.newInstance(salvadorVillalba.getBirthYear());
		this.characterOffset = salvadorVillalba.getCharacterOffset();
		this.characterOnset = salvadorVillalba.getCharacterOnset();
		if (salvadorVillalba.getPosition() != null)
			this.position = salvadorVillalba.getPosition().getClass()
					.getDeclaredConstructor(salvadorVillalba.getPosition().getClass())
					.newInstance(salvadorVillalba.getPosition());
		for (int j = 0; j < salvadorVillalba.getTeams().size(); j++) {
			if (salvadorVillalba.getTeams().get(j) != null) {
				teams.add(salvadorVillalba.getTeams().get(j).getClass()
						.getDeclaredConstructor(salvadorVillalba.getTeams().get(j).getClass())
						.newInstance(salvadorVillalba.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SalvadorVillalba [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
