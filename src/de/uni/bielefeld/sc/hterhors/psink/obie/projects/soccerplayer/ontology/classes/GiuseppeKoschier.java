...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IGiuseppeKoschier;
...
public class GiuseppeKoschier implements IGiuseppeKoschier {
	...
	public GiuseppeKoschier(GiuseppeKoschier giuseppeKoschier) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = giuseppeKoschier.getAnnotationID();
		for (int j = 0; j < giuseppeKoschier.getBirthPlaces().size(); j++) {
			if (giuseppeKoschier.getBirthPlaces().get(j) != null) {
				birthPlaces.add(giuseppeKoschier.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(giuseppeKoschier.getBirthPlaces().get(j).getClass())
						.newInstance(giuseppeKoschier.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (giuseppeKoschier.getBirthYear() != null)
			this.birthYear = giuseppeKoschier.getBirthYear().getClass()
					.getDeclaredConstructor(giuseppeKoschier.getBirthYear().getClass())
					.newInstance(giuseppeKoschier.getBirthYear());
		this.characterOffset = giuseppeKoschier.getCharacterOffset();
		this.characterOnset = giuseppeKoschier.getCharacterOnset();
		if (giuseppeKoschier.getPosition() != null)
			this.position = giuseppeKoschier.getPosition().getClass()
					.getDeclaredConstructor(giuseppeKoschier.getPosition().getClass())
					.newInstance(giuseppeKoschier.getPosition());
		for (int j = 0; j < giuseppeKoschier.getTeams().size(); j++) {
			if (giuseppeKoschier.getTeams().get(j) != null) {
				teams.add(giuseppeKoschier.getTeams().get(j).getClass()
						.getDeclaredConstructor(giuseppeKoschier.getTeams().get(j).getClass())
						.newInstance(giuseppeKoschier.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "GiuseppeKoschier [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
