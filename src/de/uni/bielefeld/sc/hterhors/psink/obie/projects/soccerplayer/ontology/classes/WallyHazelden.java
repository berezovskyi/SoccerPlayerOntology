...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IWallyHazelden;
...
public class WallyHazelden implements IWallyHazelden {
	...
	public WallyHazelden(WallyHazelden wallyHazelden) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = wallyHazelden.getAnnotationID();
		for (int j = 0; j < wallyHazelden.getBirthPlaces().size(); j++) {
			if (wallyHazelden.getBirthPlaces().get(j) != null) {
				birthPlaces.add(wallyHazelden.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(wallyHazelden.getBirthPlaces().get(j).getClass())
						.newInstance(wallyHazelden.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (wallyHazelden.getBirthYear() != null)
			this.birthYear = wallyHazelden.getBirthYear().getClass()
					.getDeclaredConstructor(wallyHazelden.getBirthYear().getClass())
					.newInstance(wallyHazelden.getBirthYear());
		this.characterOffset = wallyHazelden.getCharacterOffset();
		this.characterOnset = wallyHazelden.getCharacterOnset();
		if (wallyHazelden.getPosition() != null)
			this.position = wallyHazelden.getPosition().getClass()
					.getDeclaredConstructor(wallyHazelden.getPosition().getClass())
					.newInstance(wallyHazelden.getPosition());
		for (int j = 0; j < wallyHazelden.getTeams().size(); j++) {
			if (wallyHazelden.getTeams().get(j) != null) {
				teams.add(wallyHazelden.getTeams().get(j).getClass()
						.getDeclaredConstructor(wallyHazelden.getTeams().get(j).getClass())
						.newInstance(wallyHazelden.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "WallyHazelden [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
