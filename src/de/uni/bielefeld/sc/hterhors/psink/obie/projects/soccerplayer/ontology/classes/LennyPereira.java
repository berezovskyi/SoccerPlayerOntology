...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ILennyPereira;
...
public class LennyPereira implements ILennyPereira {
	...
	public LennyPereira(LennyPereira lennyPereira) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = lennyPereira.getAnnotationID();
		for (int j = 0; j < lennyPereira.getBirthPlaces().size(); j++) {
			if (lennyPereira.getBirthPlaces().get(j) != null) {
				birthPlaces.add(lennyPereira.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(lennyPereira.getBirthPlaces().get(j).getClass())
						.newInstance(lennyPereira.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (lennyPereira.getBirthYear() != null)
			this.birthYear = lennyPereira.getBirthYear().getClass()
					.getDeclaredConstructor(lennyPereira.getBirthYear().getClass())
					.newInstance(lennyPereira.getBirthYear());
		this.characterOffset = lennyPereira.getCharacterOffset();
		this.characterOnset = lennyPereira.getCharacterOnset();
		if (lennyPereira.getPosition() != null)
			this.position = lennyPereira.getPosition().getClass()
					.getDeclaredConstructor(lennyPereira.getPosition().getClass())
					.newInstance(lennyPereira.getPosition());
		for (int j = 0; j < lennyPereira.getTeams().size(); j++) {
			if (lennyPereira.getTeams().get(j) != null) {
				teams.add(lennyPereira.getTeams().get(j).getClass()
						.getDeclaredConstructor(lennyPereira.getTeams().get(j).getClass())
						.newInstance(lennyPereira.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "LennyPereira [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
