...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.ISolomonMarch;
...
public class SolomonMarch implements ISolomonMarch {
	...
	public SolomonMarch(SolomonMarch solomonMarch) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = solomonMarch.getAnnotationID();
		for (int j = 0; j < solomonMarch.getBirthPlaces().size(); j++) {
			if (solomonMarch.getBirthPlaces().get(j) != null) {
				birthPlaces.add(solomonMarch.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(solomonMarch.getBirthPlaces().get(j).getClass())
						.newInstance(solomonMarch.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (solomonMarch.getBirthYear() != null)
			this.birthYear = solomonMarch.getBirthYear().getClass()
					.getDeclaredConstructor(solomonMarch.getBirthYear().getClass())
					.newInstance(solomonMarch.getBirthYear());
		this.characterOffset = solomonMarch.getCharacterOffset();
		this.characterOnset = solomonMarch.getCharacterOnset();
		if (solomonMarch.getPosition() != null)
			this.position = solomonMarch.getPosition().getClass()
					.getDeclaredConstructor(solomonMarch.getPosition().getClass())
					.newInstance(solomonMarch.getPosition());
		for (int j = 0; j < solomonMarch.getTeams().size(); j++) {
			if (solomonMarch.getTeams().get(j) != null) {
				teams.add(solomonMarch.getTeams().get(j).getClass()
						.getDeclaredConstructor(solomonMarch.getTeams().get(j).getClass())
						.newInstance(solomonMarch.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "SolomonMarch [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
