...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IAndyMoule;
...
public class AndyMoule implements IAndyMoule {
	...
	public AndyMoule(AndyMoule andyMoule) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = andyMoule.getAnnotationID();
		for (int j = 0; j < andyMoule.getBirthPlaces().size(); j++) {
			if (andyMoule.getBirthPlaces().get(j) != null) {
				birthPlaces.add(andyMoule.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(andyMoule.getBirthPlaces().get(j).getClass())
						.newInstance(andyMoule.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (andyMoule.getBirthYear() != null)
			this.birthYear = andyMoule.getBirthYear().getClass()
					.getDeclaredConstructor(andyMoule.getBirthYear().getClass()).newInstance(andyMoule.getBirthYear());
		this.characterOffset = andyMoule.getCharacterOffset();
		this.characterOnset = andyMoule.getCharacterOnset();
		if (andyMoule.getPosition() != null)
			this.position = andyMoule.getPosition().getClass()
					.getDeclaredConstructor(andyMoule.getPosition().getClass()).newInstance(andyMoule.getPosition());
		for (int j = 0; j < andyMoule.getTeams().size(); j++) {
			if (andyMoule.getTeams().get(j) != null) {
				teams.add(andyMoule.getTeams().get(j).getClass()
						.getDeclaredConstructor(andyMoule.getTeams().get(j).getClass())
						.newInstance(andyMoule.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "AndyMoule [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
