...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMariaMitkou;
...
public class MariaMitkou implements IMariaMitkou {
	...
	public MariaMitkou(MariaMitkou mariaMitkou) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = mariaMitkou.getAnnotationID();
		for (int j = 0; j < mariaMitkou.getBirthPlaces().size(); j++) {
			if (mariaMitkou.getBirthPlaces().get(j) != null) {
				birthPlaces.add(mariaMitkou.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(mariaMitkou.getBirthPlaces().get(j).getClass())
						.newInstance(mariaMitkou.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (mariaMitkou.getBirthYear() != null)
			this.birthYear = mariaMitkou.getBirthYear().getClass()
					.getDeclaredConstructor(mariaMitkou.getBirthYear().getClass())
					.newInstance(mariaMitkou.getBirthYear());
		this.characterOffset = mariaMitkou.getCharacterOffset();
		this.characterOnset = mariaMitkou.getCharacterOnset();
		if (mariaMitkou.getPosition() != null)
			this.position = mariaMitkou.getPosition().getClass()
					.getDeclaredConstructor(mariaMitkou.getPosition().getClass())
					.newInstance(mariaMitkou.getPosition());
		for (int j = 0; j < mariaMitkou.getTeams().size(); j++) {
			if (mariaMitkou.getTeams().get(j) != null) {
				teams.add(mariaMitkou.getTeams().get(j).getClass()
						.getDeclaredConstructor(mariaMitkou.getTeams().get(j).getClass())
						.newInstance(mariaMitkou.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MariaMitkou [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear=" + birthYear
