...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.soccerplayer.ontology.interfaces.IMathieuDeplagne;
...
public class MathieuDeplagne implements IMathieuDeplagne {
	...
	public MathieuDeplagne(MathieuDeplagne mathieuDeplagne) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		this.annotationID = mathieuDeplagne.getAnnotationID();
		for (int j = 0; j < mathieuDeplagne.getBirthPlaces().size(); j++) {
			if (mathieuDeplagne.getBirthPlaces().get(j) != null) {
				birthPlaces.add(mathieuDeplagne.getBirthPlaces().get(j).getClass()
						.getDeclaredConstructor(mathieuDeplagne.getBirthPlaces().get(j).getClass())
						.newInstance(mathieuDeplagne.getBirthPlaces().get(j)));
			} else {
				birthPlaces.add(null);
			}
		}
		if (mathieuDeplagne.getBirthYear() != null)
			this.birthYear = mathieuDeplagne.getBirthYear().getClass()
					.getDeclaredConstructor(mathieuDeplagne.getBirthYear().getClass())
					.newInstance(mathieuDeplagne.getBirthYear());
		this.characterOffset = mathieuDeplagne.getCharacterOffset();
		this.characterOnset = mathieuDeplagne.getCharacterOnset();
		if (mathieuDeplagne.getPosition() != null)
			this.position = mathieuDeplagne.getPosition().getClass()
					.getDeclaredConstructor(mathieuDeplagne.getPosition().getClass())
					.newInstance(mathieuDeplagne.getPosition());
		for (int j = 0; j < mathieuDeplagne.getTeams().size(); j++) {
			if (mathieuDeplagne.getTeams().get(j) != null) {
				teams.add(mathieuDeplagne.getTeams().get(j).getClass()
						.getDeclaredConstructor(mathieuDeplagne.getTeams().get(j).getClass())
						.newInstance(mathieuDeplagne.getTeams().get(j)));
			} else {
				teams.add(null);
			}
		}
	...
	public String toString() {
		return "MathieuDeplagne [annotationID=" + annotationID + ",birthPlaces=" + birthPlaces + ",birthYear="
